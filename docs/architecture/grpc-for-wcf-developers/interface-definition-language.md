...
gRPC utilise le langage IDL (Interface Definition Language) à partir des mémoires tampons de protocole. L’IDL de mémoires tampons de protocole est un langage personnalisé, indépendant de la plateforme, avec une spécification ouverte. Les développeurs créent des `.proto` fichiers pour décrire les services, ainsi que leurs entrées et sorties. Ces `.proto` fichiers peuvent ensuite être utilisés pour générer des stubs spécifiques à une langue ou à une plateforme pour les clients et les serveurs, ce qui permet à plusieurs plateformes de communiquer. En partageant des `.proto` fichiers, les équipes peuvent générer du code pour utiliser les services de chacun d’eux, sans avoir à prendre de dépendance de code.
