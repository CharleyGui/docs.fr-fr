...
La propriété `apiVersion` est utilisée pour spécifier la version (et l’API) à laquelle le fichier est destiné. La propriété `kind` spécifie le type d’objet représenté par le YAML. La propriété `metadata` contient des propriétés d’objet telles que `name`, `namespace` ou `labels`.
...
Pour vérifier que votre cluster est en cours d’exécution et accessible, exécutez la commande `kubectl version`.
...
Utilisez `kubectl` pour appliquer le fichier `namespace.yml` et vérifier que l’espace de noms est correctement créé.
...
La partie déploiement fournit les `spec` pour le déploiement proprement dit, y compris le nombre de réplicas requis et un `template` pour les objets Pod à créer et à gérer par le déploiement. Notez que les objets de déploiement sont gérés avec l’API `apps`, comme spécifié dans `apiVersion`, plutôt que l’API Kubernetes principale.
...
> [!NOTE]
> ASP.NET Core 3,0 a été optimisé et réglé pour s’exécuter dans des conteneurs à ressources limitées, et l’image de l' `dotnet/core/aspnet` d’ancrage définit une variable d’environnement pour indiquer au runtime `dotnet` qu’il se trouve dans un conteneur.
...
Pour rendre l’application Web accessible depuis l’extérieur du cluster, la propriété `type: NodePort` est utilisée. Ce type de propriété force Kubernetes à publier le port 80 sur le service sur un port arbitraire sur les sockets réseau externes du cluster. Le port affecté peut être trouvé à l’aide de la commande `kubectl get service`.

Le service `stockdata` ne doit pas être accessible depuis l’extérieur du cluster, il a donc utilisé le type par défaut, `ClusterIP`.
...
Deuxièmement, le `HttpClient` utilisé pour le client gRPC de l’application StockWeb est créé et géré par le [ASP.net Core HttpClientFactory](../microservices/implement-resilient-applications/use-httpclientfactory-to-implement-resilient-http-requests.md), et une seule instance de ce client est utilisée pour chaque appel à la page. Le client n’effectue qu’une seule recherche DNS, donc toutes les demandes sont acheminées vers la même adresse IP. En outre, étant donné que le `HttpClientHandler` est mis en cache pour des raisons de performances, plusieurs demandes en succession rapide utilisent *toutes* la même adresse IP, jusqu’à ce que l’entrée DNS mise en cache expire ou que l’instance de gestionnaire soit supprimée pour une raison quelconque.
