...
* Gérer l’état via un moteur de workflow comme des fonctions durables
...
Il est également important de prendre en compte le *modèle*de disjoncteur: Si, pour une raison quelconque, un service continue d’échouer, il n’est pas recommandé d’appeler ce service à plusieurs reprises. Au lieu de cela, un autre service est appelé ou un message est retourné jusqu’à ce que l’intégrité du service dépendant soit rétablie. L’architecture sans serveur doit prendre en compte la stratégie de résolution et de gestion des dépendances entre les services.
