...
### <a name="step-2-create-a-dockerfile-related-to-an-existing-image-plain-os-or-dev-environments-like-net-nodejs-and-ruby"></a>Étape 2 : créer un fichier dockerfile lié à une image existante (environnements de système d’exploitation ou de développement simples tels que .NET, Node.js et Ruby)
...
Lorsque vous ajoutez un fichier dockerfile, vous spécifiez l’image de base que vous utiliserez (comme avec `FROM mcr.microsoft.com/dotnet/aspnet` ). En général, vous générez votre image personnalisée au-dessus d’une image de base que vous obtenez à partir de n’importe quel dépôt officiel dans le Registre du Hub de l' [ancreur](https://hub.docker.com/) (comme une [image pour .net](https://hub.docker.com/_/microsoft-dotnet/) ou celle [de Node.js](https://hub.docker.com/_/node/)).
...
Voici un exemple de fichier dockerfile pour un conteneur .NET :
...
Dans ce cas, l’image est basée sur la version 5,0 de l’image officielle de l’Ancreur ASP.NET Core (multi-arch pour Linux et Windows), conformément à la ligne `FROM mcr.microsoft.com/dotnet/aspnet:5.0` . (Pour plus d’informations sur cette rubrique, reportez-vous à la page image de l' [ancreur ASP.net Core](https://hub.docker.com/_/microsoft-dotnet-aspnet/) et à la page d’image de l' [ancreur .net](https://hub.docker.com/_/microsoft-dotnet/) ).
...
Vous pouvez spécifier des paramètres de configuration supplémentaires dans le fichier Dockerfile, en fonction du langage et du framework que vous utilisez. Par exemple, la `ENTRYPOINT` ligne avec `["dotnet", "WebMvcApplication.dll"]` indique à dockr d’exécuter une application .net. Si vous utilisez le kit de développement logiciel (SDK) et l’interface CLI .NET ( `dotnet CLI` ) pour générer et exécuter l’application .net, ce paramètre est différent. L’essentiel à retenir est que la ligne ENTRYPOINT et certains autres paramètres varient selon le langage et la plateforme choisis pour votre application.

> [!TIP]
> Pour plus d’informations sur la création d’images de l’arrimeur pour les applications .NET, consultez <https://docs.microsoft.com/dotnet/core/docker/building-net-docker-images> .
>
...
Dans une API Web .NET simple « Hello World » déployée en tant que conteneur ou service unique, vous devez simplement accéder au service en fournissant le port TCP spécifié dans le fichier dockerfile.
...
Visual Studio Code prend en charge le débogage de l’arrimeur si vous utilisez Node.js et d’autres plateformes, telles que des conteneurs .NET.

Vous pouvez également déboguer des conteneurs .NET ou .NET Framework dans docker lors de l’utilisation de Visual Studio pour Windows ou Mac, comme décrit dans la section suivante.
