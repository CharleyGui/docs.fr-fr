...
L’utilisation de la communication asynchrone est expliquée avec des détails supplémentaires plus loin dans ce guide dans les sections [intégration de microservices asynchrones, qui applique l’autonomie du microservice et la](communication-in-microservice-architecture.md#asynchronous-microservice-integration-enforces-microservices-autonomy) [communication asynchrone basée sur les messages](asynchronous-message-based-communication.md).
...
- **Charles Row. ACID et BASE : le pH du traitement des transactions de base de données** \
