...
---
# <a name="modernizing-desktop-apps-on-windows-10-with-net-core-31"></a>Moderniser des applications de bureau sur Windows 10 avec .NET Core 3,1
...
Nous avons rédigé cet ouvrage pour les développeurs et les architectes de solutions qui veulent moderniser les applications de bureau Windows Forms et WPF existantes pour tirer parti des avantages de .NET Core et de Windows 10.
...
Ce livre aborde le « pourquoi », pour vous aider à moderniser vos applications existantes et les avantages spécifiques que vous pouvez vous procurer à l’aide de NET Core 3,1 et MSIX pour moderniser vos applications de bureau. Le contenu du livre est conçu pour les architectes et les décideurs techniques qui souhaitent une vue d’ensemble, mais qui n’ont pas besoin de se concentrer sur la mise en œuvre et les détails techniques, pas à pas.
...
Pour mettre en évidence les étapes nécessaires pour effectuer une modernisation, nous allons utiliser un exemple d’application appelé `eShopModernizing` . Cette application a deux versions, Windows Forms et WPF, et nous vous présenterons un processus pas à pas sur la façon d’effectuer la modernisation des deux à la fois sur .NET Core.
...
>[Suivant](why-modern-applications.md)
