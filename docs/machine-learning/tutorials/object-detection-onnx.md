...
- [Modèle préentraîné Tiny YOLOv2](https://github.com/onnx/models/tree/master/vision/object_detection_segmentation/tiny-yolov2)
...
1. Installez le **package NuGet Microsoft.ML** :
    ...
    - Répétez ces étapes pour **Microsoft. ml. ImageAnalytics**, **Microsoft. ml. OnnxTransformer** et **Microsoft. ml. OnnxRuntime**.

### <a name="prepare-your-data-and-pre-trained-model"></a>Préparer vos données et votre modèle préentraîné

1. Téléchargez [le fichier zip du répertoire des ressources du projet](https://github.com/dotnet/machinelearning-samples/raw/master/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/assets.zip) et décompressez-le.
...
1. Téléchargez le [modèle Tiny YOLOv2](https://onnxzoo.blob.core.windows.net/models/opset_8/tiny_yolov2/tiny_yolov2.tar.gz) depuis [ONNX Model Zoo](https://github.com/onnx/models/tree/master/vision/object_detection_segmentation/tiny-yolov2) et décompressez-le.
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément**, sélectionnez **Classe** et remplacez la valeur du champ **Nom** par *ImageNetData.cs*. Ensuite, sélectionnez le bouton **Ajouter**.
    ...
    En outre, `ImageNetData` contient une méthode `ReadFromFile` qui charge plusieurs fichiers image stockés dans le `imageFolder` chemin d’accès spécifié et les retourne sous la forme d’une collection d' `ImageNetData` objets.
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément**, sélectionnez **Classe** et remplacez la valeur du champ **Nom** par *ImageNetPrediction.cs*. Ensuite, sélectionnez le bouton **Ajouter**.
    ...
    - `PredictedLabel` contient les dimensions, le score d’objet et les probabilités de classe pour chacun des rectangles englobants détectés dans une image.
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément**, sélectionnez **Classe** et remplacez la valeur du champ **Nom** par *YoloBoundingBox.cs*. Ensuite, sélectionnez le bouton **Ajouter**.
    ...
    Juste au-dessus de la définition de classe existante, ajoutez une nouvelle définition de classe nommée `BoundingBoxDimensions` qui hérite de la `DimensionsBase` classe pour contenir les dimensions de la zone englobante respective.
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément**, sélectionnez **Classe**, puis remplacez le champ **Nom** par *YoloOutputParser.cs*. Ensuite, sélectionnez le bouton **Ajouter**.
    ...
    Dans la définition de la classe `YoloOutputParser` existante, ajoutez une classe imbriquée qui contient les dimensions de chacune des cellules de l’image. Ajoutez le code suivant pour la `CellDimensions` classe qui hérite de la `DimensionsBase` classe en haut de la `YoloOutputParser` définition de classe.

    [!code-csharp [YoloParserUsings](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/YoloParser/YoloOutputParser.cs#L10)]

1. À l’intérieur de la `YoloOutputParser` définition de classe, ajoutez les constantes et les champs suivants.
...
- `GetConfidence` extrait la valeur de confiance qui indique la manière dont le modèle a détecté un objet et utilise la `Sigmoid` fonction pour le convertir en pourcentage.
...
1. Dans la boîte de dialogue **Ajouter un nouvel élément**, sélectionnez **Classe** et remplacez la valeur du champ **Nom** par *OnnxModelScorer.cs*. Ensuite, sélectionnez le bouton **Ajouter**.
    ...
    Après cela, créez un autre struct appelé `TinyYoloModelSettings` qui contient les noms des couches d’entrée et de sortie du modèle. Pour visualiser le nom des couches d’entrée et de sortie du modèle, vous pouvez utiliser un outil comme [Netron](https://github.com/lutzroeder/netron).
    ...
    Les pipelines ML.NET doivent connaître le schéma de données à utiliser lorsque la [`Fit`](xref:Microsoft.ML.IEstimator%601.Fit%2A) méthode est appelée. Dans ce cas, un processus similaire à l’entraînement sera utilisé. Toutefois, étant donné qu’il n’y a pas d’apprentissage réel, il est acceptable d’utiliser un vide [`IDataView`](xref:Microsoft.ML.IDataView) . Créez un nouveau [`IDataView`](xref:Microsoft.ML.IDataView) pour le pipeline à partir d’une liste vide.

    [!code-csharp [LoadEmptyIDV](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L52)]

    En dessous, définissez le pipeline. Le pipeline se compose de quatre transformations.

    - [`LoadImages`](xref:Microsoft.ML.ImageEstimatorsCatalog.LoadImages%2A) charge l’image en tant que bitmap.
    - [`ResizeImages`](xref:Microsoft.ML.ImageEstimatorsCatalog.ResizeImages%2A) redimensionne l’image à la taille spécifiée (dans ce cas, `416 x 416` ).
    - [`ExtractPixels`](xref:Microsoft.ML.ImageEstimatorsCatalog.ExtractPixels%2A) modifie la représentation en pixels de l’image d’une image bitmap en un vecteur numérique.
    - [`ApplyOnnxModel`](xref:Microsoft.ML.OnnxCatalog.ApplyOnnxModel%2A) charge le modèle ONNX et l’utilise pour noter les données fournies.

    Définissez votre pipeline dans la méthode `LoadModel` sous la variable `data`.

    [!code-csharp [ScoringPipeline](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/OnnxModelScorer.cs#L55-L58)]

    Il est maintenant temps d’instancier le modèle pour le scoring. Appelez la [`Fit`](xref:Microsoft.ML.IEstimator%601.Fit%2A) méthode sur le pipeline et renvoyez-la pour un traitement supplémentaire.
...
Ensuite, utilisez la [`Transform`](xref:Microsoft.ML.ITransformer.Transform%2A) méthode pour noter les données.
...
Dans le bloc `try`, commencez à implémenter la logique de détection d’objet. Tout d’abord, chargez les données dans un [`IDataView`](xref:Microsoft.ML.IDataView) .
...
Pour dessiner sur l’image, convertissez-la en un [`Graphics`](xref:System.Drawing.Graphics) objet.
...
Dans le `using` bloc de code, ajustez les [`Graphics`](xref:System.Drawing.Graphics) paramètres d’objet du graphique.
...
Créez et remplissez un rectangle au-dessus du cadre englobant pour contenir le texte à l’aide de la [`FillRectangle`](xref:System.Drawing.Graphics.FillRectangle%2A) méthode. Cela permettra de distinguer le texte et d’améliorer la lisibilité.

[!code-csharp [DrawTextBackground](~/machinelearning-samples/samples/csharp/getting-started/DeepLearning_ObjectDetection_Onnx/ObjectDetectionConsoleApp/Program.cs#L117)]

Dessinez ensuite le texte et le cadre englobant sur l’image à l’aide des [`DrawString`](xref:System.Drawing.Graphics.DrawString%2A) [`DrawRectangle`](xref:System.Drawing.Graphics.DrawRectangle%2A) méthodes et.
