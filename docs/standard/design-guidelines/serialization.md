 ...
 **✓ CONSIDER** prenant en charge la sérialisation du Runtime si vos types seront utilisées avec la communication à distance .NET. Par exemple, l’espace de noms <xref:System.AddIn?displayProperty=nameWithType> utilise .NET Remoting, et tous les types échangés entre les compléments `System.AddIn` doivent prendre en charge la sérialisation du Runtime.  
