...
1. Pour chaque ajustement (c’est-à-dire, pour chaque transition en dehors et vers l’heure standard sur un intervalle de temps donné), procédez comme suit :
    ...
    1. Définissez l’heure de début de la transition pour le réglage du fuseau horaire.
       ...
       Vous devez appeler la méthode <xref:System.TimeZoneInfo.TransitionTime.CreateFloatingDateRule%2A?displayProperty=nameWithType> et lui transmettre une valeur <xref:System.DateTime> qui définit l’heure de la transition, une valeur entière qui définit le mois de la transition, une valeur entière qui définit la semaine au niveau de laquelle la transition se produit et une valeur de <xref:System.DayOfWeek> qui définit jour de la semaine où la transition se produit. Cet appel de méthode instancie un objet <xref:System.TimeZoneInfo.TransitionTime>.

    2. Définissez l’heure de fin de la transition pour le réglage du fuseau horaire. Cela nécessite un autre appel à la méthode <xref:System.TimeZoneInfo.TransitionTime.CreateFloatingDateRule%2A?displayProperty=nameWithType>. Cet appel de méthode instancie un deuxième <xref:System.TimeZoneInfo.TransitionTime> objet.

    3. Appelez la méthode <xref:System.TimeZoneInfo.AdjustmentRule.CreateAdjustmentRule%2A> et passez-lui les dates de début et de fin effectives de l’ajustement, un objet <xref:System.TimeSpan> qui définit la durée de la transition et les deux objets <xref:System.TimeZoneInfo.TransitionTime> qui définissent le moment où les transitions vers et depuis l’heure d’été se produisent. Cet appel de méthode instancie un objet <xref:System.TimeZoneInfo.AdjustmentRule>.
...
6. Instanciez un objet <xref:System.TimeSpan> qui définit le décalage du fuseau horaire par rapport à l’heure UTC. Les fuseaux horaires dont les heures sont postérieures à l’heure UTC ont un décalage positif. Les fuseaux horaires dont les heures sont antérieures à l’heure UTC ont un décalage négatif.

7. Appelez la méthode <xref:System.TimeZoneInfo.CreateCustomTimeZone%28System.String%2CSystem.TimeSpan%2CSystem.String%2CSystem.String%2CSystem.String%2CSystem.TimeZoneInfo.AdjustmentRule%5B%5D%29?displayProperty=nameWithType> pour instancier le nouveau fuseau horaire.
...
Pour les règles d’ajustement flottantes, la valeur 5 est passée au paramètre `week` de la méthode <xref:System.TimeZoneInfo.TransitionTime.CreateFloatingDateRule%2A> pour indiquer que la transition se produit à la dernière semaine d’un mois donné.

Lors de la création du tableau d’objets <xref:System.TimeZoneInfo.AdjustmentRule> à utiliser dans l’appel de méthode <xref:System.TimeZoneInfo.CreateCustomTimeZone%28System.String%2CSystem.TimeSpan%2CSystem.String%2CSystem.String%2CSystem.String%2CSystem.TimeZoneInfo.AdjustmentRule%5B%5D%29?displayProperty=nameWithType>, le code peut initialiser le tableau à la taille requise par le nombre d’ajustements à créer pour le fuseau horaire. Au lieu de cela, cet exemple de code appelle la méthode <xref:System.Collections.Generic.List%601.Add%2A> pour ajouter chaque règle d’ajustement à une collection générique <xref:System.Collections.Generic.List%601> d’objets <xref:System.TimeZoneInfo.AdjustmentRule>. Le code appelle ensuite la méthode <xref:System.Collections.Generic.List%601.CopyTo%2A> pour copier les membres de cette collection dans le tableau.

L’exemple utilise également la méthode <xref:System.TimeZoneInfo.TransitionTime.CreateFixedDateRule%2A> pour définir des ajustements de date fixe. Cela est similaire à l’appel de la méthode <xref:System.TimeZoneInfo.TransitionTime.CreateFloatingDateRule%2A>, sauf qu’elle nécessite uniquement l’heure, le mois et le jour des paramètres de transition.
