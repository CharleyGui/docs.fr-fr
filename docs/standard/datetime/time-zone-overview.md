...
- <xref:System.TimeZoneInfo.ConvertTimeBySystemTimeZoneId%2A>, qui utilise des identificateurs de fuseau horaire ( <xref:System.TimeZoneInfo> au lieu d’objets) comme paramètres pour convertir l’heure d’un fuseau horaire désigné en heure d’un autre fuseau horaire désigné.
