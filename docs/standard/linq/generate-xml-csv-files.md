...
---
# <a name="how-to-generate-xml-from-csv-files-linq-to-xml"></a>Génération de code XML à partir de fichiers CSV (LINQ to XML)

Vous pouvez utiliser LINQ to XML en C# ou Visual Basic pour créer du code XML à partir d’un fichier de valeurs séparées par des virgules (CSV).

## <a name="example-generate-xml-from-a-csv-file"></a>Exemple : générer du code XML à partir d’un fichier CSV

Cet exemple exécute une requête LINQ sur un tableau de chaînes, puis utilise la `let` clause pour fractionner chaque chaîne en un tableau de champs.
