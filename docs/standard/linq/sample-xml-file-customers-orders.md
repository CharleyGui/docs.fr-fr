...
description: Ce fichier XML, qui est utilisé dans les exemples, contient des données sur les clients et les commandes.
ms.date: 07/20/2015
ms.assetid: d6d1c9ea-be74-4e6d-bfdd-d4bcc2d301cf
ms.openlocfilehash: 244a17069e050c88fc74c3c7a2cb427d83d5e1a7
ms.sourcegitcommit: 0c3ce6d2e7586d925a30f231f32046b7b3934acb
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/08/2020
ms.locfileid: "89553604"
---
# <a name="sample-xml-file-customers-and-orders-linq-to-xml"></a><span data-ttu-id="45fd7-103">Exemple de fichier XML : clients et commandes (LINQ to XML)</span><span class="sxs-lookup"><span data-stu-id="45fd7-103">Sample XML file: Customers and orders (LINQ to XML)</span></span>

<span data-ttu-id="45fd7-104">Le fichier XML suivant est utilisé dans différents exemples de la documentation de LINQ to XML.</span><span class="sxs-lookup"><span data-stu-id="45fd7-104">The following XML file is used in various examples in the LINQ to XML documentation.</span></span> <span data-ttu-id="45fd7-105">Ce fichier contient des clients et des commandes.</span><span class="sxs-lookup"><span data-stu-id="45fd7-105">This file contains customers and orders.</span></span>

<span data-ttu-id="45fd7-106">L’article [exemple de fichier XSD : Customers et Orders](sample-xsd-file-customers-orders.md) contient un XSD qui peut être utilisé pour valider ce document.</span><span class="sxs-lookup"><span data-stu-id="45fd7-106">The article [Sample XSD file: Customers and orders](sample-xsd-file-customers-orders.md) contains an XSD that can be used to validate this document.</span></span> <span data-ttu-id="45fd7-107">Il utilise les fonctionnalités `xs:key` et `xs:keyref` de XSD pour déterminer que l'attribut `CustomerID` de l'élément `Customer` est une clé et pour établir une relation entre l'élément `CustomerID` dans chaque élément `Order` et l'attribut `CustomerID` dans chaque élément `Customer`.</span><span class="sxs-lookup"><span data-stu-id="45fd7-107">It uses the `xs:key` and `xs:keyref` features of XSD to establish that the `CustomerID` attribute of the `Customer` element is a key, and to establish a relationship between the `CustomerID` element in each `Order` element and the `CustomerID` attribute in each `Customer` element.</span></span>

<span data-ttu-id="45fd7-108">Pour obtenir un exemple d’écriture de requêtes LINQ qui tirent parti de cette relation à l’aide de la `Join` clause, consultez [Comment joindre deux collections](join-two-collections.md).</span><span class="sxs-lookup"><span data-stu-id="45fd7-108">For an example of writing LINQ queries that take advantage of this relationship using the `Join` clause, see [How to join two collections](join-two-collections.md).</span></span>

## <a name="customersordersxml"></a><span data-ttu-id="45fd7-109">CustomersOrders.xml</span><span class="sxs-lookup"><span data-stu-id="45fd7-109">CustomersOrders.xml</span></span>
