...
---
# <a name="how-to-sort-elements-on-multiple-keys-linq-to-xml"></a>Comment trier des éléments sur plusieurs clés (LINQ to XML)

Cet article montre comment effectuer un tri sur plusieurs clés en C# et Visual Basic.

## <a name="example-sort-xml-elements-on-multiple-keys"></a>Exemple : trier des éléments XML sur plusieurs clés

Cet exemple trie sur deux clés : le code postal d’expédition (principal) et la date de commande. Il utilise le document XML [exemple de fichier XML : Customers et Orders](sample-xml-file-customers-orders.md).
...
## <a name="example-sort-xml-thats-in-a-namespace"></a>Exemple : Trier du code XML qui se trouve dans un espace de noms

Cet exemple effectue le même tri que le premier, mais pour le code XML qui se trouve dans un espace de noms. Il utilise le document XML [exemple de fichier XML : clients et commandes dans un espace de noms](sample-xml-file-customers-orders-namespace.md).
