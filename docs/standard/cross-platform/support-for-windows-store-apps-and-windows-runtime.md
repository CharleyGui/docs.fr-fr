...
- Développement d’applications du Windows 8. x Store avec des contrôles XAML, comme décrit dans feuille [de route C# pour les applications du Windows Store à l’aide de ou de Visual Basic, guide](https://docs.microsoft.com/previous-versions/windows/apps/br229583(v=win.10))pratique [(XAML)](https://docs.microsoft.com/previous-versions/windows/apps/br229566(v=win.10))et [.net pour les applications du Windows Store](https://docs.microsoft.com/previous-versions/windows/apps/br230302(v=vs.140)).
...
Le .NET Framework prend en charge les trois scénarios de développement répertoriés précédemment en fournissant des [!INCLUDE[net_win8_profile](../../../includes/net-win8-profile-md.md)]et en prenant en charge le Windows Runtime lui-même.
...
La façon dont les interfaces apparaissent dans le code managé affecte la façon dont les types qui implémentent ces interfaces apparaissent. Par exemple, la classe `PropertySet` implémente `IMap<K, V>`, qui s'affiche dans le code managé sous la forme `IDictionary<TKey, TValue>`. `PropertySet` s’affiche comme s’il implémentait `IDictionary<TKey, TValue>` au lieu de `IMap<K, V>`. par conséquent, dans le code managé, il semble avoir une méthode `Add`, qui se comporte comme la méthode `Add` sur les dictionnaires .NET Framework. Il ne semble pas avoir de méthode `Insert`.
...
- Pour le Windows Runtime primitives `Int32`, `Int64`, `Single`, `Double`, `Boolean`, `String` (une collection immuable de caractères Unicode), `Enum`, `UInt32`, `UInt64`et `Guid`, utilisez le type du même nom dans l’espace de noms `System`.
...
D’autres types de Windows Runtime couramment utilisés qui apparaissent dans du code managé comme leurs équivalents .NET Framework incluent la structure `Windows.Foundation.DateTime`, qui apparaît dans le code managé comme la structure <xref:System.DateTimeOffset?displayProperty=nameWithType>, et la structure `Windows.Foundation.TimeSpan`, qui apparaît comme la structure <xref:System.TimeSpan?displayProperty=nameWithType>.
...
Vous pouvez empaqueter du code managé à partir de vos applications de bureau existantes en tant que composant Windows Runtime autonome. Cela vous permet d’utiliser le composant dans les applications du Windows 8. x Store C++ créées à l’aide de ou de JavaScript, ainsi que dans les applications du C# Windows 8. x Store générées à l’aide de ou de Visual Basic. Le contrôle de version est une option s’il existe plusieurs scénarios de réutilisation du code.
