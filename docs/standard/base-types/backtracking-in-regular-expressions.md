...
## <a name="linear-comparison-without-backtracking"></a>Comparaison linéaire sans rétroaction  
 ...
 L'exemple suivant en est l'illustration. L'expression régulière `e{2}\w\b` recherche deux occurrences de la lettre « e » suivie de n'importe quel caractère de mot suivi d'une limite de mot.  
...
### <a name="atomic-groups"></a>Groupes atomiques
 L’élément `(?>` `)` de langage *de sous-expression* supprime le retour en arrière dans la sous-expression. Une fois qu’il a réussi à égaler, il ne renoncera à aucune partie de son match à retour ultérieur. Par exemple, dans `(?>\w*\d*)1`le `1` modèle , si `\d*` le ne peut pas être égalé, le `1` ne renoncera à aucun de ses matchs, même si cela signifie qu’il permettrait à l’assortir avec succès. Les groupes atomiques peuvent aider à prévenir les problèmes de performance associés aux matchs ratés.
...
### <a name="lookbehind-assertions"></a>assertions de postanalyse  
 .NET comprend deux `(?<=`éléments de langage, *la sous-expression* `)` et `(?<!`la *sous-expression*`)`, qui correspondent au caractère précédent ou des caractères dans la chaîne d’entrée. Les deux éléments de langage sont des assertions de largeur nulle ; c’est-à-dire qu’ils déterminent si le ou les caractères qui précèdent immédiatement le caractère actuel peuvent être mis en correspondance par la *sous-expression*, sans avancer ou utiliser la rétroaction.  
  
 `(?<=`*la sous-expression* `)` est une affirmation positive; c’est-à-dire que le personnage ou les personnages avant la position actuelle doivent correspondre *à la sous-expression*. `(?<!`*la sous-expression* `)` est une affirmation négative de lookbehind ; c’est-à-dire que le personnage ou les personnages avant la position actuelle ne doivent pas correspondre *à la sous-expression*. Les affirmations positives et négatives de lookbehind sont plus utiles quand *la sous-expression* est un sous-ensemble de la sous-expression précédente.  
...
### <a name="lookahead-assertions"></a>assertions de préanalyse  
 .NET comprend deux `(?=`éléments de langage, *la sous-expression* `)` et `(?!`la *sous-expression*`)`, qui correspondent au caractère suivant ou des caractères dans la chaîne d’entrée. Les deux éléments de langage sont des assertions de largeur nulle ; c’est-à-dire qu’ils déterminent si le ou les caractères qui suivent immédiatement le caractère actuel peuvent être mis en correspondance par la *sous-expression*, sans avancer ou utiliser la rétroaction.  
  
 `(?=`*la sous-expression* `)` est une affirmation positive de la tête d’homme; c’est-à-dire que le personnage ou les personnages après la position actuelle doivent correspondre *à la sous-expression*. `(?!`*la sous-expression* `)` est une affirmation négative de la tête d’homme; c’est-à-dire que le personnage ou les personnages après la position actuelle ne doivent pas correspondre *à la sous-expression*. Les affirmations positives et négatives de la tête de regard sont plus utiles lorsque la *sous-expression* est un sous-ensemble de la sous-expression suivante.  
