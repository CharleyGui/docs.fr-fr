...
Cet article explique comment créer des convertisseurs personnalisés pour les classes de sérialisation JSON fournies dans l' <xref:[!OP.NO-LOC(System.Text.Json)]> espace de noms. Pour une introduction à `[!OP.NO-LOC(System.Text.Json)]`, consultez [comment sérialiser et désérialiser JSON dans .net](system-text-json-how-to.md).

Un *convertisseur* est une classe qui convertit un objet ou une valeur vers et à partir de JSON. L' `[!OP.NO-LOC(System.Text.Json)]` espace de noms contient des convertisseurs intégrés pour la plupart des types primitifs qui mappent aux primitives JavaScript. Vous pouvez écrire des convertisseurs personnalisés :

* Pour remplacer le comportement par défaut d’un convertisseur intégré. Par exemple, vous pouvez souhaiter `DateTime` que les valeurs soient représentées par le format mm/jj/aaaa au lieu du format ISO 8601-1:2019 par défaut.
* Pour prendre en charge un type valeur personnalisé. Par exemple, un `PhoneNumber` struct.

Vous pouvez également écrire des convertisseurs personnalisés pour personnaliser ou `[!OP.NO-LOC(System.Text.Json)]` étendre des fonctionnalités qui ne sont pas incluses dans la version actuelle. Les scénarios suivants sont abordés plus loin dans cet article :
...
Il existe deux modèles pour créer un convertisseur personnalisé : le modèle de base et le modèle de fabrique. Le modèle de fabrique est destiné aux convertisseurs qui `Enum` gèrent le type ou les génériques ouverts. Le modèle de base concerne les types génériques non génériques et fermés.  Par exemple, les convertisseurs pour les types suivants requièrent le modèle de fabrique :
...
L’exemple suivant est un convertisseur qui remplace la sérialisation par défaut d’un type de données existant. Le convertisseur utilise le format mm/jj/aaaa `DateTimeOffset` pour les propriétés.

[!code-csharp[](~/samples/snippets/core/system-text-json/csharp/DateTimeOffsetConverter.cs)]

## <a name="sample-factory-pattern-converter"></a>Exemple de convertisseur de modèle de fabrique

Le code suivant illustre un convertisseur personnalisé qui fonctionne avec `Dictionary<Enum,TValue>`. Le code suit le modèle de fabrique, car le premier paramètre de `Enum` type générique est et le second est ouvert. La `CanConvert` méthode retourne `true` uniquement pour un `Dictionary` avec deux paramètres génériques, le premier étant un `Enum` type. Le convertisseur interne obtient un convertisseur existant pour gérer le type fourni au moment de l' `TValue`exécution pour.
...
* Substituez la `Read` méthode pour désérialiser le JSON entrant et le convertir en type `T`. Utilisez le <xref:[!OP.NO-LOC(System.Text.Json)].Utf8JsonReader> passé à la méthode pour lire le JSON.
* Substituez la `Write` méthode pour sérialiser l’objet entrant de type `T`. Utilisez le <xref:[!OP.NO-LOC(System.Text.Json)].Utf8JsonWriter> passé à la méthode pour écrire le JSON.
* Substituez la `CanConvert` méthode uniquement si nécessaire. L’implémentation par défaut `true` retourne lorsque le type à convertir est de `T`type. Par conséquent, les convertisseurs qui prennent `T` en charge uniquement le type n’ont pas besoin de substituer cette méthode. Pour obtenir un exemple de convertisseur qui doit substituer cette méthode, consultez la section sur la [désérialisation polymorphe](#support-polymorphic-deserialization) plus loin dans cet article.
...
* Substituez la `CanConvert` méthode pour retourner la valeur true lorsque le type à convertir est un qui peut être géré par le convertisseur. Par exemple, si le convertisseur est destiné `List<T>` à lui, il `List<int>`peut `List<string>`uniquement gérer `List<DateTime>`, et.
...
* Créez la classe de convertisseur instanciée par la `CreateConverter` méthode.
...
Si vous devez lever une exception dans le code de gestion des erreurs, envisagez de lever un <xref:[!OP.NO-LOC(System.Text.Json)].JsonException> sans message. Ce type d’exception crée automatiquement un message qui comprend le chemin d’accès à la partie du JSON qui a provoqué l’erreur. Par exemple, l’instruction `throw new JsonException();` génère un message d’erreur comme dans l’exemple suivant :
...
Si vous fournissez un message (par exemple, `throw new JsonException("Error occurred")`, l’exception fournit toujours le chemin d’accès <xref:[!OP.NO-LOC(System.Text.Json)].JsonException.Path> dans la propriété.

## <a name="register-a-custom-converter"></a>Inscrire un convertisseur personnalisé

*Inscrire* un convertisseur personnalisé pour que les `Serialize` méthodes `Deserialize` et l’utilisent. Choisissez l’une des approches suivantes :

* Ajoutez une instance de la classe de convertisseur à <xref:[!OP.NO-LOC(System.Text.Json)].JsonSerializerOptions.Converters?displayProperty=nameWithType> la collection.
...
Voici un exemple qui rend la <xref:System.ComponentModel.DateTimeOffsetConverter> valeur par défaut pour les propriétés de <xref:System.DateTimeOffset>type :
...
Le code suivant utilise la même approche pour désérialiser à l’aide `DateTimeOffset` du convertisseur personnalisé :
...
Le code à sérialiser `WeatherForecastWithConverterAttribute` ne requiert pas l’utilisation `JsonSerializeOptions.Converters`de :

[!code-csharp[](~/samples/snippets/core/system-text-json/csharp/RegisterConverterWithAttributeOnProperty.cs?name=SnippetSerialize)]

Le code à désérialiser ne nécessite pas non plus l' `Converters`utilisation de :
...
Voici le code qui crée un struct et lui applique `[JsonConverter]` l’attribut :
...
L' `[JsonConvert]` attribut sur le struct inscrit le convertisseur personnalisé comme valeur par défaut pour les propriétés de `Temperature`type. Le convertisseur est automatiquement utilisé sur la `TemperatureCelsius` propriété du type suivant lors de la sérialisation ou de la désérialisation de ce dernier :
...
Si plusieurs convertisseurs personnalisés pour un type sont inscrits dans la `Converters` collection, le premier convertisseur qui retourne la valeur `CanConvert` true pour est utilisé.
...
Lors de la désérialisation vers une propriété de `object`type, `JsonElement` un objet est créé. Cela est dû au fait que le désérialiseur ne sait pas quel type CLR créer, et qu’il ne tente pas de deviner. Par exemple, si une propriété JSON a la valeur « true », le désérialiseur ne déduit pas que la `Boolean`valeur est un et, si un élément a « 01/01/2019 », le désérialiseur ne déduit pas `DateTime`qu’il s’agit d’un.

L’inférence de type peut être inexacte. Si le désérialiseur analyse un nombre JSON qui n’a pas de virgule décimale `long`comme un, cela peut entraîner des problèmes hors limites si la valeur a été sérialisée à l’origine `ulong` en `BigInteger`tant que ou. L’analyse d’un nombre qui a une virgule décimale `double` peut perdre la précision si le nombre a été initialement sérialisé en `decimal`tant que.

Pour les scénarios qui requièrent l’inférence de type, le code suivant montre `object` un convertisseur personnalisé pour les propriétés. Le code convertit :
...
Voici un exemple de type avec `object` des propriétés :

[!code-csharp[](~/samples/snippets/core/system-text-json/csharp/WeatherForecast.cs?name=SnippetWFWithObjectProperties)]

L’exemple suivant de JSON à désérialiser contient des valeurs qui seront désérialisées en `DateTime`tant `long`que, `string`et :
...
Le convertisseur peut sérialiser et désérialiser la `TemperatureRanges` propriété de la classe suivante qui utilise les éléments `Enum`suivants :
...
Le [dossier tests unitaires](https://github.com/dotnet/runtime/blob/81bf79fd9aa75305e55abe2f7e9ef3f60624a3a1/src/libraries/System.Text.Json/tests/Serialization/) de `System.Text.Json.Serialization` l’espace de noms contient des exemples de convertisseurs personnalisés qui gèrent des dictionnaires non-clés.
...
Supposons, par exemple, que vous `Person` avez une classe de base `Employee` abstraite, avec des classes dérivées et `Customer` . La désérialisation polymorphe signifie qu’au moment du design vous pouvez `Person` spécifier en tant que cible de désérialisation `Customer` , `Employee` et les objets et dans le JSON sont correctement désérialisés au moment de l’exécution. Pendant la désérialisation, vous devez trouver des indices qui identifient le type requis dans le JSON. Les types d’indices disponibles varient en fonction de chaque scénario. Par exemple, une propriété de discriminateur peut être disponible ou vous devrez peut-être vous appuyer sur la présence ou l’absence d’une propriété particulière. La version actuelle de `System.Text.Json` ne fournit pas d’attributs pour spécifier comment gérer les scénarios de désérialisation polymorphe, donc les convertisseurs personnalisés sont requis.
...
L’article [migrer de Newtonsoft.Json System.Text.Json vers](system-text-json-migrate-from-newtonsoft-how-to.md) contient des exemples supplémentaires de convertisseurs personnalisés.
