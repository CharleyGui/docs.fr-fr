...
.NET définit un format de fichier binaire, *l’assemblage*, qui est utilisé pour décrire et contenir pleinement les programmes .NET. Les assemblys sont utilisés pour les programmes eux-mêmes comme n’importe quelle bibliothèque dépendante. Un programme .NET peut être exécuté comme tout autre assembly, sans que d’autres artefacts soient nécessaires, en dehors de l’implémentation de .NET appropriée. Les dépendances autochtones, y compris les API du système d’exploitation, sont une préoccupation distincte et ne sont pas contenues dans le format d’assemblage .NET, bien qu’elles soient parfois décrites avec ce format (par exemple, WinRT).
...
Le format d’assembly .NET est également très descriptif de la structure d’une bibliothèque ou d’un programme donné. Il décrit les composantes internes d’un assemblage, en particulier les références et les types d’assemblage définis et leur structure interne. Les API ou les outils peuvent lire et traiter ces informations pour l’affichage ou pour prendre des décisions de programmation.
...
## <a name="process-the-assemblies"></a>Traiter les assemblées
