...
Pour décharger un assembly dans le .NET Framework, vous devez décharger tous les domaines d’application qui le contiennent. Pour décharger un domaine d’application, utilisez la méthode <xref:System.AppDomain.Unload%2A?displayProperty=nameWithType>. Pour plus d’informations, consultez [Comment : décharger un domaine d’application](../../framework/app-domains/how-to-unload-an-application-domain.md).
