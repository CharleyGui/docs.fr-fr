...
- **Limite inférieure cohérente**  
     ...
     La limite inférieure d'une collection est l'index de son premier élément. Toutes les collections indexées dans l'espace de noms <xref:System.Collections> ont une limite inférieure de zéro, ce qui signifie qu'elles sont indexées à 0. <xref:System.Array>a une limite inférieure de zéro par défaut, mais une limite **Array** inférieure différente <xref:System.Array.CreateInstance%2A?displayProperty=nameWithType>peut être définie lors de la création d’une instance de la classe Array en utilisant .  
  
- **Synchronisation pour l’accès à partir de plusieurs threads** (classes<xref:System.Collections> seulement).  
...
|[Thread-Safe Collections](../../../docs/standard/collections/thread-safe/index.md)|Décrit les types de collections tels que <xref:System.Collections.Concurrent.BlockingCollection%601?displayProperty=nameWithType> et <xref:System.Collections.Concurrent.ConcurrentBag%601?displayProperty=nameWithType> qui prennent en charge l'accès simultané sécurisé et efficace de plusieurs threads.|  
