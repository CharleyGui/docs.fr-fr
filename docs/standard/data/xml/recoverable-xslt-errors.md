...
|La collection de nœuds du second argument de la fonction de document est vide et la référence URI est relative.|errata 14|Récupérer|  
 ...
 <sup>*</sup> Ce comportement est différent de celui de la <xref:System.Xml.Xsl.XslTransform> classe. Pour plus d’informations, consultez [Implémentation de comportements discrétionnaires dans la classe XslTransform](implementation-of-discretionary-behaviors-in-the-xsltransform-class.md).  
