...
Commencez par insérer une ligne comme d’habitude. Utilisez la fonction SQL `zeroblob()` pour allouer de l’espace dans la base de données afin de conserver l’objet volumineux. La fonction `last_insert_rowid()` offre un moyen pratique d’obtenir son ROWID.
