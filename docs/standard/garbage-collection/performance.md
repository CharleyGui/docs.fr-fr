...
- <span data-ttu-id="d13f6-121">Ce qui a déclenché le garbage collection</span><span class="sxs-lookup"><span data-stu-id="d13f6-121">What triggered the garbage collection.</span></span>
...
<span data-ttu-id="d13f6-148">Il existe deux cas légitimes pour la levée d'une <xref:System.OutOfMemoryException> managée :</span><span class="sxs-lookup"><span data-stu-id="d13f6-148">There are two legitimate cases for a managed <xref:System.OutOfMemoryException> to be thrown:</span></span>
...
<span data-ttu-id="d13f6-250">La procédure suivante décrit comment définir un point d'arrêt pour mesurer le tas managé.</span><span class="sxs-lookup"><span data-stu-id="d13f6-250">The following procedure describes how to set a breakpoint so that you can measure the managed heap.</span></span>
...
- [<span data-ttu-id="d13f6-262">Déterminez la quantité de mémoire virtuelle pouvant être réservée.</span><span class="sxs-lookup"><span data-stu-id="d13f6-262">Determine how much virtual memory can be reserved.</span></span>](#GetVM)
...
- <span data-ttu-id="d13f6-276">Examinez les deux compteurs de performances de mémoire suivants :</span><span class="sxs-lookup"><span data-stu-id="d13f6-276">Examine the following two memory performance counters:</span></span>
...
1. <span data-ttu-id="d13f6-288">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande d’exception d’impression (**pe**) suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-288">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the print exception (**pe**) command:</span></span>
    ...
    <span data-ttu-id="d13f6-289">**!pe**</span><span class="sxs-lookup"><span data-stu-id="d13f6-289">**!pe**</span></span>
...
2. <span data-ttu-id="d13f6-311">Sous l’onglet **Performances**, regardez la valeur validée.</span><span class="sxs-lookup"><span data-stu-id="d13f6-311">On the **Performance** tab, look at the committed value.</span></span> <span data-ttu-id="d13f6-312">(Dans Windows 7, regardez **Validation (Ko)** sous **Groupe système**.)</span><span class="sxs-lookup"><span data-stu-id="d13f6-312">(In Windows 7, look at **Commit (KB)** in the **System group**.)</span></span>
...
- <span data-ttu-id="d13f6-325">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-325">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
  ...
  <span data-ttu-id="d13f6-328">Les adresses indiquées par "segment" sont les adresses de début des segments.</span><span class="sxs-lookup"><span data-stu-id="d13f6-328">The addresses indicated by "segment" are the starting addresses of the segments.</span></span>
...
- <span data-ttu-id="d13f6-330">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-330">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
  ...
  <span data-ttu-id="d13f6-339">Voici un exemple de résultat.</span><span class="sxs-lookup"><span data-stu-id="d13f6-339">An example of the results is as follows.</span></span>
...
1. <span data-ttu-id="d13f6-359">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-359">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
    ...
    <span data-ttu-id="d13f6-360">**!finalizequeue**</span><span class="sxs-lookup"><span data-stu-id="d13f6-360">**!finalizequeue**</span></span>
...
- <span data-ttu-id="d13f6-370">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-370">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
  ...
  <span data-ttu-id="d13f6-371">**!dumpheap -type Free -stat**</span><span class="sxs-lookup"><span data-stu-id="d13f6-371">**!dumpheap -type Free -stat**</span></span>
...
- <span data-ttu-id="d13f6-381">La commande suivante fait un dump pour l'espace libre dans la plage de la génération 0 :</span><span class="sxs-lookup"><span data-stu-id="d13f6-381">The following command dumps the free space within the generation 0 range:</span></span>
...
- <span data-ttu-id="d13f6-388">Dans le débogueur WinDbg ou Visual Studio avec l’extension SOS chargée, tapez la commande suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-388">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command:</span></span>
  ...
  <span data-ttu-id="d13f6-389">**!gchandles**</span><span class="sxs-lookup"><span data-stu-id="d13f6-389">**!gchandles**</span></span>
...
- <span data-ttu-id="d13f6-406">Vous pouvez également déterminer la durée d’un garbage collection à l’aide des [événements ETW de garbage collection](../../../docs/framework/performance/garbage-collection-etw-events.md) en analysant les informations fournies.</span><span class="sxs-lookup"><span data-stu-id="d13f6-406">Alternatively, you can determine the length of a garbage collection by using [garbage collection ETW events](../../../docs/framework/performance/garbage-collection-etw-events.md), and analyze the information to determine the duration of garbage collection.</span></span>
  ...
  <span data-ttu-id="d13f6-407">Par exemple, les données suivantes indiquent une séquence d’événements qui se sont produits pendant un garbage collection non simultané.</span><span class="sxs-lookup"><span data-stu-id="d13f6-407">For example, the following data shows an event sequence that occurred during a non-concurrent garbage collection.</span></span>
  ...
  <span data-ttu-id="d13f6-413">L'événement `GCStart_V1` à la ligne 42504816 indique qu'il s'agit d'un garbage collection d'arrière-plan, car le dernier champ est `1`.</span><span class="sxs-lookup"><span data-stu-id="d13f6-413">The `GCStart_V1` event at 42504816 indicates that this is a background garbage collection, because the last field is `1`.</span></span> <span data-ttu-id="d13f6-414">Cela devient le garbage collection numéro</span><span class="sxs-lookup"><span data-stu-id="d13f6-414">This becomes garbage collection No.</span></span> <span data-ttu-id="d13f6-415">102019.</span><span class="sxs-lookup"><span data-stu-id="d13f6-415">102019.</span></span>

  <span data-ttu-id="d13f6-416">L'événement `GCStart` se produit, car un garbage collection éphémère doit être effectué avant de démarrer un garbage collection d'arrière-plan.</span><span class="sxs-lookup"><span data-stu-id="d13f6-416">The `GCStart` event occurs because there is a need for an ephemeral garbage collection before you start a background garbage collection.</span></span> <span data-ttu-id="d13f6-417">Cela devient le garbage collection numéro</span><span class="sxs-lookup"><span data-stu-id="d13f6-417">This becomes garbage collection No.</span></span> <span data-ttu-id="d13f6-418">102020.</span><span class="sxs-lookup"><span data-stu-id="d13f6-418">102020.</span></span>

  <span data-ttu-id="d13f6-419">À la ligne 42514170, le garbage collection numéro</span><span class="sxs-lookup"><span data-stu-id="d13f6-419">At 42514170, garbage collection No.</span></span> <span data-ttu-id="d13f6-420">102020 prend fin.</span><span class="sxs-lookup"><span data-stu-id="d13f6-420">102020 finishes.</span></span> <span data-ttu-id="d13f6-421">Les threads managés sont redémarrés à cet endroit.</span><span class="sxs-lookup"><span data-stu-id="d13f6-421">The managed threads are restarted at this point.</span></span> <span data-ttu-id="d13f6-422">L'opération se termine au thread 4372, qui a déclenché ce garbage collection d'arrière-plan.</span><span class="sxs-lookup"><span data-stu-id="d13f6-422">This is completed on thread 4372, which triggered this background garbage collection.</span></span>

  <span data-ttu-id="d13f6-423">Au thread 4744, une suspension se produit.</span><span class="sxs-lookup"><span data-stu-id="d13f6-423">On thread 4744, a suspension occurs.</span></span> <span data-ttu-id="d13f6-424">C'est la seule fois où le garbage collection d'arrière-plan doit suspendre les threads managés.</span><span class="sxs-lookup"><span data-stu-id="d13f6-424">This is the only time at which the background garbage collection has to suspend managed threads.</span></span> <span data-ttu-id="d13f6-425">Cette durée est d'environ 99 ms ((63784407-63685394)/1000).</span><span class="sxs-lookup"><span data-stu-id="d13f6-425">This duration is approximately 99ms ((63784407-63685394)/1000).</span></span>

  <span data-ttu-id="d13f6-426">L'événement `GCEnd` pour le garbage collection d'arrière-plan se trouve à la ligne 89931423.</span><span class="sxs-lookup"><span data-stu-id="d13f6-426">The `GCEnd` event for the background garbage collection is at 89931423.</span></span> <span data-ttu-id="d13f6-427">Cela signifie que le garbage collection d'arrière-plan a duré environ 47 secondes ((89931423-42504816)/1000).</span><span class="sxs-lookup"><span data-stu-id="d13f6-427">This means that the background garbage collection lasted for about 47seconds ((89931423-42504816)/1000).</span></span>

  <span data-ttu-id="d13f6-428">Pendant que les threads managés sont exécutés, vous pouvez voir tous les garbage collection éphémères en cours.</span><span class="sxs-lookup"><span data-stu-id="d13f6-428">While the managed threads are running, you can see any number of ephemeral garbage collections occurring.</span></span>

<a name="Triggered"></a>

### <a name="to-determine-what-triggered-a-garbage-collection"></a><span data-ttu-id="d13f6-429">Pour déterminer ce qui a déclenché un garbage collection</span><span class="sxs-lookup"><span data-stu-id="d13f6-429">To determine what triggered a garbage collection</span></span>

- <span data-ttu-id="d13f6-430">Dans le débogueur WinDbg ou Visual Studio avec l’extension de débogueur SOS chargée, tapez la commande suivante pour afficher tous les threads avec leurs piles d’appels :</span><span class="sxs-lookup"><span data-stu-id="d13f6-430">In the WinDbg or Visual Studio debugger with the SOS debugger extension loaded, type the following command to show all the threads with their call stacks:</span></span>

  <span data-ttu-id="d13f6-431">**~\*kb**</span><span class="sxs-lookup"><span data-stu-id="d13f6-431">**~\*kb**</span></span>

  <span data-ttu-id="d13f6-432">Cette commande affiche une sortie similaire à la suivante :</span><span class="sxs-lookup"><span data-stu-id="d13f6-432">This command displays output similar to the following.</span></span>
  ...
  <span data-ttu-id="d13f6-433">Si le garbage collection a été provoqué par une notification de mémoire insuffisante du système d'exploitation, la pile des appels sera similaire, à ceci près que le thread sera le thread finaliseur.</span><span class="sxs-lookup"><span data-stu-id="d13f6-433">If the garbage collection was caused by a low memory notification from the operating system, the call stack is similar, except that the thread is the finalizer thread.</span></span> <span data-ttu-id="d13f6-434">Le thread finaliseur reçoit une notification asynchrone de mémoire insuffisante et déclenche le garbage collection.</span><span class="sxs-lookup"><span data-stu-id="d13f6-434">The finalizer thread gets an asynchronous low memory notification and induces the garbage collection.</span></span>

  <span data-ttu-id="d13f6-435">Si le garbage collection a été provoqué par l'allocation de mémoire, la pile se présente comme ceci :</span><span class="sxs-lookup"><span data-stu-id="d13f6-435">If the garbage collection was caused by memory allocation, the stack appears as follows:</span></span>
  ...
  <span data-ttu-id="d13f6-436">Un programme d'assistance juste-à-temps (`JIT_New*`) appelle `GCHeap::GarbageCollectGeneration`.</span><span class="sxs-lookup"><span data-stu-id="d13f6-436">A just-in-time helper (`JIT_New*`) eventually calls `GCHeap::GarbageCollectGeneration`.</span></span> <span data-ttu-id="d13f6-437">Si vous déterminez que les garbage collection de génération 2 sont dus à des allocations, vous devez déterminer quels objets sont collectés par les garbage collection de génération 2 et comment les éviter.</span><span class="sxs-lookup"><span data-stu-id="d13f6-437">If you determine that generation 2 garbage collections are caused by allocations, you must determine which objects are collected by a generation 2 garbage collection and how to avoid them.</span></span> <span data-ttu-id="d13f6-438">Autrement dit, vous devez déterminer la différence entre le début et la fin d'un garbage collection de génération 2, ainsi que les objets qui ont provoqué la collection de génération 2.</span><span class="sxs-lookup"><span data-stu-id="d13f6-438">That is, you want to determine the difference between the start and the end of a generation 2 garbage collection, and the objects that caused the generation 2 collection.</span></span>

  <span data-ttu-id="d13f6-439">Par exemple, tapez la commande suivante dans le débogueur pour afficher le début d’une collection de génération 2 :</span><span class="sxs-lookup"><span data-stu-id="d13f6-439">For example, type the following command in the debugger to show the beginning of a generation 2 collection:</span></span>

  <span data-ttu-id="d13f6-440">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="d13f6-440">**!dumpheap –stat**</span></span>

  <span data-ttu-id="d13f6-441">Exemple de sortie (abrégé pour montrer les objets qui utilisent le plus d'espace) :</span><span class="sxs-lookup"><span data-stu-id="d13f6-441">Example output (abridged to show the objects that use the most space):</span></span>
  ...
  <span data-ttu-id="d13f6-442">Répétez la commande à la fin de la génération 2 :</span><span class="sxs-lookup"><span data-stu-id="d13f6-442">Repeat the command at the end of generation 2:</span></span>

  <span data-ttu-id="d13f6-443">**!dumpheap –stat**</span><span class="sxs-lookup"><span data-stu-id="d13f6-443">**!dumpheap –stat**</span></span>

  <span data-ttu-id="d13f6-444">Exemple de sortie (abrégé pour montrer les objets qui utilisent le plus d'espace) :</span><span class="sxs-lookup"><span data-stu-id="d13f6-444">Example output (abridged to show the objects that use the most space):</span></span>
  ...
  <span data-ttu-id="d13f6-445">Les objets `double[]` ont disparu de la fin de la sortie, ce qui signifie qu'ils ont été collectés.</span><span class="sxs-lookup"><span data-stu-id="d13f6-445">The `double[]` objects disappeared from the end of the output, which means that they were collected.</span></span> <span data-ttu-id="d13f6-446">Ces objets représentent environ 70 Mo.</span><span class="sxs-lookup"><span data-stu-id="d13f6-446">These objects account for approximately 70 MB.</span></span> <span data-ttu-id="d13f6-447">Les objets restants n'ont pas beaucoup changé.</span><span class="sxs-lookup"><span data-stu-id="d13f6-447">The remaining objects did not change much.</span></span> <span data-ttu-id="d13f6-448">Ces objets `double[]` étaient donc à l'origine du déclenchement du garbage collection de génération 2.</span><span class="sxs-lookup"><span data-stu-id="d13f6-448">Therefore, these `double[]` objects were the reason why this generation 2 garbage collection occurred.</span></span> <span data-ttu-id="d13f6-449">L'étape suivante consiste à déterminer pourquoi les objets `double[]` y sont situés et pourquoi ils sont morts.</span><span class="sxs-lookup"><span data-stu-id="d13f6-449">Your next step is to determine why the `double[]` objects are there and why they died.</span></span> <span data-ttu-id="d13f6-450">Vous pouvez demander à un développeur d'où proviennent ces objets ou vous pouvez utiliser la commande **gcroot**.</span><span class="sxs-lookup"><span data-stu-id="d13f6-450">You can ask the code developer where these objects came from, or you can use the **gcroot** command.</span></span>

<a name="HighCPU"></a>

### <a name="to-determine-whether-high-cpu-usage-is-caused-by-garbage-collection"></a><span data-ttu-id="d13f6-451">Pour déterminer si l’utilisation élevée du processeur est causée par le garbage collection</span><span class="sxs-lookup"><span data-stu-id="d13f6-451">To determine whether high CPU usage is caused by garbage collection</span></span>

- <span data-ttu-id="d13f6-452">Mettez en corrélation la valeur du compteur de performances de mémoire `% Time in GC` avec le temps d'exécution.</span><span class="sxs-lookup"><span data-stu-id="d13f6-452">Correlate the `% Time in GC` memory performance counter value with the process time.</span></span>

  <span data-ttu-id="d13f6-453">Si la valeur `% Time in GC` connaît un pic en même temps que le temps d'exécution, le garbage collection provoque une utilisation élevée du processeur.</span><span class="sxs-lookup"><span data-stu-id="d13f6-453">If the `% Time in GC` value spikes at the same time as process time, garbage collection is causing a high CPU usage.</span></span> <span data-ttu-id="d13f6-454">Dans le cas contraire, profilez l'application pour trouver où se produit l'utilisation élevée.</span><span class="sxs-lookup"><span data-stu-id="d13f6-454">Otherwise, profile the application to find where the high usage is occurring.</span></span>

## <a name="see-also"></a><span data-ttu-id="d13f6-455">Voir aussi</span><span class="sxs-lookup"><span data-stu-id="d13f6-455">See also</span></span>

- [<span data-ttu-id="d13f6-456">Nettoyage de la mémoire</span><span class="sxs-lookup"><span data-stu-id="d13f6-456">Garbage Collection</span></span>](../../../docs/standard/garbage-collection/index.md)
