...
| --mode de déploiement         | Si vous souhaitez déployer votre pilote sur les nœuds Worker (<code>cluster</code>) ou localement en tant que client externe (<code>client</code>).</br>Par défaut : <code>client</code>
| --CONF                | Propriété de configuration Spark arbitraire au format <code>key=value</code>.</br>_par exemple, Spark. fils. appMasterEnv. DOTNET_WORKER_DIR = .\worker\Microsoft.Spark.Worker_
...
| arguments d’application | Arguments passés à la méthode main de votre classe principale, le cas échéant.</br>_par exemple, hdfs://&lt;chemin d’accès à votre application&gt;/&lt;votre application&gt;. zip &lt;le nom de votre application&gt; &lt;des arguments d’application&gt;_
...
**Réponse :** Vérifiez la version de `Microsoft.Spark.Worker` que vous utilisez. Il existe deux versions : **.NET Framework 4.6.1** et **.net Core 2.1. x**. Dans ce cas, `Microsoft.Spark.Worker.net461.win-x64-<version>` (que vous pouvez [Télécharger](https://github.com/dotnet/spark/releases)) doit être utilisé, car `System.Runtime.Remoting.Contexts.Context` est destiné uniquement à .NET Framework.
...
**Réponse :** Pour lancer l’application Spark sur les fils, les variables d’environnement doivent être spécifiées en tant que `spark.yarn.appMasterEnv.[EnvironmentVariableName]`. Pour obtenir un exemple d’utilisation de `spark-submit`, voir ci-dessous :
