...
Il existe deux façons de déployer votre .NET pour Apache Spark travail sur Databricks : `spark-submit` et Set jar. 
...
2. Téléchargez le `microsoft-spark-<spark-version>-<spark-dotnet-version>.jar`approprié.
...
1. Vérifiez que vous avez publié votre application et que votre code d’application n’utilise pas `SparkSession.Stop()`.
