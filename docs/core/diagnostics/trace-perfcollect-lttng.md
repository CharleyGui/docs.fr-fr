...
---
# <a name="tracing-net-applications-with-perfcollect"></a>Suivi des applications .NET avec PerfCollect
...
## <a name="preparing-your-machine"></a>Préparation de votre ordinateur

Procédez comme suit pour préparer votre ordinateur à la collecte d’un suivi des performances avec `perfcollect` .

> [!NOTE]
> Si vous êtes dans un environnement de conteneur, votre conteneur doit avoir une `SYS_ADMIN` capacité. Pour plus d’informations sur le suivi des applications à l’intérieur d’un conteneur à l’aide de PerfCollect, consultez la documentation [collecter des diagnostics dans des conteneurs](./diagnostics-in-containers.md) .
...
3. Installer la configuration requise pour le suivi : il s’agit des bibliothèques de suivi réelles.
    ...
    2. `LTTng`: Utilisé pour capturer les données d’événement émises lors de l’exécution par CoreCLR. Ces données sont ensuite utilisées pour analyser le comportement de différents composants du runtime, tels que le GC, le JIT et le pool de threads.
...
## <a name="viewing-a-trace"></a>Affichage d’une trace

Il existe un certain nombre d’options pour l’affichage de la trace qui a été collectée. Les traces sont mieux affichées à l’aide de [PerfView](http://aka.ms/perfview>) sur Windows, mais elles peuvent être consultées directement sur Linux à l’aide de `PerfCollect` ou de `TraceCompass` .

### <a name="using-perfcollect-to-view-the-trace-file"></a>Utilisation de PerfCollect pour afficher le fichier de trace
...
### <a name="using-perfview-to-open-the-trace-file"></a>Utilisation de PerfView pour ouvrir le fichier de trace
...
2. Téléchargez PerfView à partir de <http://aka.ms/perfview> .
...
### <a name="using-tracecompass-to-open-the-trace-file"></a>Utilisation de TraceCompass pour ouvrir le fichier de trace
...
## <a name="resolving-framework-symbols"></a>Résolution des symboles de Framework
...
## <a name="getting-symbols-for-the-native-runtime"></a>Obtention de symboles pour le runtime natif

La plupart du temps, vous êtes intéressé par votre propre code, qui est `perfcollect` résolu par défaut. Parfois, il est très utile de voir ce qui se passe à l’intérieur des dll .NET (ce que fait la dernière section), mais parfois ce qui se passe dans les dll du runtime natif (généralement libcoreclr.so), est intéressant.  `perfcollect` résoudra les symboles pour ceux-ci lorsqu’il convertit ses données, mais uniquement si les symboles de ces DLL natives sont présents (et sont en regard de la bibliothèque à laquelle ils sont destinés).
...
2. Téléchargez les symboles. Si votre version installée du Runtime .NET Core est 2.1.0, la commande pour effectuer cette opération est la suivante :
...
3. Copier les symboles à l’emplacement approprié
...
## <a name="collecting-in-a-docker-container"></a>Collecter dans un conteneur d’ancrage

Pour plus d’informations sur l’utilisation `perfcollect` de dans les environnements de conteneur, consultez la documentation sur la [collecte des diagnostics dans les conteneurs](./diagnostics-in-containers.md) .
