...
ms.locfileid: "85325020"
---
# <a name="publish-net-core-apps-with-the-net-core-cli"></a>Publier des applications .NET Core avec le CLI .NET Core
...
Sauf indication contraire, le répertoire de sortie de la [`dotnet publish`](../tools/dotnet-publish.md) commande est `./bin/<BUILD-CONFIGURATION>/<TFM>/publish/` . Le mode **BUILD-CONFIGURATION** par défaut est **Debug**, sauf si vous le modifiez avec le paramètre `-c`. Par exemple, `dotnet publish -c Release -f netcoreapp2.1` publie dans `myfolder/bin/Release/netcoreapp2.1/publish/`.
...
Lorsque vous exécutez l’application ( [`dotnet run`](../tools/dotnet-run.md) ), la sortie suivante s’affiche :
...
Pour .NET Core 2,2 et versions antérieures, vous devez utiliser les commutateurs suivants avec la `dotnet publish` commande pour publier un FDE :
