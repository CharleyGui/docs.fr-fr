...
---
# <a name="most-code-access-security-apis-are-obsolete"></a>La plupart des API de sécurité d’accès du code sont obsolètes

La plupart des types liés à la sécurité d’accès du code (CAS) dans .NET sont désormais obsolètes en tant qu’avertissement. Cela comprend les attributs CAS, tels que <xref:System.Security.Permissions.SecurityPermissionAttribute> , les objets d’autorisation cas, tels que les <xref:System.Net.SocketPermission> <xref:System.Security.Policy.EvidenceBase> types dérivés de, et d’autres API de prise en charge.

## <a name="change-description"></a>Description de la modification

Dans .NET Framework 2. x-4. x, les API et les attributs CAS peuvent influencer le déroulement de l’exécution du code, notamment s’assurer que le parcours de la pile CAS-demande réussit ou échoue.
...
Dans .NET Core 2. x-3. x, le runtime ne respecte pas les attributs ou les API CAS. Le runtime ignore les attributs sur l’entrée de la méthode, et la plupart des API de programmation n’ont aucun effet.
...
En outre, les appels de programmation à des API vastes ( `Assert` ) fonctionnent toujours, tandis que les appels de programmation à des API restrictives ( `Deny` , `PermitOnly` ) lèvent toujours une exception au moment de l’exécution. ( <xref:System.Security.Permissions.PrincipalPermission> est une exception à cette règle. Consultez la section [action recommandée](#cas-action) ci-dessous.)
...
Dans .NET 5,0 et versions ultérieures, la plupart des API associées à l’autorité de certification sont obsolètes et produisent un avertissement au moment de la compilation `SYSLIB0003` .
...
Il s’agit d’une modification uniquement au moment de la compilation. Il n’existe aucun changement au moment de l’exécution par rapport aux versions précédentes de .NET Core. Les méthodes qui n’effectuent aucune opération dans .NET Core 2. x-3. x continuent à ne pas effectuer d’opération au moment de l’exécution dans .NET 5,0 et versions ultérieures. Les méthodes qui lèvent <xref:System.PlatformNotSupportedException> dans .net Core 2. x-3. x continuent à lever une <xref:System.PlatformNotSupportedException> au moment de l’exécution dans .net 5,0 et versions ultérieures.

## <a name="reason-for-change"></a>Motif de modification

La [sécurité d’accès du code (cas)](../../../../framework/misc/code-access-security.md) est une technologie héritée non prise en charge. L’infrastructure permettant d’activer les autorités de certification existe uniquement dans .NET Framework 2. x-4. x, mais elle est dépréciée et ne reçoit pas de correctifs de sécurité ou de maintenance.

En raison de l’obsolescence de l’autorité de certification, l' [infrastructure de prise en charge n’a pas été transmise à .net Core](../../../porting/net-framework-tech-unavailable.md) ou .net 5.0 +. Toutefois, les API ont été avancées afin que les applications puissent effectuer des compilations croisées par rapport à .NET Framework et .NET Core. Cela a conduit à des scénarios « d’échec d’ouverture », où certaines API liées à l’autorité de certification existent et peuvent être appelées mais n’exécutent aucune action au moment de l’exécution. Cela peut entraîner des problèmes de sécurité pour les composants qui s’attendent à ce que le runtime respecte les attributs liés à l’autorité de certification ou les appels d’API par programme. Pour mieux communiquer que le runtime ne respecte pas ces attributs ou ces API, nous avons obsolète la majorité d’entre eux dans .NET 5,0.

## <a name="version-introduced"></a>Version introduite

5.0

## <a name=""></a><a id="cas-action">Action recommandée</a>

- Si vous déclarez une autorisation de sécurité, supprimez l’attribut ou l’appel qui déclare l’autorisation.
...
- Si vous refusez ou restreignez (via `PermitOnly` ) toute autorisation, contactez votre conseiller de sécurité. Étant donné que les attributs CAS ne sont pas honorés par le Runtime .NET 5.0 +, votre application peut avoir une brèche de sécurité si elle s’appuie incorrectement sur l’infrastructure CAS pour restreindre l’accès à ces méthodes.
...
- Si vous exigez une autorisation (sauf <xref:System.Security.Permissions.PrincipalPermission> ), supprimez la demande. Toutes les demandes échoueront au moment de l’exécution.
...
- Si vous en avez <xref:System.Security.Permissions.PrincipalPermission> la demande, consultez les conseils pour [PrincipalPermissionAttribute est obsolète en tant qu’erreur](principalpermissionattribute-obsolete.md). Ces instructions s’appliquent à la fois à <xref:System.Security.Permissions.PrincipalPermission> et <xref:System.Security.Permissions.PrincipalPermissionAttribute> .

- Si vous devez absolument désactiver ces avertissements (ce qui n’est pas recommandé), vous pouvez supprimer l' `SYSLIB0003` avertissement dans le code.
  ...
  > [!NOTE]
  > La suppression `SYSLIB0003` désactive uniquement les avertissements obsoletion liés à l’autorité de certification. Il ne désactive pas les autres avertissements et ne modifie pas le comportement du Runtime .NET 5.0 +.
- Sécurité
