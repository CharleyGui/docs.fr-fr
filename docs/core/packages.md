...
- [System. Runtime](https://www.nuget.org/packages/System.Runtime) : Package .net core le plus fondamental, notamment <xref:System.Object>, <xref:System.String> <xref:System.Array> <xref:System.Action>,, et <xref:System.Collections.Generic.IList%601>.
...
- [System. Reflection](https://www.nuget.org/packages/System.Reflection) : ensemble de types pour le chargement, l’inspection et l’activation de types, <xref:System.Reflection.Assembly>y <xref:System.Reflection.TypeInfo> compris <xref:System.Reflection.MethodInfo>, et.
...
- [Netstandard. Library](https://www.nuget.org/packages/NETStandard.Library) : décrit les bibliothèques qui font partie de .NET standard. S’applique à toutes les implémentations .NET qui prennent en charge .NET Standard (par exemple, .NET Framework, .NET Core et mono). Établit le `netstandard` Framework.

Les principaux métapackages .NET Core sont les suivants :

- [Microsoft.NETCore.App](https://www.nuget.org/packages/Microsoft.NETCore.App) : décrit les bibliothèques qui font partie de la distribution .NET Core. Établit le `.NETCoreApp` Framework. Dépend de la bibliothèque `NETStandard.Library` (plus petite).
...
L' `netstandard` infrastructure référence implicitement le [`NETStandard.Library`](https://www.nuget.org/packages/NETStandard.Library) repackage. Par exemple, le fichier projet MSBuild suivant indique que le projet cible `netstandard1.6`, qui fait référence au produit de la [ `NETStandard.Library` version 1,6](https://www.nuget.org/packages/NETStandard.Library/1.6.0) .
...
En ajoutant l' `<NetStandardImplicitPackageVersion>` élément à votre fichier projet, ce qui spécifie implicitement une version de l’infrastructure, vous pouvez spécifier une version de Framework inférieure à la version de la collection de packages. L' `<NetStandardImplicitPackageVersion>` élément n’est applicable que lorsque vous ciblez .net Core et .NET standard. Par exemple, le fichier projet suivant est valide.
