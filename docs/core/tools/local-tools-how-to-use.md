...
Le fichier *. config/dotnet-Tools. JSON* ne contient pas encore d’outils :
...
Lorsque vous utilisez une commande CLI qui fait référence à un outil local, le kit de développement logiciel (SDK) recherche un fichier manifeste dans le répertoire actif et les répertoires parents. S’il trouve un fichier manifeste, mais que le fichier n’inclut pas l’outil référencé, il continue la recherche dans les répertoires parents. La recherche se termine lorsqu’elle trouve l’outil référencé ou lorsqu’elle trouve un fichier manifeste avec `isRoot` défini sur `true`.
...
Appelez l’outil en exécutant la commande `dotnet tool run` à partir du dossier du *référentiel* :
