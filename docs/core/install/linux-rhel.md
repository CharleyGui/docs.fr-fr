...
> [!TIP]
> .NET 5,0 n’est pas encore disponible dans les dépôts FluxApp, mais .NET Core 3,1 est. Pour installer .NET Core 3,1, utilisez la `dnf install` commande avec le package approprié, tel que `aspnetcore-runtime-3.1` ou `dotnet-sdk-3.1` . Les instructions suivantes concernent .NET 5,0.
