...
Vous pouvez appliquer la configuration uniquement à cette règle, à toutes les règles ou à toutes les règles de cette catégorie (sécurité). Pour plus d’informations, consultez Options de configuration d’une [règle de qualité du code](../code-quality-rule-options.md).
