...
Les types des paramètres de méthode et des valeurs de retour sont spécifiés dans la signature de méthode. La signature suivante présente une méthode qui nécessite l’argument d’entrée [int](language-reference/builtin-types/integral-numeric-types.md) et retourne une chaîne :  
