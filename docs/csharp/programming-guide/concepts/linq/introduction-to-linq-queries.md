...
```  
 ...
 Pour plus d’informations sur la création de types spécifiques de sources de données, consultez la documentation relative aux différents fournisseurs LINQ. Toutefois, la règle de base est très simple : une source de données LINQ est tout objet qui prend en charge l’interface <xref:System.Collections.Generic.IEnumerable%601> générique, ou une interface qui hérite de celui-ci.  
  
> [!NOTE]
> Les types tels que les <xref:System.Collections.ArrayList> qui prennent en charge l’interface de <xref:System.Collections.IEnumerable> non générique peuvent également être utilisés comme source de données LINQ. Pour plus d’informations, consultez [Comment interroger une ArrayList avec LINQC#()](./how-to-query-an-arraylist-with-linq.md).  
