...
description: Les méthodes d’extension en C# vous permettent d’ajouter des méthodes à des types existants sans créer de nouveau type dérivé, en recompilant ou en modifiant le type d’origine.
ms.date: 03/19/2020
helpviewer_keywords:
...
L'exemple suivant indique comment appeler la méthode `OrderBy` d'opérateur de requête standard sur un tableau d'entiers. L'expression entre parenthèses est une expression lambda. De nombreux opérateurs de requête standard prennent des expressions lambda comme paramètres, mais ce n’est pas une exigence pour les méthodes d’extension. Pour plus d’informations, consultez [expressions lambda](../../language-reference/operators/lambda-expressions.md).
...
- [Règles de conversion pour les paramètres Instance et leur impact](/archive/blogs/sreekarc/conversion-rules-for-instance-parameters-and-their-impact)
- [Interopérabilité des méthodes d’extension entre les langages](/archive/blogs/sreekarc/extension-methods-interoperability-between-languages)
- [Méthodes d’extension et délégués curryfiés](/archive/blogs/sreekarc/extension-methods-and-curried-delegates)
