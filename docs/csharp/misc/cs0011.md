...
La classe de base ou l’interface 'classe' dans l’assembly 'assembly' référencé par le type 'type' n’a pas pu être résolue  
 ...
 Pour plus d’informations, consultez [Ajouter une référence, boîte de dialogue](/visualstudio/ide/how-to-add-or-remove-references-by-using-the-reference-manager) et [/Reference (C# options du compilateur)](../language-reference/compiler-options/reference-compiler-option.md).  
