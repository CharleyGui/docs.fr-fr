...
Bien entendu, le jeu ayant été coupé en deux, il faut réunir les deux moitiés. Dans le code, cela signifie que vous allez énumérer les séquences que vous avez acquises via <xref:System.Linq.Enumerable.Take%2A> et <xref:System.Linq.Enumerable.Skip%2A> à *`interleaving`* la fois, les éléments et la création d’une séquence : votre jeu de cartes à présent mélangé. Pour écrire une méthode LINQ qui fonctionne avec deux séquences, vous devez comprendre comment <xref:System.Collections.Generic.IEnumerable%601> fonctionne.
