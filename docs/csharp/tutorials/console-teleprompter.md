...
- Les bases de l’ICIC de base .NET
...
Vous allez créer une application qui lit un fichier texte, et fait écho au contenu de ce fichier texte à la console. La sortie sur la console se fait à un rythme permettant de la lire à haute voix. Vous pouvez accélérer ou ralentir le rythme en appuyant sur les touches «<» (moins de) ou «>» (plus grandes que).

Il existe un grand nombre de fonctionnalités dans ce didacticiel. Construisons-les un par un.

## <a name="prerequisites"></a>Conditions préalables requises

- Configurez votre machine pour exécuter .NET Core. Vous pouvez trouver les instructions d’installation sur la page [téléchargements .NET Core.](https://dotnet.microsoft.com/download) Vous pouvez exécuter cette application sur Windows, Linux, macOS ou dans un conteneur Docker.

- Installez votre éditeur de code préféré.
...
Le code d’application Hello World simple se trouve intégralement dans le fichier Program.cs. Ouvrez ce fichier avec votre éditeur de texte préféré. Nous sommes sur le point d’apporter nos premiers changements. En haut du fichier, remarquez l’instruction using :
...
Cette méthode utilise les types de deux nouveaux espaces de noms. Pour que cela compile, vous devrez ajouter les deux lignes suivantes en haut du fichier :
...
Ce que vous avez s’affiche beaucoup trop rapidement pour le lire à haute voix. Vous devez maintenant ajouter des délais à la sortie. Au début, vous construireez une partie du code de base qui permet un traitement asynchrone. Toutefois, ces premières étapes suivront quelques anti-modèles. Les anti-modèles sont signalés dans les commentaires lorsque vous ajoutez le code, et le code sera actualisé ultérieurement.

Il existe deux étapes dans cette section. Tout d’abord, vous mettre à jour la méthode itérateur pour retourner les mots simples au lieu de lignes entières. C’est fait avec ces modifications. Remplacez l’instruction `yield return line;` par le code suivant :
...
Exécutez l’échantillon, et vous serez en mesure de lire à haute voix à son rythme pré-configuré.

## <a name="async-tasks"></a>Tâches asynchrones

Dans cette dernière étape, vous ajouterez le code pour écrire la sortie asynchronement dans une tâche, tout en exécutant une autre tâche pour lire l’entrée de l’utilisateur s’ils veulent accélérer ou ralentir l’affichage du texte, ou arrêter l’affichage du texte tout à fait. Cela comporte quelques étapes et à la fin, vous aurez toutes les mises à jour dont vous avez besoin. La première étape consiste à créer <xref:System.Threading.Tasks.Task> une méthode de retour asynchrone qui représente le code que vous avez créé jusqu’à présent pour lire et afficher le fichier.

Ajoutez cette méthode `Program` à votre classe (elle est `Main` prise à partir du corps de votre méthode):
...
Ici, dans `Main`, le code attend de façon synchrone. Vous devez utiliser l’opérateur `await` au lieu d’attendre de manière synchrone lorsque cela est possible. Mais, selon la méthode `Main` d’une application `await` de console, vous ne pouvez pas utiliser l’opérateur. Cela entraînerait la fermeture de l’application avant la fin de toutes les tâches.
...
Ensuite, vous devez écrire la deuxième méthode asynchrone pour lire à partir de la console et regarder pour le «<» (moins de), «>» (plus grand que) et «X» ou «x» clés. Voici la méthode que vous ajoutez pour cette tâche :
...
Cela crée une expression lambda pour représenter un <xref:System.Action> délégué qui lit une clé de la console et modifie une variable locale représentant le retard lorsque l’utilisateur presse le «<» (moins de) ou «>» (plus grand que) les touches. La méthode déléguée se termine lorsque l’utilisateur appuie sur les touches 'X' ou 'x', qui permettent à l’utilisateur d’arrêter l’affichage du texte à tout moment. Cette méthode utilise <xref:System.Console.ReadKey> pour bloquer et attendre que l’utilisateur appuie sur une touche.
