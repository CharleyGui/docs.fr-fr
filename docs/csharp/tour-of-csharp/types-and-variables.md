...
Le plan suivant fournit une vue d' C#ensemble du système de type de.
...
Les types `class`, `struct`, `interface`et `delegate` prennent tous en charge les génériques, dans lesquels ils peuvent être paramétrés avec d’autres types.
...
Les types valeur Nullable n’ont pas non plus besoin d’être déclarés avant de pouvoir être utilisés. Pour chaque type valeur n’acceptant pas les valeurs NULL `T`, il existe un type valeur Nullable correspondant `T?`, qui peut contenir une valeur supplémentaire, `null`. Par exemple, `int?` est un type qui peut contenir un entier 32 bits ou la valeur `null`.
