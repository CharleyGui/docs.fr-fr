...
Le nom 'identifier' n’existe pas dans le contexte actuel
 ...
 Cette erreur se produit fréquemment si vous `try` déclarez une variable en boucle ou en bloc, `if` puis tentez d’y accéder à partir d’un bloc de code ou d’un bloc de code distinct, comme indiqué dans l’exemple suivant :
