...
Un type ou un espace de noms utilisé dans le programme est introuvable. Vous avez peut-être oublié de référencer ([-Reference](../compiler-options/reference-compiler-option.md)) l’assembly qui contient le type, ou vous n’avez peut-être pas ajouté la [directive using](../keywords/using-directive.md)requise.  Il se peut aussi qu’il y ait un problème avec l’assembly que vous tentez de référencer.  
