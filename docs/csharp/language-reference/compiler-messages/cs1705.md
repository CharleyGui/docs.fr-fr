...
- Un client qui fait référence uniquement à la version 1.0 des DLL identiques, mais accède à une classe de la version 2.0.  
 ...
 Le code suivant crée la première des DLL identiques. Pour plus d’informations sur la façon de générer un fichier de clé, consultez [-keyfile (C# options du compilateur)](../compiler-options/keyfile-compiler-option.md).  
