...
|---------------|-----------------|  
|type|Nom de type CLR du gestionnaire de jetons à supprimer. Pour plus d’informations sur la spécification de l' `type` attribut, consultez [références de types personnalisés](https://docs.microsoft.com/previous-versions/windows-identity-foundation/gg638728(v=msdn.10)#custom-type-references). Obligatoire.|  
