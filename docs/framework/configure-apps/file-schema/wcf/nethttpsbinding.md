...
Représente une liaison qu’un service Windows Communication Foundation (WCF) peut utiliser pour configurer et exposer des points de terminaison qui sont en mesure de communiquer via HTTPs. Lorsqu'elle est utilisée avec un contrat duplex, WebSocket est utilisé, sinon HTTPS est utilisé.  
...
|`messageEncoding`|Définit l'encodeur utilisé pour encoder le message SOAP. Les valeurs valides sont les suivantes :<br /><br /> Financière Utilisez un encodeur de message texte.<br />MTOM Utilisez un encodeur de transmission de message (MTOM) message transmission Organization 1,0.<br /><br /> La valeur par défaut est Text. Cet attribut est de type <xref:System.ServiceModel.WSMessageEncoding>.|  
...
|Élément|Description|  
|-------------|-----------------|  
|[\<> de sécurité](security-of-nethttpbinding.md)|Définit les paramètres de sécurité de la liaison. Cet élément est de type <xref:System.ServiceModel.Configuration.BasicHttpsSecurityElement>. |  
...
## <a name="remarks"></a>Notes  
 ...
 La sécurité est désactivée par défaut, mais peut être ajoutée en affectant à l’attribut mode de la [ \<sécurité >](security-of-basichttpbinding.md) élément enfant `None`une valeur autre que. Par défaut, il utilise un encodage de message "Texte" et un encodage texte UTF-8.  
