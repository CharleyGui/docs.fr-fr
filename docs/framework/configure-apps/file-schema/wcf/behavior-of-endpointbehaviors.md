...
|[\<enableWebScript>](enablewebscript.md)|Active le comportement de point de terminaison qui permet de consommer le service à partir de pages Web ASP.NET AJAX. Le comportement doit être utilisé uniquement avec la \<liaison WebHttpBinding > standard ou l' \<élément de liaison webMessageEncoding >.|  
...
|[\<webHttp>](webhttp.md)|Spécifie le WebHttpBehavior d'un point de terminaison via la configuration. Ce comportement, lorsqu’il est utilisé conjointement avec \<WebHttpBinding > la liaison standard, active le modèle de programmation Web pour un service WCF.|  
