...
|---------------|-----------------|  
|auditLogLocation|Spécifie l'emplacement du journal d'audit. Les valeurs valides sont les suivantes :<br /><br /> Valeurs Les événements de sécurité sont écrits dans le journal des applications sur Windows XP et dans le journal des événements sur Windows Server 2003 et Windows Vista.<br />Oeuvre Les événements d’audit sont écrits dans le journal des événements de l’application.<br />Caution Les événements d’audit sont écrits dans le journal des événements de sécurité.<br /><br /> La valeur par défaut est Default. Pour plus d'informations, consultez <xref:System.ServiceModel.AuditLogLocation>.|  
...
|serviceAuthorizationAuditLevel|Spécifie les types d'événements d'autorisation enregistrés dans le journal d'audit. Les valeurs valides sont les suivantes :<br /><br /> None Aucun audit d’événements d’autorisation de service n’est effectué.<br />Fructueux Seuls les événements d’autorisation de service réussis sont audités.<br />Toute Seuls les événements d’autorisation de service d’échec sont audités.<br />- SuccessOrFailure: Les événements d’autorisation de service de succès et d’échec sont audités.<br /><br /> La valeur par défaut est Aucun. Pour plus d'informations, consultez <xref:System.ServiceModel.AuditLevel>.|  
|messageAuthenticationAuditLevel|Spécifie le type d'événements d'audit d'authentification de message enregistrés. Les valeurs valides sont les suivantes :<br /><br /> None Aucun événement d’audit n’est généré.<br />Fructueux Seule la sécurité réussie (validation complète incluant la validation de signature de message, le chiffrement et la validation de jeton) est journalisée.<br />Toute Seuls les événements d’échec sont journalisés.<br />- SuccessOrFailure: Les événements de réussite et d’échec sont journalisés.<br /><br /> La valeur par défaut est Aucun. Pour plus d'informations, consultez <xref:System.ServiceModel.AuditLevel>.|  
...
## <a name="remarks"></a>Notes  
 ...
 Les événements de l'audit d'autorisation du service permettent de connaître la décision d'autorisation prise par un gestionnaire d'autorisation du service. Ils fournissent des informations sur la réussite ou l’échec de l’autorisation, ainsi que sur l’identité du client, le point de terminaison auquel le message a été envoyé, l’action associée au message, l’identificateur du contexte d’autorisation généré à partir du message entrant et type du gestionnaire d’autorisations qui a pris la décision d’accès.  
...
- [Guide pratique : Auditer les événements de sécurité](../../../wcf/feature-details/how-to-audit-wcf-security-events.md)
- [Comportement de l’audit de service](../../../wcf/samples/service-auditing-behavior.md)
