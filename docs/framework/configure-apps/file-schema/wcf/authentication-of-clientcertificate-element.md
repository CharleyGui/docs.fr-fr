...
|mapClientCertificateToWindowsAccount|Booléen. Spécifie si le client peut être mappé à une identité Windows à l'aide du certificat. Active Directory doit être activé pour cela.|  
...
|Valeur|Description|  
|-----------|-----------------|  
|Énumération|L’une des valeurs suivantes : NOCHECK, en ligne, hors connexion. Pour plus d’informations, consultez [utilisation des certificats](../../../wcf/feature-details/working-with-certificates.md).|  
...
## <a name="remarks"></a>Notes  
 ...
 Vous pouvez également utiliser `Custom`. Lorsque vous utilisez `Custom`, vous devez également affecter l'assembly et le type utilisés pour valider le certificat à l'attribut `customCertificateValidatorType`. Pour créer votre propre validateur personnalisé, vous devez hériter de la classe <xref:System.IdentityModel.Selectors.X509CertificateValidator> abstraite. Pour plus d’informations, consultez [Guide pratique pour Créez un service qui utilise un validateur](../../../wcf/extending/how-to-create-a-service-that-employs-a-custom-certificate-validator.md)de certificat personnalisé.  
