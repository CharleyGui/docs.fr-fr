...
|Élément|Description|  
|-------------|-----------------|  
|ChannelInitializationTimeout|Obtient ou définit une <xref:System.TimeSpan> qui détermine la durée maximale pendant laquelle un canal peut être dans l’état d’initialisation avant d’être déconnecté.|  
...
|[\<> connectionPoolSettings de \<namedPipeTransport >](connectionpoolsettings.md)|Spécifie des paramètres de pool de connexions supplémentaires pour une liaison de canal nommé.|  
