...
|deadLetterQueue|Indique le type de file d'attente de lettres mortes à utiliser.<br /><br /> Les valeurs valides sont les suivantes :<br /><br /> Propre File d’attente de lettres mortes personnalisée.<br />None Aucune file d’attente de lettres mortes ne doit être utilisée.<br />Requise Utilisez la file d’attente lettres mortes du système.<br /><br /> Cet attribut est de type DeadLetterQueue.|  
...
|serializationFormat|Indique le formateur utilisé pour sérialiser des objets envoyés comme partie d'un message MSMQ. Les valeurs valides sont les suivantes :<br /><br /> ActiveX Le formateur ActiveX est utilisé lors de la sérialisation d’objets COM.<br />Binaire2  Sérialise l’objet en un paquet binaire.<br />-   ByteArray:  Sérialise l'objet en un tableau d'octets.<br />Train  Sérialise l'objet en un flux de données.<br />Langage  Sérialise l’objet en un paquet XML. La valeur par défaut est XML.<br /><br /> Cet attribut est de type <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>.|  
