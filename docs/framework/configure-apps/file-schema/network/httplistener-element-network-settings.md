...
|---------------|-----------------|  
|unescapeRequestUrl|Valeur booléenne qui indique si une instance de <xref:System.Net.HttpListener> utilise l’URI brut sans séquence d’échappement au lieu de l’URI converti.|  
...
- URI converti  
 ...
 Le service `http.sys` combine la valeur de la propriété <xref:System.Uri.Host%2A?displayProperty=nameWithType> et la chaîne dans la ligne de demande pour créer un URI converti. En outre, `http.sys` et la classe <xref:System.Uri?displayProperty=nameWithType> effectue également les opérations suivantes :  
...
|------------------|-------------------|-----------------|  
|EnableNonUTF8|1|Si la valeur est zéro, `http.sys` n’accepte que les URL encodées en UTF-8.<br /><br /> Si la valeur est différente de zéro, `http.sys` accepte également les URL encodées en ANSI ou DBCS dans les demandes.|  
|FavorUTF8|1|Si la valeur est différente de zéro, `http.sys` essaie toujours de décoder une URL au format UTF-8 en premier ; Si cette conversion échoue et que EnableNonUTF8 est différent de zéro, http. sys essaie ensuite de le décoder en ANSI ou DBCS.<br /><br /> Si zéro (et EnableNonUTF8 est différent de zéro), `http.sys` essaie de le décoder comme ANSI ou DBCS ; Si ce n’est pas le cas, il tente une conversion UTF-8.|  
