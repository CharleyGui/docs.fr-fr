...
---
# <a name="iriparsing-element-uri-settings"></a>\<élément iriParsing > (paramètres d’URI)
...
2. Spécifie si les règles d’analyse IRI doivent être appliquées. Cela est spécifié dans le fichier machine.config ou app.config.  
 ...
 L’activation de l’analyse IRI (iriParsing enabled = `true`) effectue la normalisation et la vérification des caractères selon les dernières règles IRI de la norme RFC 3987. La valeur par défaut est `false` et effectue la normalisation et la vérification des caractères conformément aux RFC 2396 et RFC 3986 (pour les littéraux IPv6).  
