...
## <a name="remarks"></a>Notes  
 ...
 Pour utiliser votre propre fabrique d’hôte de service personnalisée au lieu de la fabrique par défaut, il vous suffit de fournir le nom de type dans la directive [@ServiceHost](servicehost.md) comme suit :  
...
```  
 ...
 Par exemple, pour activer un point de terminaison compatible AJAX pour `MyService`, spécifiez le <xref:System.ServiceModel.Activation.WebScriptServiceHostFactory> pour la valeur de l’attribut `Factory`, au lieu du <xref:System.ServiceModel.Activation.ServiceHostFactory>par défaut, dans la directive [@ServiceHost](servicehost.md) comme indiqué dans l’exemple suivant.  
