...
Lorsque vous exécutez l’exemple sur le .NET Framework 3.5 ou une version antérieure, il affiche la sortie suivante :
...
Toutefois, si vous ajoutez le fichier de configuration suivant au répertoire de l’exemple et puis exécutez l’exemple sur le .NET Framework 4 ou version ultérieure, la sortie est identique à celle produite par l’exemple lorsqu’il est exécuté sur .NET Framework 3.5.
