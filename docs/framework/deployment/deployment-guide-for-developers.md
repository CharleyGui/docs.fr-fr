...
Vous pouvez vérifier si un module linguistique spécifique est installé en recherchant la valeur DWORD \\ dans le dossier HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\FullLCID`Release` du Registre. (Notez que « NET Framework Setup » ne commence pas par un point.) *dans le dossier HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full\* spécifie un identificateur de paramètres régionaux ; consultez la liste des [langues prises en charge](#supported-languages) .
...
### <a name="chaining-the-language-packs-to-your-app-setup"></a>Chaînage des modules linguistiques à l'installation de votre application
