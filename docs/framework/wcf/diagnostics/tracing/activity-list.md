...
> [!NOTE]
> Vous pouvez également définir des activités par programme pour grouper des suivis utilisateur. Pour plus d’informations, consultez [émission de traces de code utilisateur](../../../../../docs/framework/wcf/diagnostics/tracing/emitting-user-code-traces.md).  
...
|-----------|-------------------|-------------------|-----------------|  
|A, M|Activité ambiante|N/A (non contrôlé par ServiceModel)|Activité dont l'ID est défini dans TLS avant les appels au code ServiceModel (côté client ou côté serveur).<br /><br /> Exemple : Activité où Open est appelé sur le client WCF ou serviceHost. Open est appelé.|  
...
|-------------------|-------------------|-----------------|  
|Installer la session sécurisée|SetupSecurity|Existe uniquement sur le côté client. Contient tous les échanges RST*/SCT pour l'authentification et la définition du contexte de sécurité. Si `propagateActivity` =\*la méthode est, cette activité est fusionnée avec les activités de processus d’action RST/SCT correspondantes du service correspondantes. `true`|  
|Fermer la session sécurisée|SetupSecurity|Existe sur le côté client. Contient l'échange de messages Cancel permettant de fermer la session sécurisée. Si `propagateActivity` ,cetteactivité`true`est fusionnée avec l’action de traitement «annuler» à partir du service. =|  
...
|Exécuter une \<opération com+ >|TransferToCOMPlus|1 instance d’activité pour chaque appel COM+ à partir du code WCF|  
