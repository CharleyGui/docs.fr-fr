...
1. Sélectionnez un mode de sécurité. <xref:System.ServiceModel.WSFederationHttpBinding> prend en charge `Message`, qui fournit la sécurité de bout en bout au niveau du message, même avec plusieurs sauts, et `TransportWithMessageCredential`, qui fournit les meilleures performances dans les cas où le client et le service peuvent établir une connexion directe sur HTTPS.
    ...
    > [!NOTE]
    > <xref:System.ServiceModel.WSFederationHttpBinding> prend également en charge `None` comme mode de sécurité. Ce mode, non sécurisé, est fourni à des fins de débogage uniquement. Si un point de terminaison de service est <xref:System.ServiceModel.WSFederationHttpBinding> déployé avec avec le mode de `None`sécurité défini sur, la liaison cliente résultante (générée par l' [outil ServiceModel Metadata Utility Tool (Svcutil. exe)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md)) est un <xref:System.ServiceModel.WSHttpBinding> avec le mode de `None`sécurité.
...
4. facultatif. Sur les services fédérés, affectez à la propriété <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.IssuerMetadataAddress%2A> l'URL de métadonnées d'un service de jeton de sécurité. Le point de terminaison de métadonnées permet aux clients du service de sélectionner une paire liaison/point de terminaison appropriée, si le service est configuré pour publier des métadonnées. Pour plus d’informations sur la publication de métadonnées, consultez [publication](publishing-metadata.md)de métadonnées.
...
2. Affectez à la propriété <xref:System.ServiceModel.WSFederationHttpSecurity.Mode%2A> la valeur <xref:System.ServiceModel.WSFederationHttpSecurityMode> ou <xref:System.ServiceModel.WSFederationHttpSecurityMode.Message> selon le cas. Si une suite d’algorithmes <xref:System.ServiceModel.Security.SecurityAlgorithmSuite.Basic256%2A> autre que est requise, <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.AlgorithmSuite%2A> affectez à la propriété une <xref:System.ServiceModel.Security.SecurityAlgorithmSuite>valeur extraite de.
...
4. Affectez <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.IssuedKeyType%2A> <xref:System.IdentityModel.Tokens.SecurityKeyType> à`SymmetricKey` la propriété la valeur ou.`AsymmetricKey` Si nécessaire.

5. Affectez la valeur appropriée à la propriété <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.IssuedTokenType%2A>. Si aucune valeur n’est définie, WCF prend par `http://docs.oasis-open.org/wss/oasis-wss-saml-token-profile-1.1#SAMLV1.1`défaut la valeur, qui indique les jetons SAML 1,1.
...
7. Requis sur le client si aucun émetteur local n'est spécifié ; non utilisé sur le service. Créez un <xref:System.ServiceModel.Channels.Binding> pour le `SecurityTokenService` et assignez l' <xref:System.ServiceModel.Channels.Binding> instance <xref:System.ServiceModel.FederatedMessageSecurityOverHttp.IssuerBinding%2A> à la propriété.
...
1. Créez un [ \<> WSFederationHttpBinding](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) en tant qu’enfant de [ \<](../../../../docs/framework/configure-apps/file-schema/wcf/bindings.md) l’élément Bindings > dans le fichier de configuration de l’application.

2. Créez une [ \<liaison >](../../../../docs/framework/misc/binding.md) élément en tant qu’enfant de [ \<WSFederationHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md) et affectez à l' `name` attribut une valeur appropriée.

3. Créez un `<security>` élément en tant qu’enfant de l' [ \<élément Binding >](../../../../docs/framework/misc/binding.md) .
...
13. Requis sur le client si aucun émetteur local n'est spécifié ; non utilisé sur le service. Créez un [ \<](../../../../docs/framework/misc/binding.md) élément de > de liaison dans la section liaisons qui peut être utilisée pour communiquer avec le service d’accès aux jetons de sécurité. Pour plus d’informations sur la création d’une [liaison, consultez Procédure: Spécifiez une liaison de service](../../../../docs/framework/wcf/how-to-specify-a-service-binding-in-configuration.md)dans la configuration.
...
16. Facultatif sur le client et le service. Ajoutez un élément `<claimTypeRequirements>` en tant qu'enfant de l'élément `<message>`. Spécifiez les revendications obligatoires et facultatives sur lesquelles repose le service en ajoutant [ \<ajouter](../../../../docs/framework/configure-apps/file-schema/wcf/add-of-claimtyperequirements.md) des éléments `<claimTypeRequirements>` > à l’élément et en spécifiant `claimType` le type de revendication avec l’attribut. Spécifiez si une revendication donnée est requise ou facultative en définissant l'attribut `isOptional`.
...
- [Guide pratique pour Désactiver les sessions sécurisées sur un WSFederationHttpBinding](how-to-disable-secure-sessions-on-a-wsfederationhttpbinding.md)
