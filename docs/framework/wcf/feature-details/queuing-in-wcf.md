...
- En fonction de la liaison mise en file d’attente, une configuration supplémentaire en dehors de WCF est requise. Par exemple, la classe <xref:System.ServiceModel.NetMsmqBinding> fournie avec WCF nécessite que vous configuriez les liaisons et que vous configuriez au minimum Message Queuing (MSMQ).  
...
- `UseActiveDirectory` : valeur booléenne permettant d'indiquer si Active Directory doit être utilisé pour la résolution d'adresse de file d'attente. Par défaut, cette propriété a la valeur Off. Pour plus d’informations, consultez [points de terminaison de service et adressage de file d’attente](../../../../docs/framework/wcf/feature-details/service-endpoints-and-queue-addressing.md).  
  
### <a name="msmqintegrationbinding"></a>MsmqIntegrationBinding  
 La `MsmqIntegrationBinding` est utilisée lorsque vous souhaitez qu’un point de terminaison WCF communique avec une application MSMQ existante écrite C++en C,, com ou les API System. Messaging.  
