...
Il est important, dans la première tâche, que l’élément de configuration de point de terminaison contienne un attribut `bindingConfiguration` qui référence une configuration de liaison nommée (dans cet exemple) `MessageSecurity`. La [**liaison\<** ](../../configure-apps/file-schema/wcf/bindings.md) élément de configuration référence ensuite ce nom pour activer des sessions fiables en affectant à l’attribut `enabled` de l’élément [ **\<ReliableSession >** ](https://docs.microsoft.com/previous-versions/ms731375(v=vs.90)) la valeur `true`. Vous pouvez requérir que les assurances de remise ordonnée soient disponibles dans une session fiable en affectant à l'attribut `ordered` la valeur `true`.

Pour obtenir la copie source de l’exemple sur lequel cette procédure de configuration est basée, consultez la [session WS Reliable](../../../../docs/framework/wcf/samples/ws-reliable-session.md).

Les éléments essentiels de la deuxième tâche sont obtenus en définissant l’attribut `mode` de l’élément de **> de sécurité\<** contenu dans l’élément **> de liaison\<** du client et du service à `Message`.

Les éléments essentiels de la troisième tâche sont obtenus en définissant l’attribut `clientCredentialType` de l’élément **\<message** contenu dans l’élément **\<Security >** du client et du service sur `Certificate`.
...
Cette procédure est décrite dans [Comment : échanger des messages au sein d’une session fiable](../../../../docs/framework/wcf/feature-details/how-to-exchange-messages-within-a-reliable-session.md).
...
1. Ajoutez un élément de liaison approprié aux [**liaisons\<** ](../../../../docs/framework/configure-apps/file-schema/wcf/bindings.md) élément du fichier de configuration. L’exemple suivant ajoute un élément [ **\<wsHttpBinding >** ](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md) .

1. Ajoutez un élément de **> de liaison\<** et affectez à son attribut `name` une valeur appropriée. L’exemple utilise le nom `MessageSecurity`.

1. Ajoutez un élément **\<security >** et affectez à l’attribut `mode` la valeur `Message`.

1. Dans l’élément **\<security >** , ajoutez un élément **\<message >** et affectez à l’attribut `clientCredentialType` la valeur `Certificate`.
