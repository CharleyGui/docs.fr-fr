...
### <a name="special-security-considerations-for-large-data"></a>Considérations spéciales en matière de sécurité pour les données volumineuses  
 Toutes les liaisons vous permettent de contraindre la taille des messages entrants afin d’empêcher des attaques par déni de service. , Par exemple, expose une propriété [System. ServiceModel. BasicHttpBinding. MaxReceivedMessageSize](xref:System.ServiceModel.HttpBindingBase.MaxReceivedMessageSize%2A) qui limite la taille du message entrant et, par conséquent, limite la quantité maximale de mémoire qui est accédée lors du traitement de l’opération <xref:System.ServiceModel.BasicHttpBinding> Message. Cette unité est définie en octets et s'élève par défaut à 65 536 octets.  
