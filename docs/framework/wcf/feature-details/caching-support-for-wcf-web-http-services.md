...
> Si le mode de compatibilité ASP.NET n'est pas activé et si l'attribut <xref:System.ServiceModel.Web.AspNetCacheProfileAttribute> est utilisé, une exception est levée.  
 ...
 Le nom du profil de cache spécifié par l'attribut <xref:System.ServiceModel.Web.AspNetCacheProfileAttribute> identifie un profil de cache qui est ajouté à votre fichier de configuration Web.config. Le profil de cache est défini avec dans un élément <`outputCacheSetting`>, comme indiqué dans l’exemple de configuration suivant.  
...
```  
 ...
 Vous devez ensuite activer la dépendance de cache SQL au sein d’un <`caching`élément > dans le <`system.web`élément > comme indiqué dans l’exemple de configuration suivant.  
...
```  
 ...
 Ensuite, vous devez configurer les paramètres du cache de sortie dans le <`caching`> élément comme indiqué dans l’exemple suivant.  
