...
```  
 ...
 Vous devez ensuite activer la dépendance de cache SQL dans`caching`un élément < > au`system.web`sein de l’élément < >, comme indiqué dans l’exemple de configuration suivant.  
...
```  
 ...
 Ensuite, vous devez configurer les paramètres du cache de sortie`caching`dans l’élément < > comme indiqué dans l’exemple suivant.  
