...
### <a name="configuring-the-definitionidentity"></a>Configurer le DefinitionIdentity  
 Lorsqu’un service de flux de travail est <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> créé à l’aide du concepteur de flux de travail, le est réglé à l’aide de la fenêtre **Propriétés.** Cliquez en dehors de l’activité racine du service dans le concepteur pour sélectionner le service de flux de travail, et choisissez **Properties Window** dans le menu **View.** Sélectionnez **WorkflowIdentity** de la liste d’abandon qui apparaît à côté de <xref:System.Activities.WorkflowIdentity> la propriété **DefinitionIdentity,** puis agrandir et spécifier les propriétés souhaitées. Dans l’exemple <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> suivant, le <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` est <xref:System.Activities.WorkflowIdentity.Version%2A> `1.0.0.0`configuré avec le et un de . <xref:System.Activities.WorkflowIdentity.Package%2A> est facultatif et dans cet exemple est `null`.  
 ...
 Lorsqu'un service de workflow est auto-hébergé, <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> est configuré lorsque le service de workflow est construit. Dans l’exemple <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> suivant, le est configuré avec les <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` mêmes <xref:System.Activities.WorkflowIdentity.Name%2A> `1.0.0.0`valeurs que l’exemple précédent, avec le et un de .  
...
```  
 ...
 A <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> n’est pas nécessaire, bien qu’une seule version du service de flux de travail puisse avoir un **null**<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A>.  
