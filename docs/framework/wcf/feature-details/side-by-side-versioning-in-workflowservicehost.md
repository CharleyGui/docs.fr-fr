...
### <a name="configuring-the-definitionidentity"></a>Configurer le DefinitionIdentity  
 Lorsqu’un service de workflow est créé à l’aide du concepteur <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> de flux de travail, le est défini à l’aide de la fenêtre **Propriétés** . Cliquez en dehors de l’activité racine du service dans le concepteur pour sélectionner le service de flux de travail, puis choisissez **fenêtre Propriétés** dans le menu **affichage** . Sélectionnez **WorkflowIdentity** dans la liste déroulante qui s’affiche en regard de la propriété **DefinitionIdentity** , puis développez et <xref:System.Activities.WorkflowIdentity> spécifiez les propriétés souhaitées. Dans l’exemple suivant, <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> le est configuré avec <xref:System.Activities.WorkflowIdentity.Name%2A> le `MortgageWorkflow` et <xref:System.Activities.WorkflowIdentity.Version%2A> un `1.0.0.0`de. <xref:System.Activities.WorkflowIdentity.Package%2A> est facultatif et dans cet exemple est `null`.  
 ...
 Lorsqu'un service de workflow est auto-hébergé, <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> est configuré lorsque le service de workflow est construit. Dans l’exemple suivant, le <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> est configuré avec les mêmes valeurs que l’exemple précédent, avec les <xref:System.Activities.WorkflowIdentity.Name%2A> `MortgageWorkflow` et un <xref:System.Activities.WorkflowIdentity.Name%2A> de `1.0.0.0`.  
...
```  
 ...
 Un <xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A> n’est pas obligatoire, bien qu’une seule version du service de workflow puisse avoir une **valeur null**<xref:System.ServiceModel.Activities.WorkflowService.DefinitionIdentity%2A>.  
