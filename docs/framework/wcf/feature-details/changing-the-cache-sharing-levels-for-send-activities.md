...
- **Niveau hôte**: Au niveau de partage de l’hôte, le cache n’est disponible que pour les instances de flux de travail hébergées dans l’hôte du service de flux de travail. Un cache peut également être partagé entre des hôtes du service de workflow dans un cache au niveau du processus.  
  
- **Niveau d’instance**: Dans le niveau de partage par exemple, le cache est disponible pour une instance particulière de flux de travail tout au long de sa vie, mais le cache n’est pas disponible pour d’autres instances de flux de travail.  
  
- **Pas de cache**: Le cache est désactivé par défaut si vous avez un flux de travail qui utilise des paramètres définis dans la configuration. Dans ce cas, il est également recommandé de garder le cache désactivé, car son activation peut être non sécurisée. Citons, par exemple, le cas où une identité différente (informations d'identification différentes ou utilisation de l'emprunt d'identité) serait requise pour chaque envoi.  
...
## <a name="customizing-cache-settings"></a>Personnalisation des paramètres de cache  
 Vous pouvez personnaliser les paramètres du cache de fabrication de canaux et du cache de canaux. Les paramètres de cache sont définis dans la classe <xref:System.ServiceModel.Activities.ChannelCacheSettings>. La <xref:System.ServiceModel.Activities.SendMessageChannelCache> classe définit les paramètres de cache par défaut pour le cache de l’usine de canal et le cache du canal dans son constructeur sans paramètres. Le tableau suivant répertorie les valeurs par défaut de ces paramètres de cache, par type de cache.  
