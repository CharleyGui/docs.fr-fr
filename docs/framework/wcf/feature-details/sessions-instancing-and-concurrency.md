...
> Le fonctionnement et le développement de code utilisant plusieurs threads en toute sécurité peuvent s'avérer difficile à écrire. Avant d'utiliser les valeurs <xref:System.ServiceModel.ConcurrencyMode.Multiple> ou <xref:System.ServiceModel.ConcurrencyMode.Reentrant> , assurez-vous que votre service est correctement conçu pour ces modes. Pour plus d'informations, consultez <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A>.  
 ...
 L'utilisation de l'accès concurrentiel est associée au mode d'instanciation. Dans <xref:System.ServiceModel.InstanceContextMode.PerCall> instanciation, la concurrence n’est pas pertinente, car chaque message est traité par un nouveau <xref:System.ServiceModel.InstanceContext> et, par conséquent, il n’y a jamais plus d’un thread actif dans le <xref:System.ServiceModel.InstanceContext>.  
...
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|  
|PerCall|-Comportement avec un canal de session : une session et <xref:System.ServiceModel.InstanceContext> pour chaque appel.<br />-Behavior avec canal sans session : une exception est levée.|-Comportement avec un canal de session : une session et <xref:System.ServiceModel.InstanceContext> pour chaque appel.<br />-Behavior avec canal sans session : <xref:System.ServiceModel.InstanceContext> pour chaque appel.|-Comportement avec un canal de session : une exception est levée.<br />-Behavior avec canal sans session : <xref:System.ServiceModel.InstanceContext> pour chaque appel.|  
|PerSession|-Comportement avec un canal de session : une session et <xref:System.ServiceModel.InstanceContext> pour chaque canal.<br />-Behavior avec canal sans session : une exception est levée.|-Comportement avec un canal de session : une session et <xref:System.ServiceModel.InstanceContext> pour chaque canal.<br />-Behavior avec canal sans session : <xref:System.ServiceModel.InstanceContext> pour chaque appel.|-Comportement avec un canal de session : une exception est levée.<br />-Behavior avec canal sans session : <xref:System.ServiceModel.InstanceContext> pour chaque appel.|  
|Single|-Comportement avec canal de session : une session et une <xref:System.ServiceModel.InstanceContext> pour tous les appels.<br />-Behavior avec canal sans session : une exception est levée.|-Comportement avec canal de session : session et <xref:System.ServiceModel.InstanceContext> pour le singleton créé ou spécifié par l’utilisateur.<br />-Behavior avec canal sans session : <xref:System.ServiceModel.InstanceContext> pour le singleton créé ou spécifié par l’utilisateur.|-Comportement avec un canal de session : une exception est levée.<br />-Behavior avec canal sans session : <xref:System.ServiceModel.InstanceContext> pour chaque Singleton créé ou pour le singleton spécifié par l’utilisateur.|  
