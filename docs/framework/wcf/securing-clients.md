...
- En la programmant dans votre code client (à l'aide de la méthode `SetCertificate`).  
 ...
 En ajoutant une [\<behaviors>](../configure-apps/file-schema/wcf/behaviors.md) section du fichier de configuration pour le client et en utilisant l' `clientCredentials` élément (illustré ci-dessous).  
  
#### <a name="setting-a-clientcredentials-value-in-code"></a>Définition d’une \<clientCredentials> valeur dans le code  

 Pour définir une [\<clientCredentials>](../configure-apps/file-schema/wcf/clientcredentials.md) valeur dans le code, vous devez accéder à la <xref:System.ServiceModel.ClientBase%601.ClientCredentials%2A> propriété de la <xref:System.ServiceModel.ClientBase%601> classe. La propriété retourne un objet <xref:System.ServiceModel.Description.ClientCredentials> qui autorise l'accès à différents types d'informations d'identification, comme le montre le tableau suivant.  
...
```  
 ...
 Pour définir les informations d’identification du client dans la configuration, ajoutez un [\<endpointBehaviors>](../configure-apps/file-schema/wcf/endpointbehaviors.md) élément au fichier de configuration. En outre, l’élément de comportement ajouté doit être lié au point de terminaison du service à l’aide `behaviorConfiguration` de l’attribut du [ \<endpoint> \<client> d'](../configure-apps/file-schema/wcf/endpoint-of-client.md) élément, comme indiqué dans l’exemple suivant. La valeur de l'attribut `behaviorConfiguration` doit correspondre à la valeur de l'attribut de comportement `name`.  
...
- [Procédure : spécifier des informations d’identification de client](how-to-specify-client-credential-values.md)
