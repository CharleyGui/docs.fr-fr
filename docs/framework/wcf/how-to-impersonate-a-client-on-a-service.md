...
2. Après avoir créé le client WCF, affectez à la propriété <xref:System.ServiceModel.Security.WindowsClientCredential.AllowedImpersonationLevel%2A> de la classe <xref:System.ServiceModel.Security.WindowsClientCredential> la valeur de l’une des valeurs d’énumération <xref:System.Security.Principal.TokenImpersonationLevel>.  
