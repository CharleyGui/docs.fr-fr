...
L'application cliente utilise la classe proxy générée pour communiquer avec le service. Cette procédure est décrite dans [le didacticiel : Utilisez un client](how-to-use-a-wcf-client.md).
...
Une fois le client créé, Visual Studio crée le fichier de configuration **app. config** dans le projet **GettingStartedClient** , qui doit ressembler à l’exemple suivant :
...
Dans la [ \<section System. ServiceModel >](../configure-apps/file-schema/wcf/system-servicemodel.md) , notez l’élément [ \<> de point de terminaison](../configure-apps/file-schema/wcf/endpoint-element.md) . L’élément de **point de terminaison&gt; définit le point de terminaison que le client utilise pour accéder au service comme suit : &lt;**
