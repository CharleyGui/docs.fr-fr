...
Cet exemple illustre comment établir une communication en file d'attente avec transaction à l'aide de MSMQ (Message Queuing) sur HTTP.  
 ...
 Le protocole MSMQ permet d'utiliser HTTP (notamment HTTPS) pour l'envoi des messages à la file d'attente. Dans cet exemple, nous démontrons en utilisant windows Communication Foundation (WCF) communication en file d’attente et comment envoyer des messages sur HTTP. Le protocole MSMQ utilise le protocole SRMP basé sur le protocole SOAP pour communiquer sur HTTP.  
...
4. Avant d’exécuter l’échantillon dans **les composants De Windows Add/Supprimer,** assurez-vous que MSMQ est installé avec le support HTTP. L'installation de la prise en charge HTTP installe automatiquement les services IIS et ajoute la prise en charge de ce protocole dans IIS pour MSMQ.  
...
6. Une fois que vous avez sélectionné MSMQ pour fonctionner en mode durci, la machine nécessite une re-boot sur Windows Server 2003.  
