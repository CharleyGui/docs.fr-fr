...
```  
 ...
 L' `<in-reply-to>` élément spécifie trois attributs requis`ref`( `type` , `href`et), tout en autorisant également la présence d’attributs d’extension et d’éléments d’extension supplémentaires.  
