...
```  
 ...
 Étant donné que nous ne souhaitons pas ignorer les configurations qui ont été fournies dans le fichier de configuration de l’application, la première chose que `ApplyConfiguration`notre substitution de () appelle l’implémentation de base. Une fois cette méthode terminée, nous pouvons ajouter impérativement le <xref:System.ServiceModel.Description.ServiceMetadataBehavior> à la description à l'aide du code impératif suivant.  
...
1. Ouvrez le fichier Web. config du service et Notez qu’il n’existe aucune configuration activant explicitement les métadonnées pour le service.  
  
2. Ouvrez le fichier. svc du service et observez que @ServiceHost sa directive contient un attribut Factory qui spécifie le nom d’un ServiceHostFactory personnalisé.  
