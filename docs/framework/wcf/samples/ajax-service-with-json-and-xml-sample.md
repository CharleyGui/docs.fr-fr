...
Le format de données par défaut pour <xref:System.ServiceModel.Description.WebHttpEndpoint> est XML, tandis que le format de données par défaut pour <xref:System.ServiceModel.Description.WebScriptEndpoint> est JSON. Pour plus d’informations, consultez [création de services WCF Ajax sans ASP.net](../../../../docs/framework/wcf/feature-details/creating-wcf-ajax-services-without-aspnet.md).
...
Le format de réponse pour l’opération est spécifié en tant que XML, ce qui correspond au paramètre par défaut pour le comportement de [\<Webhttp](../../../../docs/framework/configure-apps/file-schema/wcf/webhttp.md) . Il est toutefois conseillé de spécifier explicitement le format de réponse.
