...
1. Créez un objet <xref:System.ServiceModel.Description.IEndpointBehavior> avec les étapes de validation souhaitées dans la méthode <xref:System.ServiceModel.Description.IEndpointBehavior.Validate%2A>. Le code suivant fournit un exemple. (Le `InternetClientValidatorBehavior` est tiré de l’exemple [Security validation](../samples/security-validation.md) .)
...
2. Installez l’assembly avec nom fort dans le Global Assembly Cache à l’aide de [Gacutil. exe (outil global assembly cache)](../../tools/gacutil-exe-gac-tool.md).

3. Utilisez les types d'espaces de noms <xref:System.Configuration?displayProperty=nameWithType> pour :

    1. Ajoutez l’extension à la section [\<behaviorExtensions >](../../configure-apps/file-schema/wcf/behaviorextensions.md) à l’aide d’un nom de type qualifié complet et verrouillez l’élément.

         [!code-csharp[LockdownValidation#5](../../../../samples/snippets/csharp/VS_Snippets_CFX/lockdownvalidation/cs/hostapplication.cs#5)]

    2. Ajoutez l’élément de comportement à la propriété `EndpointBehaviors` de la section [\<commonBehaviors](../../configure-apps/file-schema/wcf/commonbehaviors.md) et verrouillez l’élément. (Pour installer le validateur sur le service, le validateur doit être un <xref:System.ServiceModel.Description.IServiceBehavior> et ajouté à la propriété `ServiceBehaviors`.) L’exemple de code suivant montre la configuration appropriée après les étapes a. et b., à la seule exception qu'il n'y a pas de nom fort.
...
L'exemple de code suivant indique comment ajouter un comportement commun au fichier machine.config et enregistrer une copie sur le disque. Le `InternetClientValidatorBehavior` est tiré de l’exemple [Security validation](../samples/security-validation.md) .
...
- [Chiffrement des éléments du fichier de configuration à l’aide de DPAPI](https://docs.microsoft.com/previous-versions/msp-n-p/ff647398(v=pandp.10))
- [Chiffrement des éléments du fichier de configuration à l’aide de RSA](https://docs.microsoft.com/previous-versions/msp-n-p/ff650304(v=pandp.10))
