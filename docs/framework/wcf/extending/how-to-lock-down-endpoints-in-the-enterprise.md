...
1. Créez un objet <xref:System.ServiceModel.Description.IEndpointBehavior> avec les étapes de validation souhaitées dans la méthode <xref:System.ServiceModel.Description.IEndpointBehavior.Validate%2A>. Le code suivant fournit un exemple. (Est extrait de l’exemple [Security validation.)](../samples/security-validation.md) `InternetClientValidatorBehavior`
...
L'exemple de code suivant indique comment ajouter un comportement commun au fichier machine.config et enregistrer une copie sur le disque. Est extrait de l’exemple [Security validation.](../samples/security-validation.md) `InternetClientValidatorBehavior`
