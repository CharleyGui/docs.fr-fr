...
```  
 ...
 La façon dont vous écrivez votre nouvel élément de liaison dépend de ses fonctionnalités exactes. L’un des exemples, [transport : UDP](../samples/transport-udp.md), fournit une description détaillée de la façon d’implémenter un type d’élément de liaison.  
...
### <a name="example"></a> Exemple  
 ...
 Cet exemple implémente la liaison de profil dans `SampleProfileUdpBinding`, lequel dérive de <xref:System.ServiceModel.Channels.Binding>. Le `SampleProfileUdpBinding` contient jusqu’à quatre éléments de liaison : un créé par l’utilisateur `UdpTransportBindingElement` et trois fournis par le système : `TextMessageEncodingBindingElement` , `CompositeDuplexBindingElement` et `ReliableSessionBindingElement` .  
...
### <a name="one-shot-security"></a>Sécurité « one-shot »  
 ...
 Vous pouvez implémenter une sécurité « ponctuelle », où toutes les informations d’identification de sécurité nécessaires sont envoyées dans un message unique, en affectant `negotiateServiceCredential` à l’attribut de l’élément de configuration la valeur \<message> `false` .  
