...
**Sécurité**  
 Dans WCF, inclut la confidentialité (chiffrement des messages pour éviter l’espionnage), l’intégrité (la détection de falsification des messages), l’authentification (la validation des serveurs et clients) et autorisation (le contrôle d’accès à ressources). Ces fonctions sont fournies par deux mécanismes de sécurité existants en tirant parti, tels que TLS sur HTTP (également appelé HTTPS), ou en implémentant une ou plusieurs des divers WS -\* spécifications de sécurité.

**mode de sécurité de transport**  
