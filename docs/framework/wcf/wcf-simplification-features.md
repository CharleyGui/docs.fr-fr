...
WCF fournit le mode de compatibilité ASP.NET pour accorder aux développeurs l'accès total aux fonctionnalités dans le pipeline HTTP ASP.NET lors de l'écriture des services WCF. Pour utiliser ce mode, vous devez affecter la `aspNetCompatibilityEnabled` valeur true à l’attribut dans la [\<serviceHostingEnvironment>](../configure-apps/file-schema/wcf/servicehostingenvironment.md) section de web.config. En outre, tout service de cet appDomain doit avoir la `RequirementsMode` propriété <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsAttribute> définie sur ou sur <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsMode.Allowed> <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsMode.Required> . Par défaut <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsAttribute> , a la valeur <xref:System.ServiceModel.Activation.AspNetCompatibilityRequirementsMode.Allowed> et le modèle d’application de service WCF par défaut affecte la valeur `aspNetCompatibilityEnabled` à l’attribut `true` . Pour plus d’informations, consultez [What’s New in Windows Communication Foundation 4,5](whats-new.md) and [WCF services and ASP.net](./feature-details/wcf-services-and-aspnet.md).
