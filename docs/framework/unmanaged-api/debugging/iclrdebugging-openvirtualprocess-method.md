...
## <a name="parameters"></a>Paramètres  
 ...
 `pVersion`  
 [in, out] Version du CLR. En entrée, cette valeur peut être `null` . Il peut également pointer vers une structure [CLR_DEBUGGING_VERSION](clr-debugging-version-structure.md) , auquel cas le champ de la structure `wStructVersion` doit être initialisé à 0 (zéro).  
  
 Lors de la sortie, la structure retournée est `CLR_DEBUGGING_VERSION` renseignée avec les informations de version pour le CLR.  
