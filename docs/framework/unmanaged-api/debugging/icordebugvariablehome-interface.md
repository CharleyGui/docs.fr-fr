...
|[GetCode, méthode](icordebugvariablehome-getcode-method.md)|Obtient l’instance « ICorDebugCode » qui contient cet `ICorDebugVariableHome` objet.|  
...
|[GetRegister, méthode](icordebugvariablehome-getregister-method.md)|Obtient le Registre qui contient une variable avec un type d’emplacement `VLT_REGISTER` et le registre de base pour une variable avec un type d’emplacement `VLT_REGISTER_RELATIVE` .|  
