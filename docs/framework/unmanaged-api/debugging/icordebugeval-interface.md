...
|Méthode|Description|  
|------------|-----------------|  
|[Abort, méthode](icordebugeval-abort-method.md)|Abandonne le calcul actuellement effectué `ICorDebugEval` par cet objet.|  
...
|[GetResult, méthode](icordebugeval-getresult-method.md)|Obtient un pointeur d’interface vers `ICorDebugValue` un qui contient les résultats de l’évaluation.|  
