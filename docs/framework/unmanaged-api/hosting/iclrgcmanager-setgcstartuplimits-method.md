...
> [!IMPORTANT]
> En commençant par le cadre .NET 4.5, vous pouvez définir la `DWORD` taille du segment et la taille maximale de génération 0 à des valeurs supérieures à en utilisant la méthode [ICLRGCManager2::SetGCStartupLimitsEx.](../../../../docs/framework/unmanaged-api/hosting/iclrgcmanager2-setgcstartuplimitsex-method.md)  
...
## <a name="remarks"></a>Notes   
 Les valeurs `SetGCStartupLimits` qui se fixent ne peuvent être spécifiées qu’une seule fois. Les appels `SetGCStartupLimits` ultérieurs sont ignorés.  
