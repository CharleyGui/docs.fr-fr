...
## <a name="parameters"></a>Paramètres  
 `id`  
 dans Identificateur spécifique à l’hôte pour la connexion avec laquelle associer le tableau de `ppCLRTask`.  
  
 `dwCount`  
 dans Nombre de membres de `ppCLRTask`. Ce nombre doit être supérieur à zéro.  
  
 `ppCLRTask`  
 dans Tableau de pointeurs `ICLRTask` à associer à la connexion identifiée par `id`. Ce tableau doit contenir au moins un membre.  
...
|E_INVALIDARG|[BeginConnection](../../../../docs/framework/unmanaged-api/hosting/iclrdebugmanager-beginconnection-method.md) n’a pas été appelé à l’aide de la valeur de `id`, ou `dwCount` ou `id` est égal à zéro, ou l’un des éléments de `ppCLRTask` a la valeur null.|  
