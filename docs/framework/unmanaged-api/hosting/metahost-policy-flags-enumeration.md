...
|`METAHOST_POLICY_IGNORE_ERROR_MODE`|`GetRequestedRuntime` doit ignorer SEM_FAILCRITICALERRORS (qui est défini en appelant la fonction [SetErrorMode](https://go.microsoft.com/fwlink/p/?LinkId=255242) ) et afficher la boîte de dialogue d’erreur. Par défaut, SEM_FAILCRITICALERRORS supprime la boîte de dialogue d’erreur. Elle a peut-être été héritée d’un autre processus et l’erreur silencieuse peut ne pas être souhaitable dans votre scénario.|  
