...
|`COR_PRF_HIGH_REQUIRE_PROFILE_IMAGE`|Représente tous les indicateurs `COR_PRF_HIGH_MONITOR` qui requièrent des images à profil optimisé. Il correspond à l' `COR_PRF_REQUIRE_PROFILE_IMAGE` indicateur dans l’énumération [COR_PRF_MONITOR](cor-prf-monitor-enumeration.md) .|  
...
Les `COR_PRF_HIGH_MONITOR` indicateurs sont utilisés avec le `pdwEventsHigh` paramètre des méthodes [ICorProfilerInfo5 :: GetEventMask2](icorprofilerinfo5-geteventmask2-method.md) et [ICorProfilerInfo5 :: SetEventMask2](icorprofilerinfo5-seteventmask2-method.md) .  
...
`COR_PRF_HIGH_MONITOR_IMMUTABLE`est destiné à être un masque de réinitialisation qui représente tous les indicateurs qui peuvent être définis uniquement pendant l’initialisation. Toute tentative de modification d’un de ces indicateurs ailleurs entraîne l’échec d’un `HRESULT` .
