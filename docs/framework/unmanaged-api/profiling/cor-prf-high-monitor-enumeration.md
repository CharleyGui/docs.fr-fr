...
|`COR_PRF_HIGH_ADD_ASSEMBLY_REFERENCES`|Contrôle le rappel de [ICorProfilerCallback6::GetAssemblyReference](../../../../docs/framework/unmanaged-api/profiling/icorprofilercallback6-getassemblyreferences-method.md) quant à l'ajout de références d'assembly durant le parcours de fermeture des références d'assembly CLR.|  
...
|`COR_PRF_HIGH_REQUIRE_PROFILE_IMAGE`|Représente tous les indicateurs `COR_PRF_HIGH_MONITOR` qui requièrent des images à profil optimisé. Il correspond à l'indicateur `COR_PRF_REQUIRE_PROFILE_IMAGE` de l'énumération [COR_PRF_MONITOR](../../../../docs/framework/unmanaged-api/profiling/cor-prf-monitor-enumeration.md).|  
