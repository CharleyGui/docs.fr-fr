...
Fournit des méthodes utilisées par les profileurs de code pour communiquer avec le Common Language Runtime (CLR) pour contrôler la surveillance des événements et demander des informations. L’interface `ICorProfilerInfo3` est une extension de l’interface [ICorProfilerInfo2](icorprofilerinfo2-interface.md) . Il fournit de nouvelles méthodes prises en charge dans le .NET Framework 4 et versions ultérieures.  
