...
| `WBEM_FLAG_OWNER_UPDATE` | 0x10000 | Les fournisseurs de notifications push doivent spécifier cet indicateur lors de l’appel de `PutClassWmi` pour indiquer que cette classe a été modifiée. |
...
| `WBEM_E_ALREADY_EXISTS` | 0x80041019 | L’indicateur de `WBEM_FLAG_CREATE_ONLY` a été spécifié, mais la classe existe déjà. |
