...
|<xref:System.Windows.Automation.AutomationElementIdentifiers.BoundingRectangleProperty>|Voir les notes.|Le rectangle englobant d’une barre de titre doit englober tous les contrôles qu’il contient.|  
...
|<xref:System.Windows.Automation.AutomationElementIdentifiers.LabeledByProperty>|Voir les notes.|Le contrôle de barre de titre n’a généralement pas d’étiquette.|  
|<xref:System.Windows.Automation.AutomationElementIdentifiers.ControlTypeProperty>|TitleBar|Cette valeur est la même pour toutes les infrastructures d’interface utilisateur.|  
