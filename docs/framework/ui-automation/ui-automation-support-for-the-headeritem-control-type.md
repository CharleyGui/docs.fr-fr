...
|---------------------|-------------|-----------|  
|<xref:System.Windows.Automation.Provider.ITransformProvider>|Dépend|Implémentez ce modèle de contrôle si le contrôle header item peut être redimensionné.|  
|<xref:System.Windows.Automation.Provider.IInvokeProvider>|Dépend|Implémentez ce modèle de contrôle si un clic sur le contrôle header item permet de trier les données.|  
