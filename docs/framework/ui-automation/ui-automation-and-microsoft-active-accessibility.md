...
## <a name="servers-and-clients"></a>Serveurs et clients  
 ...
 Fournisseurs UI Automation peuvent fournir des informations aux clients d’accessibilité Active, et les serveurs Active Accessibility peuvent fournir des informations aux applications clientes UI Automation. Toutefois, étant donné que Active Accessibility n’expose pas autant d’informations en tant que [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)], les deux modèles ne sont pas entièrement compatibles.  
  
<a name="UI_Elements_compare"></a>   
## <a name="ui-elements"></a>Éléments de l'interface utilisateur  
 Présente Active Accessibility [!INCLUDE[TLA2#tla_ui](../../../includes/tla2sharptla-ui-md.md)] éléments soit comme un `IAccessible` interface ou comme un identificateur enfant. Il est difficile de comparer deux pointeurs `IAccessible` pour déterminer s’ils font référence au même élément.  
...
## <a name="tree-views-and-navigation"></a>Arborescences et navigation  
 ...
 Dans Active Accessibility, nombreux éléments automation sans importance pour les utilisateurs finaux sont exposées dans l’arborescence. Les applications clientes doivent examiner tous les éléments pour déterminer les éléments significatifs.  
 ...
 Navigation entre les éléments dans Active Accessibility, est soit spatiale (par exemple, le passage à l’élément qui est située à gauche de l’écran), logique (par exemple, déplacement vers l’élément de menu suivant ou l’élément suivant dans l’ordre de tabulation dans une boîte de dialogue) ou hiérarchique () par exemple, quand vous déplacez le premier enfant dans un conteneur, ou à partir de l’enfant à son parent). La navigation hiérarchique est compliquée par le fait que les éléments enfants ne sont pas toujours des objets qui implémentent `IAccessible`.  
...
## <a name="roles-and-control-types"></a>Rôles et types de contrôle  
 ...
 Le tableau suivant présente le mappage de rôles Active Accessibility pour [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] types de contrôle.  
...
## <a name="states-and-properties"></a>États et propriétés  
 Dans Active Accessibility, les éléments prennent en charge un ensemble commun de propriétés et certaines propriétés (telles que `accState`) doit décrire des aspects très différents, en fonction du rôle de l’élément. Les serveurs doivent implémenter toutes les méthodes de `IAccessible` qui retournent une propriété, y compris celles qui ne sont pas pertinentes pour l’élément.  
...
|`get_accHelpTopic`|Non pris en charge dans [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]||  
 ...
 Le tableau suivant montre quelles [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] propriétés correspondent aux constantes d’état Active Accessibility.  
  
|État de l’accessibilité active|Propriété [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)]|Déclenche un changement d’état ?|  
|-----------------------------------------------------------------------|------------------------------------------------------------------------------------|----------------------------|  
...
|STATE_SYSTEM_UNAVAILABLE|<xref:System.Windows.Automation.AutomationElement.IsEnabledProperty>|Y|  
 ...
 Les états suivants n'ont pas été implémentés par Active Accessibility la plupart des serveurs de contrôle ou n’ont aucun équivalent [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)].  
  
|État de l’accessibilité active|Notes|  
|-----------------------------------------------------------------------|-------------|  
...
## <a name="events"></a>Events  
 Le mécanisme d’événement dans [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)], contrairement à Active Accessibility, ne repose pas sur le routage d’événement Windows (qui étroitement lié aux handles de fenêtre) et ne nécessite pas de l’application cliente pour pouvoir installer des raccordements. Les abonnements aux événements peuvent être ajustés non seulement à des événements particuliers mais également à des parties spécifiques de l’arborescence. Les fournisseurs peuvent également ajuster leur déclenchement d’événements en assurant le suivi des événements qui sont écoutés.  
 ...
 Le tableau suivant montre la correspondance entre les WinEvents accessibilité Active et [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)] événements.  
...
|EVENT_OBJECT_REORDER|Pas régulièrement utilisé dans Active Accessibility. Aucun événement directement correspondant n’est défini dans [!INCLUDE[TLA2#tla_uiautomation](../../../includes/tla2sharptla-uiautomation-md.md)].|  
