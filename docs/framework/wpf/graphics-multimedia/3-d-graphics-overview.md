...
## <a name="3-d-in-a-2-d-container"></a>3D dans un conteneur 2D  
 ...
 Dans une application 2D <xref:System.Windows.Controls.Viewport3D> conventionnelle, utilisez comme vous le feriez un autre élément de conteneur comme Grid ou Canvas.  Bien que <xref:System.Windows.Controls.Viewport3D> vous puissiez utiliser avec d’autres objets de dessin 2D dans le même graphique de <xref:System.Windows.Controls.Viewport3D>scène, vous ne pouvez pas interpenetrate 2-D et 3-D objets dans un .  Ce sujet se concentrera sur la façon de <xref:System.Windows.Controls.Viewport3D>dessiner des graphiques 3D à l’intérieur de la .  
...
- <xref:System.Windows.Media.Media3D.EmissiveMaterial>vous permet de spécifier que la texture sera appliquée comme si le modèle émettait de la lumière égale à la couleur de la brosse. Cela ne fait pas du modèle un éclairage. Toutefois, il participera différemment à l’ombrage que s’il était texturé avec DiffuseMaterial ou SpecularMaterial.  
 ...
 Les exemples de code suivants montrent comment appliquer une couleur solide et un dessin comme brosses aux modèles 3D.  
