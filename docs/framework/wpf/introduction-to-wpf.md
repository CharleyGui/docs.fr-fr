...
L’exemple suivant utilise une <xref:System.Windows.Controls.DockPanel> pour disposer plusieurs contrôles <xref:System.Windows.Controls.TextBox> :
...
Le balisage suivant lie l' <xref:System.Windows.Controls.TextBox> à une instance d’un objet `Person` personnalisé :
...
Pour plus d’informations, consultez [Vue d’ensemble des formes et dessins de base dans WPF](../../desktop-wpf/data/data-binding-overview.md).
...
WPF comprend également des fonctionnalités de rendu 3D qui s’intègrent aux graphiques 2D, permettant ainsi la création d’interfaces utilisateur plus intéressantes. Par exemple, l’illustration suivante montre des images 2D restituées sur des formes 3D :
...
L’exemple suivant montre comment modifier l’apparence d’un <xref:System.Windows.Controls.Button> à l’aide d’un <xref:System.Windows.Controls.ControlTemplate>:
