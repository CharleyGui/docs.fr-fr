...
## <a name="who-should-implement-the-weak-event-pattern"></a>Qui doit implémenter le modèle d’événement faible ?  
 ...
 Certains scénarios se prêtent par nature à l’application du modèle d’événement faible. La liaison de données est un scénario de ce type. Dans la liaison de données, il est courant que l’objet source soit complètement indépendant de l’objet d’écouteur, qui est la cible d’une liaison. De nombreux aspects de [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] la liaison de données ont déjà le modèle d’événement faible qui est appliqué dans la manière dont les événements sont implémentés.  
...
1. Recherchez un gestionnaire d’événements faible existant.  
     ...
     Pour obtenir la liste des gestionnaires d’événements faibles inclus dans WPF, consultez la hiérarchie d’héritage dans la classe <xref:System.Windows.WeakEventManager>.  
...
1. Utilisez la classe de <xref:System.Windows.WeakEventManager%602> générique au lieu du raccordement d’événement normal.  
...
2. Remplacez le nom du `SomeEventWeakEventManager` par votre propre nom.  
