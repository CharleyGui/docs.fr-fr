...
### <a name="keyboard-and-mouse-classes"></a>Classes de souris et de clavier
 ...
 Les exemples d’API d’entrée sur la classe <xref:System.Windows.Input.Keyboard> sont la propriété <xref:System.Windows.Input.Keyboard.Modifiers%2A>, qui retourne le <xref:System.Windows.Input.ModifierKeys> actuellement enfoncé et la méthode <xref:System.Windows.Input.Keyboard.IsKeyDown%2A>, qui détermine si une touche spécifiée est enfoncée.

 L’exemple suivant utilise la méthode <xref:System.Windows.Input.Keyboard.GetKeyStates%2A> pour déterminer si une <xref:System.Windows.Input.Key> est à l’état inactif.

 [!code-csharp[keyargssnippetsample#KeyEventArgsKeyBoardGetKeyStates](~/samples/snippets/csharp/VS_Snippets_Wpf/KeyArgsSnippetSample/CSharp/Window1.xaml.cs#keyeventargskeyboardgetkeystates)]
 [!code-vb[keyargssnippetsample#KeyEventArgsKeyBoardGetKeyStates](~/samples/snippets/visualbasic/VS_Snippets_Wpf/KeyArgsSnippetSample/visualbasic/window1.xaml.vb#keyeventargskeyboardgetkeystates)]

 Les exemples d’API d’entrée sur la classe <xref:System.Windows.Input.Mouse> sont <xref:System.Windows.Input.Mouse.MiddleButton%2A>, qui obtient l’état du bouton central de la souris, et <xref:System.Windows.Input.Mouse.DirectlyOver%2A>, qui obtient l’élément sur lequel le pointeur de la souris se trouve actuellement.

 L’exemple suivant détermine si le <xref:System.Windows.Input.Mouse.LeftButton%2A> sur la souris est dans l’État <xref:System.Windows.Input.MouseButtonState.Pressed>.

 [!code-csharp[mouserelatedsnippets#MouseRelatedSnippetsGetLeftButtonMouse](~/samples/snippets/csharp/VS_Snippets_Wpf/MouseRelatedSnippets/CSharp/Window1.xaml.cs#mouserelatedsnippetsgetleftbuttonmouse)]
 [!code-vb[mouserelatedsnippets#MouseRelatedSnippetsGetLeftButtonMouse](~/samples/snippets/visualbasic/VS_Snippets_Wpf/MouseRelatedSnippets/visualbasic/window1.xaml.vb#mouserelatedsnippetsgetleftbuttonmouse)]

 Les classes <xref:System.Windows.Input.Mouse> et <xref:System.Windows.Input.Keyboard> sont traitées plus en détail dans cette vue d’ensemble.

### <a name="stylus-input"></a>Entrée du stylet
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] a intégré la prise en charge de la <xref:System.Windows.Input.Stylus>.  La <xref:System.Windows.Input.Stylus> est une entrée PEN rendue populaire par le Tablet PC.  les applications [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] peuvent traiter le stylet comme une souris à l’aide de l’API Mouse, mais [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] expose également une abstraction d’appareil Stylus qui utilise un modèle semblable au clavier et à la souris.  Toutes les API liées à Stylus contiennent le mot « Stylus ».
...
### <a name="keyboard-input-event-example"></a>Exemple d’événement d’entrée de clavier
 L’exemple suivant détecte un appui sur la touche de direction gauche.  Un <xref:System.Windows.Controls.StackPanel> est créé, avec une <xref:System.Windows.Controls.Button>.  Un gestionnaire d’événements pour écouter l’appui sur la touche de direction gauche est attaché à l’instance <xref:System.Windows.Controls.Button>.

 La première section de l’exemple crée le <xref:System.Windows.Controls.StackPanel> et la <xref:System.Windows.Controls.Button> et attache le gestionnaire d’événements pour le <xref:System.Windows.UIElement.KeyDown>.
...
### <a name="mouse-input-event-example"></a>Exemple d’événement d’entrée de souris
 Dans l’exemple suivant, la couleur <xref:System.Windows.Controls.Control.Background%2A> d’une <xref:System.Windows.Controls.Button> est modifiée lorsque le pointeur de la souris entre dans le <xref:System.Windows.Controls.Button>.  La couleur <xref:System.Windows.Controls.Control.Background%2A> est restaurée lorsque la souris quitte la <xref:System.Windows.Controls.Button>.
 ...
 La deuxième section est écrite en code et définit le gestionnaire d’événements.  Lorsque la souris entre dans le <xref:System.Windows.Controls.Button>, la couleur <xref:System.Windows.Controls.Control.Background%2A> du <xref:System.Windows.Controls.Button> est remplacée par <xref:System.Windows.Media.Brushes.SlateGray%2A>.  Lorsque la souris quitte le <xref:System.Windows.Controls.Button>, la couleur <xref:System.Windows.Controls.Control.Background%2A> du <xref:System.Windows.Controls.Button> est rétablie à <xref:System.Windows.Media.Brushes.AliceBlue%2A>.
...
## <a name="text-input"></a>Entrée de texte
 ...
 Pour les entrées au clavier, [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] envoie d’abord les événements <xref:System.Windows.ContentElement.KeyDown> @ no__t-2 @ no__t-3 appropriés. Si ces événements ne sont pas gérés et que la clé est textuelle (au lieu d’une touche de contrôle telle que des flèches directionnelles ou des touches de fonction), un événement <xref:System.Windows.ContentElement.TextInput> est déclenché.  Il n’existe pas toujours un mappage un-à-un simple entre les événements <xref:System.Windows.ContentElement.KeyDown> @ no__t-1 @ no__t-2 et <xref:System.Windows.ContentElement.TextInput>, car plusieurs séquences de touches peuvent générer un caractère unique d’entrée de texte et les séquences de touches uniques peuvent générer des chaînes à plusieurs caractères.  Cela est particulièrement vrai pour les langues telles que le chinois, le japonais et le coréen qui utilisent des éditeurs de méthode d’entrée (IME) pour générer les milliers de caractères possibles dans les alphabets correspondants.

 Lorsque [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] envoie un événement <xref:System.Windows.ContentElement.KeyUp> @ no__t-2 @ no__t-3, <xref:System.Windows.Input.KeyEventArgs.Key%2A> est défini sur <xref:System.Windows.Input.Key.System?displayProperty=nameWithType> si les séquences de touches peuvent faire partie d’un événement <xref:System.Windows.ContentElement.TextInput> (si ALT + S est enfoncé, par exemple). Cela permet au code dans un gestionnaire d’événements <xref:System.Windows.ContentElement.KeyDown> de vérifier <xref:System.Windows.Input.Key.System?displayProperty=nameWithType> et, s’il existe, de conserver le traitement du gestionnaire de l’événement <xref:System.Windows.ContentElement.TextInput> déclenché par la suite. Dans ce cas, les différentes propriétés de l’argument <xref:System.Windows.Input.TextCompositionEventArgs> peuvent être utilisées pour déterminer les séquences de touches d’origine. De même, si un IME est actif, <xref:System.Windows.Input.Key> a la valeur de <xref:System.Windows.Input.Key.ImeProcessed?displayProperty=nameWithType> et <xref:System.Windows.Input.KeyEventArgs.ImeProcessedKey%2A> donne la séquence d’origine ou les séquences de touches.
 ...
 Étant donné que les événements d’entrée propagent l’itinéraire des événements, le <xref:System.Windows.Controls.StackPanel> reçoit l’entrée quel que soit l’élément qui a le focus clavier. Le contrôle <xref:System.Windows.Controls.TextBox> est notifié en premier et le gestionnaire `OnTextInputKeyDown` est appelé uniquement si le <xref:System.Windows.Controls.TextBox> n’a pas géré l’entrée. Si l’événement <xref:System.Windows.UIElement.PreviewKeyDown> est utilisé à la place de l’événement <xref:System.Windows.UIElement.KeyDown>, le gestionnaire `OnTextInputKeyDown` est appelé en premier.
...
- Placez deux doigts sur un objet et faites pivoter les doigts pour appeler une manipulation de rotation. Cela fait pivoter habituellement l’objet.
 ...
 Pour plus d’informations sur la création d’une application qui permet à l’utilisateur de déplacer, redimensionner et faire pivoter un objet, consultez [Walkthrough : Création de votre première application Touch @ no__t-0.
...
6. L’événement <xref:System.Windows.UIElement.ManipulationCompleted> se produit lorsque la manipulation et l’inertie se terminent. Autrement dit, une fois que tous les événements <xref:System.Windows.UIElement.ManipulationDelta> se produisent, l’événement <xref:System.Windows.UIElement.ManipulationCompleted> se produit pour signaler que la manipulation est terminée.
 ...
 Vous pouvez annuler la manipulation en appelant la méthode <xref:System.Windows.Input.ManipulationStartingEventArgs.Cancel%2A> sur les arguments d’événement dans n’importe quel événement de manipulation, à l’exception de l’événement <xref:System.Windows.UIElement.ManipulationBoundaryFeedback>. Lorsque vous appelez <xref:System.Windows.Input.ManipulationStartingEventArgs.Cancel%2A>, les événements de manipulation ne sont plus déclenchés et les événements de souris se produisent pour les fonctions tactiles. Le tableau suivant décrit la relation entre le moment où la manipulation est annulée et les événements de souris qui se produisent.
...
### <a name="the-relationship-between-touch-and-manipulation-events"></a>Relation entre les événements tactiles et les événements de manipulation
 Un <xref:System.Windows.UIElement> peut toujours recevoir des événements tactiles. Lorsque la propriété <xref:System.Windows.UIElement.IsManipulationEnabled%2A> est définie sur `true`, un <xref:System.Windows.UIElement> peut recevoir à la fois des événements tactiles et des événements de manipulation.  Si l’événement <xref:System.Windows.UIElement.TouchDown> n’est pas géré (c’est-à-dire que la propriété <xref:System.Windows.RoutedEventArgs.Handled%2A> est `false`), la logique de manipulation capture le toucher à l’élément et génère les événements de manipulation. Si la propriété <xref:System.Windows.RoutedEventArgs.Handled%2A> est définie sur `true` dans l’événement <xref:System.Windows.UIElement.TouchDown>, la logique de manipulation ne génère pas d’événements de manipulation. L’illustration suivante montre la relation entre les événements tactiles et les événements de manipulation.

 ![Relation entre les événements tactiles et de manipulation]événements(./media/ndp-touchmanipulateevents.png "NDP_TouchManipulateEvents") tactiles et manipulations
...
- Quand le <xref:System.Windows.UIElement.GotTouchCapture> se produit, la logique de manipulation appelle la méthode <xref:System.Windows.Input.Manipulation.AddManipulator%2A?displayProperty=nameWithType>, qui génère l’événement <xref:System.Windows.UIElement.ManipulationStarting>.
...
### <a name="keyboard-focus"></a>Focus clavier
 ...
 Pour qu’un élément obtienne le focus clavier, la propriété <xref:System.Windows.UIElement.Focusable%2A> et les propriétés <xref:System.Windows.UIElement.IsVisible%2A> doivent avoir la valeur **true**.  Certaines classes, telles que <xref:System.Windows.Controls.Panel>, ont <xref:System.Windows.UIElement.Focusable%2A> ayant la valeur `false` par défaut ; par conséquent, vous devrez peut-être définir cette propriété sur `true` Si vous souhaitez que cet élément puisse obtenir le focus.

 L’exemple suivant utilise <xref:System.Windows.Input.Keyboard.Focus%2A> pour définir le focus clavier sur un <xref:System.Windows.Controls.Button>.  L’emplacement recommandé pour définir le focus initial dans une application se trouve dans le gestionnaire d’événements <xref:System.Windows.FrameworkElement.Loaded>.
...
### <a name="logical-focus"></a>Focus logique
 ...
 Une portée de focus est un élément conteneur qui effectue le suivi de l' <xref:System.Windows.Input.FocusManager.FocusedElement%2A> dans sa portée.  Quand le focus quitte une portée de focus, l’élément ayant le focus perd le focus clavier, mais conserve le focus logique.  Quand le focus revient dans la portée de focus, l’élément ayant le focus obtient le focus clavier.  Cela permet au focus clavier de changer entre des portées de focus, et de s’assurer que l’élément ayant le focus dans la portée de focus demeure l’élément ayant le focus quand le focus revient.
 ...
 L’exemple suivant rend un <xref:System.Windows.Controls.StackPanel> dans une portée de focus en définissant la propriété jointe <xref:System.Windows.Input.FocusManager.IsFocusScope%2A>.
 ...
 Les classes de [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] qui sont des portées de focus par défaut sont <xref:System.Windows.Window>, <xref:System.Windows.Controls.Menu>, <xref:System.Windows.Controls.ToolBar> et <xref:System.Windows.Controls.ContextMenu>.

 Un élément qui a le focus clavier aura également le focus logique pour la portée de focus à laquelle il appartient ; par conséquent, le fait de définir le focus sur un élément avec la méthode <xref:System.Windows.Input.Keyboard.Focus%2A> sur la classe <xref:System.Windows.Input.Keyboard> ou les classes d’éléments de base tentera d’attribuer le focus clavier et le focus logique à l’élément.
...
## <a name="commands"></a>Commandes
 Les commandes permettent de gérer les entrées à un niveau plus sémantique que l’entrée de périphérique.  Les commandes sont des directives simples, telles que `Cut`, `Copy`, `Paste` ou `Open`.  Elles sont utiles pour centraliser la logique de commande.  La même commande est accessible à partir d’un <xref:System.Windows.Controls.Menu>, sur un <xref:System.Windows.Controls.ToolBar> ou à l’aide d’un raccourci clavier. Les commandes fournissent également un mécanisme permettant de désactiver des contrôles quand la commande devient indisponible.

 <xref:System.Windows.Input.RoutedCommand> est l’implémentation [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] de <xref:System.Windows.Input.ICommand>.  Quand une <xref:System.Windows.Input.RoutedCommand> est exécutée, un <xref:System.Windows.Input.CommandManager.PreviewExecuted> et un événement <xref:System.Windows.Input.CommandManager.Executed> sont déclenchés sur la cible de commande, qui effectue un tunneling et effectue une propagation dans l’arborescence d’éléments comme une autre entrée.  Si une cible de commande n’est pas définie, l’élément qui a le focus clavier est la cible de commande.  La logique qui exécute la commande est attachée à un <xref:System.Windows.Input.CommandBinding>.  Lorsqu’un événement <xref:System.Windows.Input.CommandManager.Executed> atteint un <xref:System.Windows.Input.CommandBinding> pour cette commande spécifique, le <xref:System.Windows.Input.ExecutedRoutedEventHandler> sur le <xref:System.Windows.Input.CommandBinding> est appelé.  Ce gestionnaire exécute l’action de la commande.
 ...
 L’exemple suivant montre comment configurer un <xref:System.Windows.Controls.MenuItem> de sorte que, lorsque l’utilisateur clique dessus, il appelle la commande <xref:System.Windows.Input.ApplicationCommands.Paste%2A> sur le <xref:System.Windows.Controls.TextBox>, en supposant que le <xref:System.Windows.Controls.TextBox> a le focus clavier.
