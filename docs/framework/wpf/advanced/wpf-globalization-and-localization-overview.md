...
Lorsque vous concevez un [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] , envisagez de mettre en œuvre les meilleures pratiques suivantes :

- Écrivez votre [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] dans [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] ; Évitez [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] de créer dans le code. Lorsque vous créez votre à [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] l’aide de [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] , vous l’exposez par le biais d’API de localisation intégrées.
...
- Évitez d’utiliser des positions absolues et des tailles fixes pour disposer le contenu ; Utilisez plutôt le dimensionnement relatif ou automatique.
  ...
  - Utilisez <xref:System.Windows.Window.SizeToContent%2A> et conservez la largeur et la hauteur définies sur `Auto` .

  - Évitez <xref:System.Windows.Controls.Canvas> d’utiliser pour disposer des [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] s.
...
- Lorsque vous créez des applications de navigation qui peuvent être localisées dans une culture qui présente du texte dans un format de droite à gauche, définissez explicitement la valeur <xref:System.Windows.FlowDirection> de chaque page pour vous assurer que la page n’hérite pas <xref:System.Windows.FlowDirection> de <xref:System.Windows.Navigation.NavigationWindow> .

- Lorsque vous créez des applications de navigation autonomes hébergées à l’extérieur d’un navigateur, définissez <xref:System.Windows.Application.StartupUri%2A> pour votre application initiale sur un au <xref:System.Windows.Navigation.NavigationWindow> lieu de la page (par exemple, `<Application StartupUri="NavigationWindow.xaml">` ). Cette conception vous permet de modifier la <xref:System.Windows.FlowDirection> de la fenêtre et la barre de navigation. Pour plus d’informations et un exemple, consultez [exemple de page d’accueil de globalisation](https://github.com/microsoft/WPF-Samples/tree/master/Globalization%20and%20Localization/GlobalizationHomepage).
...
- Utilisez les attributs de localisation pour contrôler la localisation au lieu d’omettre de manière sélective les <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> Propriétés sur les éléments. Pour plus d’informations [, consultez attributs et commentaires de localisation](localization-attributes-and-comments.md) .

- Utilisez `msbuild -t:updateuid` et `-t:checkuid` pour ajouter et vérifier des <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> Propriétés dans [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] votre. Utilisez <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> les propriétés pour suivre les modifications entre le développement et la localisation. <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A>les propriétés vous aident à localiser de nouvelles modifications de développement. Si vous ajoutez manuellement des <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> propriétés à un [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] , la tâche est généralement fastidieuse et moins précise.

  - Ne modifiez pas ou ne modifiez pas les <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> Propriétés après avoir commencé la localisation.
  ...
  - Définissez l' `UltimateResourceFallback` emplacement dans AssemblyInfo. \* pour spécifier la langue appropriée pour le secours (par exemple, `[assembly: NeutralResourcesLanguage("en-US",   UltimateResourceFallbackLocation.Satellite)]` ).
    ...
    Si vous décidez d’inclure votre langue source dans l’assembly principal en omettant la `<UICulture>` balise dans votre fichier projet, définissez l' `UltimateResourceFallback` emplacement en tant qu’assembly principal au lieu du satellite (par exemple, `[assembly: NeutralResourcesLanguage("en-US", UltimateResourceFallbackLocation.MainAssembly)]` ).
...
Lorsque vous développez une [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] application, le processus de génération de la localisation est le suivant :
...
- Lorsque le fichier est compilé dans la build, [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] est converti au format BAML de XAML. Les fichiers culturellement neutres `MyDialog.exe` et les fichiers dépendants de la culture (en anglais) `MyDialog.resources.dll` sont publiés sur le client anglophone.
...
Cette section contient des exemples d’applications localisées pour vous aider à comprendre comment créer et localiser des [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] applications.
...
<xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A>des propriétés sont nécessaires pour que les [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] API de localisation fonctionnent correctement.

Elles sont utilisées par les [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] API de localisation pour suivre les modifications entre le développement et la localisation du [!INCLUDE[TLA#tla_ui](../../../../includes/tlasharptla-ui-md.md)] . <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A>les propriétés vous permettent de fusionner une version plus récente de [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] avec une localisation plus ancienne de [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] . Vous ajoutez une <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> propriété en exécutant `msbuild -t:updateuid RunDialog.csproj` dans un interpréteur de commandes. Il s’agit de la méthode recommandée pour ajouter des <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> Propriétés, car l’ajout manuel est généralement plus long et moins précis. Vous pouvez vérifier que <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> les propriétés sont correctement définies en exécutant `msbuild -t:checkuid RunDialog.csproj` .
...
Les deux premières colonnes où l’étiquette **Open :** et <xref:System.Windows.Controls.ComboBox> sont placées utilisent 10 pour cent de la [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] Largeur totale.
...
|TextBlock_2:System.Windows.Controls.TextBlock.$Content|Texte|Ouverte :|
...
Sur les fenêtres allemandes, si ce resources.dll est placé dans un dossier de-DE à côté de l’assembly principal, cette ressource est chargée automatiquement au lieu de celle figurant dans le dossier en-US. Si vous ne disposez pas d’une version allemande de Windows pour tester cela, définissez la culture sur la culture de Windows que vous utilisez (par exemple, `en-US` ) et remplacez la dll des ressources d’origine.
...
.NET choisit automatiquement l’assembly de ressources satellites à charger en fonction de l’application <xref:System.Threading.Thread.CurrentUICulture%2A?displayProperty=nameWithType> . Par défaut, il s’agit de la culture de votre système d’exploitation Windows. Si vous utilisez des fenêtres en allemand, le fichier *de-DE\MyDialog.resources.dll* se charge. Si vous utilisez des fenêtres en anglais, le fichier *en-US\MyDialog.resources.dll* se charge. Vous pouvez définir la ressource de secours ultime pour votre application en spécifiant l' `NeutralResourcesLanguage` attribut dans le fichier *AssemblyInfo* de votre projet. Par exemple, si vous spécifiez :

`[assembly: NeutralResourcesLanguage("en-US", UltimateResourceFallbackLocation.Satellite)]`

le fichier *en-US\MyDialog.resources.dll* est alors utilisé avec les fenêtres en allemand si aucun des fichiers suivants n’est disponible : *de-DE\MyDialog.resources.dll* ou *de\MyDialog.resources.dll*.

### <a name="microsoft-saudi-arabia-homepage"></a>Page d’accueil de Microsoft pour l’Arabie Saoudite

Les graphiques suivants montrent une page d’accueil en anglais et en arabe. Pour obtenir l’exemple complet qui produit ces graphiques, consultez [exemple de page d’accueil de globalisation](https://github.com/microsoft/WPF-Samples/tree/master/Globalization%20and%20Localization/GlobalizationHomepage).
...
Cette simulation du site web de Microsoft pour l’Arabie saoudite illustre les fonctionnalités de globalisation fournies pour les langues de droite à gauche. Les langues telles que l’hébreu et l’arabe ont un ordre de lecture de droite à gauche, de sorte que la disposition de [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] doit souvent être présentée de façon très différente de celle des langues de gauche à droite, par exemple l’anglais. La localisation d’une langue de gauche à droite vers une langue de droite à gauche, ou l’inverse, peut se révéler difficile. [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] a été conçu pour faciliter ces localisations.
...
Notez la <xref:System.Windows.FrameworkElement.FlowDirection%2A> propriété sur <xref:System.Windows.Controls.Page> . La modification de cette propriété pour <xref:System.Windows.FlowDirection.RightToLeft> modifier la propriété <xref:System.Windows.FrameworkElement.FlowDirection%2A> du <xref:System.Windows.Controls.Page> et de ses éléments enfants, de sorte que la disposition de ce soit retournée de manière à ce qu’elle [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] devienne de droite à gauche, comme l’attend un utilisateur arabe. Il est possible de substituer le comportement d’héritage en spécifiant un Explicit <xref:System.Windows.FrameworkElement.FlowDirection%2A> sur tout élément. La <xref:System.Windows.FrameworkElement.FlowDirection%2A> propriété est disponible sur tout <xref:System.Windows.FrameworkElement> élément lié au document et a une valeur implicite de <xref:System.Windows.FlowDirection.LeftToRight> .

Notez que même les pinceaux de dégradé d’arrière-plan sont correctement retournées lorsque la racine <xref:System.Windows.FrameworkElement.FlowDirection%2A> est modifiée :
...
Dans de nombreux cas, le contenu peut être ambigu et difficile à traduire. Le développeur ou le concepteur a la possibilité de fournir un contexte supplémentaire et des commentaires aux localiseurs via des commentaires de localisation. Par exemple, la localisation. les commentaires ci-dessous clarifient l’utilisation du caractère' &#124; '.
...
Les attributs de localisation par défaut [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] fournis par peuvent également être remplacés par le biais du code, de sorte que vous pouvez définir correctement les valeurs par défaut appropriées pour les contrôles personnalisés. Par exemple :
...
Si vous spécifiez une police qui ne prend pas en charge une plage de point de contenu donnée, [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] passe automatiquement à une police qui effectue cette opération à l’aide de l’interface utilisateur globale. compositefont qui se trouve dans votre répertoire Windows/Fonts. Les polices composites fonctionnent exactement comme n’importe quelle autre police et peuvent être utilisées explicitement en définissant un élément `FontFamily` (par exemple, `FontFamily="Global User Interface"` ). Vous pouvez spécifier votre propre police de base préférée en créant votre propre police composite, et en spécifiant la police à utiliser pour des plages de points de code et des langues spécifiques.

Pour plus d’informations sur les polices composites <xref:System.Windows.Media.FontFamily> , consultez.

**Localisation de la page d’accueil de Microsoft**

Vous pouvez suivre la même procédure que celle de l’exemple de boîte de dialogue Run pour localiser cette application. Le fichier. csv localisé pour l’arabe est disponible dans l’exemple de la [page d’accueil de globalisation](https://github.com/microsoft/WPF-Samples/tree/master/Globalization%20and%20Localization/GlobalizationHomepage).
