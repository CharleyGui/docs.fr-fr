...
Vous pouvez tirer le meilleur parti des fonctionnalités de globalisation et de localisation intégrées [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] à en suivant les conseils relatifs à la conception et à la localisation de l’interface utilisateur fournis par cette section.

### <a name="best-practices-for-wpf-ui-design"></a>Bonnes pratiques pour la conception de l’interface utilisateur WPF

Lorsque vous concevez [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]un, [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]envisagez de mettre en œuvre les meilleures pratiques suivantes:

- Écrivez votre [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] dans [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]; Évitez de [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] créer dans le code. Lorsque vous créez votre [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] à l' [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]aide de, vous l’exposez par le biais d’API de localisation intégrées.
...
- Évitez d’utiliser des positions absolues et des tailles fixes pour disposer le contenu; Utilisez plutôt le dimensionnement relatif ou automatique.
  ...
  - Utilisez <xref:System.Windows.Window.SizeToContent%2A> et conservez la largeur et la hauteur `Auto`définies sur.

  - Évitez d' <xref:System.Windows.Controls.Canvas> utiliser pour disposer [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]des s.
...
- Lorsque vous créez des applications de navigation qui peuvent être localisées dans une culture qui présente du texte dans un format de droite à gauche, définissez <xref:System.Windows.FlowDirection> explicitement la valeur de chaque page pour vous assurer que <xref:System.Windows.FlowDirection> la page <xref:System.Windows.Navigation.NavigationWindow>n’hérite pas de.

- Lorsque vous créez des applications de navigation autonomes hébergées à l’extérieur d’un navigateur <xref:System.Windows.Application.StartupUri%2A> , définissez pour votre application initiale <xref:System.Windows.Navigation.NavigationWindow> sur un au lieu de la page (par `<Application StartupUri="NavigationWindow.xaml">`exemple,). Cette conception vous permet de modifier la <xref:System.Windows.FlowDirection> de la fenêtre et la barre de navigation. Pour plus d’informations et un exemple, consultez [exemple de page d’accueil](https://go.microsoft.com/fwlink/?LinkID=159990)de globalisation.

### <a name="best-practices-for-wpf-localization"></a>Bonnes pratiques pour la localisation WPF

Lorsque vous localisez [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]des applications basées sur, envisagez d’implémenter ces meilleures pratiques:

- Utilisez les commentaires de localisation pour fournir un contexte supplémentaire aux localisateurs.

- Utilisez les attributs de localisation pour contrôler la localisation au lieu d' <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> omettre de manière sélective les propriétés sur les éléments. Pour plus d’informations [, consultez attributs et commentaires de localisation](localization-attributes-and-comments.md) .

- Utilisez `msbuild -t:updateuid` <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]et `-t:checkuid` pour ajouter et vérifier des propriétés dans votre. Utilisez <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> les propriétés pour suivre les modifications entre le développement et la localisation. <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A>les propriétés vous aident à localiser de nouvelles modifications de développement. Si vous ajoutez <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> manuellement des propriétés à [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]un, la tâche est généralement fastidieuse et moins précise.

  - Ne modifiez pas ou ne <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> modifiez pas les propriétés après avoir commencé la localisation.

  - N’utilisez pas de <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> propriétés dupliquées (n’oubliez pas ce Conseil quand vous utilisez la commande copier-coller).

  - Définissez l' `UltimateResourceFallback` emplacement dans AssemblyInfo. * pour spécifier la langue appropriée pour le secours (par exemple `[assembly: NeutralResourcesLanguage("en-US",   UltimateResourceFallbackLocation.Satellite)]`,).

    Si vous décidez d’inclure votre langue source dans l’assembly principal en omettant la `<UICulture>` balise dans votre fichier projet, définissez `UltimateResourceFallback` l’emplacement en tant qu’assembly principal au lieu du satellite ( `[assembly: NeutralResourcesLanguage("en-US", UltimateResourceFallbackLocation.MainAssembly)]`par exemple,).

## <a name="localize-a-wpf-application"></a>Localiser une application WPF

Lorsque vous localisez une [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] application, vous disposez de plusieurs options. Par exemple, vous pouvez lier les ressources localisables dans votre application à un [!INCLUDE[TLA2#tla_xml](../../../../includes/tla2sharptla-xml-md.md)] fichier, stocker du texte localisable dans des tables resx ou faire en sorte que [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] votre localisateur utilise des fichiers. Cette section décrit un flux de travail de localisation qui utilise le formulaire BAML de XAML, qui offre plusieurs avantages:
...
- Vous pouvez valider les éléments et la sémantique de la source d’origine au moment de la compilation, car la forme BAML [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]de XAML est la forme compilée de.

### <a name="localization-build-process"></a>Processus de génération de la localisation

Lorsque vous développez [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] une application, le processus de génération de la localisation est le suivant:

- Le développeur crée et globalise l' [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] application. Dans le fichier projet, le développeur `<UICulture>en-US</UICulture>` définit afin que, lorsque l’application est compilée, un assembly principal indépendant du langage soit généré. Cet assembly a un fichier .resources.dll satellite qui contient toutes les ressources localisables. Si vous le souhaitez, vous pouvez conserver la langue source dans l’assembly principal, car nos API de localisation prennent en charge l’extraction à partir de l’assembly principal.

- Lorsque le fichier est compilé dans la build, [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] est converti au format BAML de XAML. Les fichiers culturellement `MyDialog.exe` neutres et les fichiers dépendants `MyDialog.resources.dll` de la culture (en anglais) sont publiés sur le client anglophone.

### <a name="localization-workflow"></a>Flux du travail de localisation

Le processus de localisation commence après la génération du `MyDialog.resources.dll` fichier non localisé. Les [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] éléments et les propriétés de votre [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] original sont extraits de la forme BAML de XAML dans des paires clé-valeur à l’aide <xref:System.Windows.Markup.Localizer>des API sous. Les localiseurs utilisent les paires clé-valeur pour localiser l’application. Vous pouvez générer un nouveau fichier .resource.dll à partir des nouvelles valeurs une fois la localisation terminée.

Les clés des paires clé-valeur sont `x:Uid` des valeurs qui sont placées par le développeur dans le d’origine. [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] Ces `x:Uid` valeurs permettent à l’API de suivre et de fusionner les modifications qui se produisent entre le développeur et le localisateur pendant la localisation. Par exemple, si le développeur modifie le [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] après le début de la localisation du localisateur, vous pouvez fusionner la modification de développement avec le travail de localisation déjà terminé afin de perdre le travail de traduction minimal.

Le graphique suivant montre un flux de travail de localisation standard basé sur le formulaire BAML de XAML. Ce diagramme part du principe que le développeur écrit l’application en anglais. Le développeur crée et globalise l’application WPF. Dans le fichier projet, le développeur `<UICulture>en-US</UICulture>` définit afin que, lors de la génération, un assembly principal indépendant de la langue soit généré avec un fichier. resources. dll satellite contenant toutes les ressources localisables. Si vous le voulez, vous pouvez conserver la langue source dans l’assembly principal, car nos API de localisation WPF prennent en charge l’extraction à partir de l’assembly principal. Après le processus de génération, le code XAML est compilé en BAML. Le fichier MyDialog.exe.resources.dll indépendant de la culture est livré aux clients anglophones.
...
Cette section contient des exemples d’applications localisées pour vous aider à comprendre comment créer et localiser [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] des applications.
...
Cet exemple génère une boîte de dialogue **exécuter** à [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] l' [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)]aide de et de. Cette boîte de dialogue est équivalente à la boîte de dialogue **exécuter** disponible dans [!INCLUDE[TLA#tla_win](../../../../includes/tlasharptla-win-md.md)] le menu Démarrer.
...
<xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A>des propriétés sont nécessaires pour que [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] les API de localisation fonctionnent correctement.

Elles sont utilisées par [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] les API de localisation pour suivre les modifications entre le développement et [!INCLUDE[TLA#tla_ui](../../../../includes/tlasharptla-ui-md.md)]la localisation du. <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A>les propriétés vous permettent de fusionner une version plus [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] récente de avec une localisation plus [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]ancienne de. Vous ajoutez une <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> propriété en exécutant `msbuild -t:updateuid RunDialog.csproj` dans un interpréteur de commandes. Il s’agit de la méthode recommandée <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> pour ajouter des propriétés, car l’ajout manuel est généralement plus long et moins précis. Vous pouvez vérifier que <xref:System.Windows.Markup.Localizer.BamlLocalizableResourceKey.Uid%2A> les propriétés sont correctement définies en `msbuild -t:checkuid RunDialog.csproj`exécutant.

Le [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] est structuré à l’aide <xref:System.Windows.Controls.Grid> du contrôle, qui est un contrôle utile pour tirer parti de la disposition automatique [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]dans. Notez que la boîte de dialogue est divisée en trois lignes et cinq colonnes. L’une des définitions de ligne et de colonne n’a pas une taille fixe; par conséquent, [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] les éléments positionnés dans chaque cellule peuvent s’adapter pour augmenter ou diminuer la taille lors de la localisation.

[!code-xaml[GlobalizationRunDialog#GridColumnDef](~/samples/snippets/csharp/VS_Snippets_Wpf/GlobalizationRunDialog/CS/Window1.xaml#gridcolumndef)]

Les deux premières colonnes où l’étiquette **Open:** et <xref:System.Windows.Controls.ComboBox> sont placées utilisent 10 pour cent [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] de la largeur totale.

[!code-xaml[GlobalizationRunDialog#GridColumnDef2](~/samples/snippets/csharp/VS_Snippets_Wpf/GlobalizationRunDialog/CS/Window1.xaml#gridcolumndef2)]

Notez que l’exemple utilise la fonctionnalité de dimensionnement partagé de <xref:System.Windows.Controls.Grid>. Les trois dernières colonnes tirent parti de cela en les plaçant dans le <xref:System.Windows.Controls.DefinitionBase.SharedSizeGroup%2A>même. Comme vous pouvez vous y attendre d’après le nom de la propriété, ceci permet aux colonnes de partager la même taille. Par conséquent, lorsque le bouton «Parcourir...» est localisée dans la chaîne plus longue «Durchsuchen...», tous les boutons augmentent en largeur au lieu d’avoir un petit bouton «OK» et un «Durchsuchen...» de grande taille disproportionnée... bouton.

**XML: lang**

`xml:lang="en-US"`

Notez la [gestion de XML: lang en XAML](../../xaml-services/xml-lang-handling-in-xaml.md) placée à l’élément racine de [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)]. Cette propriété décrit la culture d’un élément donné et de ses enfants. Cette valeur est utilisée par plusieurs fonctionnalités dans [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] et doit être modifiée de manière appropriée lors de la localisation. Cette valeur change le dictionnaire de langue utilisé pour la coupure et la vérification de l’orthographe des mots. Elle affecte également l’affichage des chiffres et la façon dont le système de police de base sélectionne la police à utiliser. Enfin, la propriété affecte la façon dont les nombres sont affichés et celle dont les textes écrits dans des écritures complexes sont mis en forme. La valeur par défaut est « en-US ».

**Génération d’un assembly de ressources satellite**

*Dans .csproj :*

Modifiez le `.csproj` fichier et ajoutez la balise suivante à un non `<PropertyGroup>`conditionnel:

`<UICulture>en-US</UICulture>`

Notez l’ajout d’une `UICulture` valeur. Lorsque cette valeur est définie sur une <xref:System.Globalization.CultureInfo> valeur valide telle que en-US, la génération du projet génère un assembly satellite avec toutes les ressources localisables qu’il contient.
...
Le `RunIcon.JPG` n’a pas besoin d’être localisé, car il doit être identique pour toutes les cultures. `Localizable`a la valeur `false` afin qu’elle reste dans l’assembly principal indépendant de la langue au lieu de l’assembly satellite. La valeur par défaut de toutes les ressources `Localizable` noncompilable est `true`définie sur.
...
Les graphiques suivants montrent une page d’accueil en anglais et en arabe. Pour obtenir l’exemple complet qui produit ces graphiques, consultez [exemple de page d’accueil](https://go.microsoft.com/fwlink/?LinkID=159990)de globalisation.
...
Cette simulation du site web de Microsoft pour l’Arabie saoudite illustre les fonctionnalités de globalisation fournies pour les langues de droite à gauche. Les langues telles que l’hébreu et l’arabe ont un ordre de lecture de droite à gauche, [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] de sorte que la disposition de doit souvent être présentée de façon très différente de celle des langues de gauche à droite, par exemple l’anglais. La localisation d’une langue de gauche à droite vers une langue de droite à gauche, ou l’inverse, peut se révéler difficile. [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] a été conçu pour faciliter ces localisations.
...
Notez la <xref:System.Windows.FrameworkElement.FlowDirection%2A> propriété sur <xref:System.Windows.Controls.Page>. La modification de cette <xref:System.Windows.FlowDirection.RightToLeft> propriété pour modifier <xref:System.Windows.FrameworkElement.FlowDirection%2A> la propriété <xref:System.Windows.Controls.Page> du et de ses éléments enfants, de sorte que [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] la disposition de ce soit retournée de manière à ce qu’elle devienne de droite à gauche, comme l’attend un utilisateur arabe. Il est possible de substituer le comportement d’héritage en <xref:System.Windows.FrameworkElement.FlowDirection%2A> spécifiant un Explicit sur tout élément. La <xref:System.Windows.FrameworkElement.FlowDirection%2A> propriété est disponible sur tout <xref:System.Windows.FrameworkElement> élément lié au document et a une valeur implicite <xref:System.Windows.FlowDirection.LeftToRight>de.

Notez que même les pinceaux de dégradé d’arrière-plan sont correctement <xref:System.Windows.FrameworkElement.FlowDirection%2A> retournées lorsque la racine est modifiée:
...
Outre les attributs de lisibilité et de modifiabilité, [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] fournit une énumération des catégories d’interface<xref:System.Windows.LocalizationCategory>utilisateur courantes () qui peuvent être utilisées pour fournir davantage de contexte aux localiseurs. Les [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] catégories par défaut pour les contrôles de plateforme peuvent également [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] être remplacées dans:

[!code-xaml[LocalizationComAtt#LocalizationAttributesOverridden](~/samples/snippets/csharp/VS_Snippets_Wpf/LocalizationComAtt/CSharp/Attributes.xaml#localizationattributesoverridden)]

Les attributs [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] de localisation par défaut fournis par peuvent également être remplacés par le biais du code, de sorte que vous pouvez définir correctement les valeurs par défaut appropriées pour les contrôles personnalisés. Par exemple :
...
Si vous spécifiez une police qui ne prend pas en charge une plage [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] de point de contenu donnée, passe automatiquement à une police qui effectue cette opération à l’aide de l’interface utilisateur globale. compositefont qui se trouve dans votre répertoire Windows/Fonts. Les polices composites fonctionnent exactement comme n’importe quelle autre police et peuvent être utilisées explicitement en `FontFamily` définissant un élément `FontFamily="Global User Interface"`(par exemple,). Vous pouvez spécifier votre propre police de base préférée en créant votre propre police composite, et en spécifiant la police à utiliser pour des plages de points de code et des langues spécifiques.

Pour plus d’informations sur les polices <xref:System.Windows.Media.FontFamily>composites, consultez.

**Localisation de la page d’accueil de Microsoft**

Vous pouvez suivre la même procédure que celle de l’exemple de boîte de dialogue Run pour localiser cette application. Le fichier. csv localisé pour l’arabe est disponible dans l’exemple de la [page d’accueil](https://go.microsoft.com/fwlink/?LinkID=159990)de globalisation.
