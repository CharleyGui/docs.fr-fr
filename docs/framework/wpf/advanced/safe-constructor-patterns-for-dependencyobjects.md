...
### <a name="fxcop-rule-enforcement-vs-property-system-virtuals"></a>Application des règles FXCop et virtuelles du système de propriétés  
...
```  
 ...
 Quand le code d’application appelle `new MyClass(objectvalue)`, cela appelle le constructeur sans paramètre et les constructeurs de classe de base. Il définit ensuite `Property1 = object1`, qui appelle la méthode virtuelle `OnPropertyChanged` sur le <xref:System.Windows.DependencyObject>propriétaire `MyClass`.  La substitution référence `_myList`, qui n’a pas encore été initialisé.  
...
#### <a name="setting-dependency-properties-with-setvalue"></a>Définition de propriétés de dépendance avec SetValue  
 Ces mêmes modèles s’appliquent si vous définissez une propriété qui n’a pas de wrapper pour la commodité des paramètres de propriété et que vous définissez des valeurs avec <xref:System.Windows.DependencyObject.SetValue%2A>. Vos appels à <xref:System.Windows.DependencyObject.SetValue%2A> qui passent par les paramètres du constructeur doivent également appeler le constructeur sans paramètre de la classe pour l’initialisation.  
