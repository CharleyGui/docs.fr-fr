...
Lors de la planification de la sécurisation d’un service OData basé sur le WCF Data Services, vous devez gérer l’authentification, le processus de découverte et de vérification de l’identité d’un principal, ainsi que l’autorisation, le processus de détermination de l’identité d’un principal authentifié. autorisé à accéder aux ressources demandées. Vous devez également déterminer si chiffrer le message à l'aide de SSL.  
...
### <a name="implement-role-based-interceptors"></a>Implémentation d'intercepteurs basés sur le rôle  
 Les intercepteurs vous permettent d'intercepter les demandes adressées aux ressources du service de données avant qu'elles ne soient traitées par le service de données. Pour plus d’informations, consultez [intercepteurs](interceptors-wcf-data-services.md). Les intercepteurs vous permettent de prendre des décisions d'autorisation en fonction de l'utilisateur authentifié qui effectue la demande. Pour obtenir un exemple de la façon de restreindre l’accès aux ressources du service de données en fonction d’une [identité d’utilisateur authentifié, consultez Procédure : Interceptez les messages](how-to-intercept-data-service-messages-wcf-data-services.md)du service de données.  
