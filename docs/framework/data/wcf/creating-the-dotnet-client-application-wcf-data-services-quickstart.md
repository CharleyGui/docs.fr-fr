...
Il s’agit de la dernière tâche du démarrage rapide WCF Data Services. Dans cette tâche, vous allez ajouter une application console à la solution, ajouter une référence au flux Open Data Protocol (OData) dans cette nouvelle application cliente et accéder au flux OData à partir de l’application cliente à l’aide des classes et du client du service de données client générés. bibliotheque.
...
3. Dans **Explorateur de solutions** sous **NorthwindClient**, ouvrez la page de codes du fichier MainWindow. xaml et ajoutez l’instruction `using` suivante (`Imports` dans Visual Basic).
