...
## <a name="handling-batch-update-related-events-and-errors"></a>Gestion des événements et des erreurs liés aux mises à jour par lots.  
 Le **DataAdapter** comporte deux événements liés à la mise à jour : **RowUpdating** et **RowUpdated**. Dans les versions précédentes d'ADO.NET, en cas de désactivation du traitement par lots, chacun de ces événements était généré une fois pour chaque ligne traitée. **RowUpdating** est généré avant la mise à jour, et **RowUpdated** est généré une fois la mise à jour de la base de données terminée.  
