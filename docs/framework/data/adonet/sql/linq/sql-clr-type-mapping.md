...
- [Matrice de comportement au moment de l’exécution du mappage de type](#BehaviorMatrix)  
  
- [Différences de comportement entre le CLR et l’exécution SQL](#BehaviorDiffs)  
  
- [Mappage d’énumération](#EnumMapping)  
  
- [Mappage numérique](#NumericMapping)  
  
- [Mappage texte et XML](#TextMapping)  
  
- [Mappage de la date et de l’heure](#DateMapping)  
  
- [Mappage binaire](#BinaryMapping)  
...
- Commentaires  
 ...
 Bien que vous puissiez <xref:System.Xml.Linq.XElement> mapper <xref:System.Xml.Linq.XDocument> et à SQL Server comme indiqué dans la [matrice de comportement](#BehaviorMatrix)de l’exécution du <xref:System.Data.Linq.DataContext.CreateDatabase%2A?displayProperty=nameWithType> mappage de type, la méthode n’a pas de mappage de type SQL Server par défaut pour ces types.  
  
### <a name="custom-types"></a>Types personnalisés  
 Si une classe implémente `Parse()` et `ToString()`, vous pouvez mapper l’objet à n’importe quel type de`CHAR`texte `NCHAR`SQL ( `NVARCHAR`, `TEXT`, `NTEXT` `VARCHAR`, `XML`,,,). L'objet est stocké dans la base de données en envoyant la valeur retournée par `ToString()` à la colonne de la base de données mappée. L'objet est reconstruit en appelant `Parse()` sur la chaîne retournée par la base de données.  
...
### <a name="sql-server-filestream"></a>FILESTREAM SQL Server  
 ...
 Bien que vous puissiez mapper des colonnes `VARBINARY(MAX)` avec l'attribut `FILESTREAM` à des objets <xref:System.Data.Linq.Binary>, la méthode <xref:System.Data.Linq.DataContext.CreateDatabase%2A?displayProperty=nameWithType> ne peut pas créer automatiquement des colonnes avec l'attribut `FILESTREAM`. Pour plus d’informations `FILESTREAM`sur, consultez [vue d’ensemble de FileStream](https://go.microsoft.com/fwlink/?LinkId=115291) sur documentation en ligne de Microsoft SQL Server.  
