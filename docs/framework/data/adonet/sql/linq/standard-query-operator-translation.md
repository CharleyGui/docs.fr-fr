...
<xref:System.Linq.Enumerable.Take%2A>les <xref:System.Linq.Enumerable.Skip%2A> méthodes et sont bien définies uniquement par rapport aux *jeux ordonnés*. La sémantique des jeux non ordonnés ou des multijeux n'est pas définie.
...
Pour plus d'informations, voir [Procédure : Mapper des hiérarchies](how-to-map-inheritance-hierarchies.md)d’héritage.
...
Les limitations suivantes de l’SQL Server 2000 (par rapport à Microsoft SQL Server [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 2005) affectent la prise en charge.
...
Les types `text` `nvarchar(max)`  /  `varchar(max)`de données ne peuvent pas être utilisés dans certaines opérations de requête sur, qui sont prises en charge par Microsoft SQL Server 2005.  /  `ntext`
...
Le classeur SQL Server 2000 (via SP4) a certaines particularités qui sont déclenchées par des requêtes imbriquées. Le jeu de requêtes SQL qui déclenche ces spécificités n'est pas bien défini. Pour cette raison, vous ne pouvez pas définir l' [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] ensemble de requêtes qui peuvent provoquer des exceptions de SQL Server.
...
- Les méthodes qui suivent <xref:System.Linq.Enumerable.AsEnumerable%2A> la méthode sont *exécutées localement*. Cette méthode ne provoque pas d'exécution immédiate.
