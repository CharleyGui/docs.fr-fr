...
- Utilisez votre propre code pour supprimer en premier les objets enfants qui empêchent la suppression de l'objet parent.  
 ...
 Pour plus d'informations, voir [Procédure : Supprimer des lignes de la](how-to-delete-rows-from-the-database.md)base de données.  
...
- Vous avez une `Imports` directive (Visual Basic) `using` ouC#() pour <xref:System.Linq> et <xref:System.Data.Linq>.  
