...
Les exemples de cette rubrique montrent comment utiliser les méthodes `Where` et `Where…Contains` pour interroger le [modèle de vente AdventureWorks Sales Model](https://archive.codeplex.com/?p=msftdbprodsamples) à l’aide de la syntaxe de requête fondée sur une méthode. Remarque : où...`Contains` ne peut pas être utilisé dans le cadre d’une [requête compilée](compiled-queries-linq-to-entities.md).  
