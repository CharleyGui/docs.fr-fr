...
L’exemple suivant illustre l’utilisation d’un **DataReader**, `reader` où représente un DataReader valide `command` et représente un objet Command valide.  
...
```  
 ...
 Le code suivant retourne les résultats de la commande précédente à l' <xref:System.Data.OracleClient.OracleDataReader.Read> aide <xref:System.Data.OracleClient.OracleDataReader.NextResult> des méthodes et <xref:System.Data.OracleClient.OracleDataReader>de. Les paramètres REF CURSOR sont retournés dans l'ordre.  
