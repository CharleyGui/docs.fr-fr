...
## <a name="the-net-framework-data-provider-for-odbc"></a>Fournisseur de données .NET Framework pour ODBC  
 À partir de la version 1,1, le .NET Framework fournisseur de données pour<xref:System.Data.Odbc>ODBC () est inclus dans le cadre de la .NET Framework. Le fournisseur de données ODBC est disponible pour .NET Framework les développeurs de la version 1,0 en tant que téléchargement Web à partir du [Centre de développement accès et stockage des données](https://go.microsoft.com/fwlink/?linkid=4173). L’espace de noms du .NET Framework téléchargé Fournisseur de données pour ODBC est **Microsoft. Data. ODBC**.  
  
 Si vous disposez d’une application développée pour la version .NET Framework 1,0 qui utilise le fournisseur de données ODBC pour se connecter à votre source de données, et que vous souhaitez exécuter cette application sur le .NET Framework version 1,1 ou une version ultérieure, vous devez mettre à jour l’espace de noms pour les fichiers DAT ODBC un fournisseur vers **System. Data. ODBC**. Vous devez ensuite le recompiler pour la version plus récente du .NET Framework.  
...
## <a name="the-net-framework-data-provider-for-oracle"></a>Fournisseur de données .NET Framework pour Oracle  
 À partir de la version 1,1, le .NET Framework fournisseur de données pour<xref:System.Data.OracleClient>Oracle () est inclus dans le cadre de la .NET Framework. Le fournisseur de données est disponible pour .NET Framework les développeurs de la version 1,0 en tant que téléchargement Web à partir du [Centre de développement accès et stockage des données](https://go.microsoft.com/fwlink/?linkid=4173).  
...
## <a name="sqlcommand-execution"></a>Exécution de SqlCommand  
 À partir de la version .NET Framework 1,1, la façon <xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> dont exécute les commandes au niveau de la source de données a été modifiée.  
  
 Dans la version 1,0 de .NET Framework <xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> , a exécuté toutes les commandes dans le contexte de la procédure stockée **sp_executesql** . Par conséquent, les commandes qui affectent l'état de la connexion (SET NOCOUNT ON, par exemple) ne s'appliquent qu'à l'exécution de la commande actuelle. L'état de la connexion n'est pas modifié pour les commandes suivantes qui sont exécutées pendant que la connexion est ouverte.  
...
```  
 ...
 Cette modification peut affecter la compatibilité ascendante et descendante de votre application si vous dépendez du comportement <xref:System.Data.SqlClient.SqlCommand.ExecuteReader%2A> de pour les deux versions de la .NET Framework.  
