...
## <a name="sample-details"></a>Détails de l'exemple
 ...
 `MyWhile`diffère de `MySequence` ce qu’il <xref:System.Activities.Activity> programme un seul objet à <xref:System.Activities.Activity%601> plusieurs reprises,\> et `Condition` en ce qu’il utilise un<bool nommé pour déterminer si cette planification devrait se produire. Comme `MySequence`, `MyWhile` utilise une méthode `InternalExecute` pour centraliser sa logique de planification. Il programme `Condition` <xref:System.Activities.Activity> \> le bool<avec <xref:System.Activities.CompletionCallback%601> \<un bool> `OnEvaluationCompleted`nommé . Lorsque l'exécution de `Condition` est effectuée, son résultat devient disponible via ce <xref:System.Activities.CompletionCallback> dans un paramètre fortement typé nommé `result`. Si la valeur est `true`, `MyWhile` appelle  <xref:System.Activities.NativeActivityContext.ScheduleActivity%2A>, en passant l'objet `Body`<xref:System.Activities.Activity> et `InternalExecute` comme <xref:System.Activities.CompletionCallback>. Lorsque l'exécution de `Body` est terminée, `Condition` est de nouveau planifié dans `InternalExecute`, en recommençant la boucle à zéro. Lorsque le `Condition` retourne la valeur `false`, une instance de `MyWhile` rend le contrôle à l'exécution sans planifier le `Body` et l'exécution le déplace dans l'état <xref:System.Activities.ActivityInstanceState.Closed>.

#### <a name="to-set-up-build-and-run-the-sample"></a>Pour configurer, générer et exécuter l'exemple

1. Ouvrez la solution d’échantillon Composite.sln dans Visual Studio 2010.
