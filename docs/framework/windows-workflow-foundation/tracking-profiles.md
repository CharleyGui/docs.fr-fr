...
Les profils de suivi se manifestent sous forme d’éléments XML dans un fichier de configuration cadre .NET standard ou spécifiés dans le code. L'exemple suivant illustre un modèle de suivi [!INCLUDE[netfx_current_long](../../../includes/netfx-current-long-md.md)] dans un fichier de configuration qui permet à un participant au suivi de s'abonner aux événements de flux de travail `Started` et `Completed`.
...
Supposons, par exemple, que *MyActivity* soit une activité composite dont la mise en œuvre contient deux activités, *Activité1* et *Activité2*. Lorsque cette activité est ajoutée à un flux de travail `implementationVisibility` et `RootScope`le suivi est activé avec un profil de suivi avec défini à , les enregistrements de suivi sont émis uniquement pour *MyActivity*. Cependant, aucun enregistrement n’est émis pour les activités *Activité1* et *Activité2*.

Toutefois, si `implementationVisibility` l’attribut pour le `All`profil de suivi est défini, puis les dossiers de suivi sont émis non seulement pour *MyActivity*, mais aussi pour les activités *Activity1* et *Activity2*.
...
- Objet <xref:System.Activities.Tracking.ActivityStateQuery> - Permet de suivre les changements dans le cycle de vie des activités qui composent une instance de workflow. Par exemple, vous pouvez garder une trace de chaque fois que l’activité « Envoyer un e-mail » se termine dans une instance de flux de travail. Cette requête est nécessaire pour qu'un objet <xref:System.Activities.Tracking.TrackingParticipant> s'abonne à des objets <xref:System.Activities.Tracking.ActivityStateRecord>. Les états disponibles auxquels s'abonner sont spécifiés dans l'objet <xref:System.Activities.Tracking.ActivityStates>.
...
- Objet <xref:System.Activities.Tracking.CancelRequestedQuery> - Permet de suivre les demandes d'annulation d'une activité enfant par l'activité parente. La requête est nécessaire pour qu'un objet <xref:System.Activities.Tracking.TrackingParticipant> s'abonne à des objets <xref:System.Activities.Tracking.CancelRequestedRecord>.
  ...
  La configuration et le code utilisés pour <xref:System.Activities.Tracking.CancelRequestedQuery> s’abonner aux enregistrements liés à l’annulation d’activité à l’aide sont indiqués dans l’exemple suivant.
...
Grâce aux annotations, vous pouvez baliser arbitrairement des enregistrements de suivi avec une valeur configurable après la génération. Par exemple, vous pouvez vouloir que plusieurs enregistrements de suivi sur plusieurs flux de travail soient étiquetés avec "Mail Server" -"Mail Server1". Lors de l’interrogation ultérieure d’enregistrements de suivi, cela facilite la recherche de tous les enregistrements ayant cette étiquette.
...
Les éléments de requête de suivi sont utilisés pour créer [!INCLUDE[netfx_current_long](../../../includes/netfx-current-long-md.md)] un profil de suivi à l’aide d’un fichier de configuration XML ou d’un code. Voici un exemple de modèle de suivi créé à l'aide d'un fichier de configuration :
...
Un profil configuré en tant que fichier de configuration XML est appliqué à un participant au suivi à l’aide d’une extension de comportement. Ceci est ajouté à un WorkflowServiceHost tel que décrit dans la section ultérieure [Configuring Tracking for a Workflow](configuring-tracking-for-a-workflow.md).
