...
ms.locfileid: "79182974"
---
# <a name="windows-workflow-foundation-4-performance"></a>Performances de Windows Workflow Foundation 4

 .NET Framework 4 comprend une révision majeure de la Windows Workflow Foundation (WF) avec de lourds investissements dans la performance. Cette nouvelle révision introduit des changements de [!INCLUDE[wf1](../../../includes/wf1-md.md)] conception importants par rapport aux versions précédentes de celle expédiée dans le cadre de .NET Framework 3.0 et .NET Framework 3.5. Elle a été remodelée à partir du modèle de programmation, du runtime et des outils de base afin d'améliorer considérablement ses performances et sa facilité d'utilisation. Cette rubrique décrit les caractéristiques de performances importantes de ces révisions et les compare avec celles de la version précédente.

 Les performances des composants de workflow individuels ont augmenté considérablement entre WF3 et WF4.  Cela laisse l’écart entre les services de la Windows Communication Foundation (WCF) codés à la main et les services de flux de travail WCF pour être assez faible.  La latence du workflow a été considérablement réduite dans WF4.  Les performances de la persistance ont augmenté d'un facteur de 2,5 à 3.  Le contrôle d'état au moyen du suivi de workflow nécessite une charge mémoire bien moindre.  Ces raisons incontestables justifient la migration vers WF4 ou son adoption dans vos applications.

## <a name="terminology"></a>Terminologie

 La version [!INCLUDE[wf1](../../../includes/wf1-md.md)] de l’introduction dans .NET Framework 4 sera appelée WF4 pour le reste de ce sujet. [!INCLUDE[wf1](../../../includes/wf1-md.md)]a été introduit dans le cadre 3.0 de net et a subi quelques révisions mineures par l’intermédiaire du cadre .NET 3.5 SP1. La version .NET Framework 3.5 de workflow Foundation sera appelée WF3 pour le reste de ce sujet. WF3 est expédié en .NET Framework 4 côte à côte avec WF4. Pour plus d’informations sur la migration des artefacts WF3 à WF4 voir: [Windows Workflow Foundation 4 Migration Guide](migration-guidance.md).

 Windows Communication Foundation (WCF) est le modèle de programmation unifié de Microsoft pour la construction d’applications axées sur le service. Il a d’abord été introduit dans le cadre de .NET 3.0 avec WF3 et est maintenant l’un des éléments clés du cadre .NET.

 Windows Server AppFabric est un jeu de technologies intégrées permettant de générer, mettre à l'échelle et gérer facilement des applications Web et composites qui s'exécutent sur les Services Internet (IIS). Il fournit des outils pour déployer, surveiller et gérer les services et les workflows. Pour plus d’informations, voir [Windows Server AppFabric 1.0](https://docs.microsoft.com/previous-versions/appfabric/ff384253(v=azure.10)).
...
### <a name="wf-runtime"></a>Runtime WF
 ...
 Par rapport à WF3, le runtime WF4 est doté d'un planificateur plus efficace. Il tire parti du même pool de thread I/O qui est utilisé pour WCF, qui est très efficace pour exécuter les éléments de travail en lots. La file d’attente interne du planificateur d’éléments de travail est optimisée pour les modèles d’utilisation les plus courants. Le runtime WF4 gère également les états d’exécution d’une manière très légère avec une synchronisation minimale et la logique de manipulation des événements, tandis que WF3 dépend de l’enregistrement des événements lourds et l’invocation pour effectuer une synchronisation complexe pour les transitions de l’État.
...
### <a name="data-storage-and-flow"></a>Stockage et flux de données
 ...
 WF4 utilise une logique claire de portée des données afin d'améliorer considérablement le traitement des données dans un workflow. Il sépare les données stockées dans une activité de celles transférées hors des limites de l’activité à l’aide de deux concepts différents : variables et arguments. En utilisant une portée hiérarchique claire pour les variables et les arguments « In/Out/InOut », la complexité de l’utilisation des données pour les activités est considérablement réduite et la durée de vie des données est également automatiquement étendue. Les activités ont une signature bien définie décrite par ses arguments. Il vous suffit d'inspecter une activité pour déterminer les données qu'elle s'attend à recevoir et celles qu'elle générera suite à son exécution.
...
### <a name="control-flow"></a>Flux de contrôle
 ...
 WF4 traite cela de façon très différente. Il utilise le modèle d'activité pour créer un nouvel objet ActivityInstance qu'il ajoute à la file d'attente du planificateur. Tout ce processus ne concerne que la création d’objets explicites et est très léger.
...
### <a name="messaging"></a>Messagerie
 Initialement, WF3 offrait une prise en charge de la messagerie limitée au moyen d'événements externes ou d'appels de services Web. En .NET 3.5, les flux de travail pourraient être mis en <xref:System.Workflow.Activities.SendActivity> <xref:System.Workflow.Activities.ReceiveActivity>œuvre en tant que clients WCF ou exposés comme services WCF à travers et . Dans WF4, le concept de programmation de messagerie basée sur le flux de travail a été renforcé grâce à l’intégration serrée de la logique de messagerie WCF dans WF.

 Le pipeline unifié de traitement des messages fourni dans WCF en .NET 4 aide les services WF4 à avoir des performances et une évolutivité nettement meilleures que WF3. WF4 fournit également une prise en charge plus riche de la programmation de messagerie qui peut modéliser des modèles d’échange de messages (MEP, message exchange patterns) complexes. Les développeurs peuvent utiliser des contrats de service typés pour faciliter la programmation ou non typés pour améliorer les performances sans coûts de sérialisation. La prise en charge de la mise en cache côté client à l'aide de la classe <xref:System.ServiceModel.Activities.SendMessageChannelCache> dans WF4 permet aux développeurs de créer des applications rapides avec un minimum d'effort. Pour plus d’informations, voir [Changer les niveaux de partage de caches pour envoyer des activités](../wcf/feature-details/changing-the-cache-sharing-levels-for-send-activities.md).

### <a name="declarative-programming"></a>Programmation déclarative
 WF4 fournit une structure de programmation déclarative nette et simple permettant de modéliser les processus et services d'entreprise. Le modèle de programmation prend en charge la composition entièrement déclarative des activités, sans code-beside, ce qui simplifie considérablement la création de workflow. Dans .NET Framework 4, le cadre de programmation déclarative basé sur XAML a été unifié en système d’assemblage unique.Xaml.dll pour soutenir À la fois WPF et WF.

 Dans WF4, XAML fournit une expérience véritablement déclarative et permet de définir l'intégralité du workflow dans le balisage XML, en référençant les activités et les types créés à l'aide de .NET. Il était difficile de le faire dans WF3 avec le format XOML sans impliquer une logique code-behind personnalisée. Le nouveau XAML-stack en .NET 4 a beaucoup mieux performance dans la sérialisation / déséialiser les artefacts flux de travail et rend la programmation déclarative plus attrayant et solide.
...
## <a name="component-level-performance-comparisons"></a>Comparaisons des performances au niveau des composants
 Cette section contient des données sur des comparaisons directes entre les activités individuelles dans les workflows de WF3 et de WF4.  Des domaines clés tels que la persistance ont un impact plus profond sur les performances que les composants d'activité individuels.  Les améliorations de performances des composants individuels dans WF4 sont toutefois importantes, car les composants sont désormais suffisamment rapides pour pouvoir être comparés avec une logique d'orchestration codée manuellement.  Un exemple de ce qui est couvert dans la section suivante: "Scénario de composition des services."
...
![Flux de travail de rémunération de base WF3 et WF4](./media/performance/basic-compensation-workflows-wf3-wf4.gif)

### <a name="performance-test-results"></a>Résultats des test de performances

 ![Tableau montrant les données sur les résultats des tests de performance](./media/performance/performance-test-data.gif)

 ![Graphique de colonne comparant les données des tests de performance WF3 et WF4](./media/performance/performance-test-chart.gif)
...
## <a name="service-composition-scenario"></a>Scénario de composition de service
 Comme le montre la section précédente, « Comparaisons du rendement au niveau des composantes », il y a eu une réduction significative des frais généraux entre WF3 et WF4.  Les services de flux de travail WCF peuvent maintenant presque correspondre aux performances des [!INCLUDE[wf1](../../../includes/wf1-md.md)] services WCF codés à la main, mais ont encore tous les avantages de l’exécution.  Ce scénario de test compare un service WCF à un service de flux de travail WCF dans WF4.

### <a name="online-store-service"></a>Service de magasin en ligne
 L’un des points forts de Windows Workflow Foundation est la capacité de composer des processus à l’aide de plusieurs services.  Dans cet exemple, un service de magasin en ligne organise deux appels de service pour passer une commande.  La première étape consiste à valider la commande par le biais d'un service de validation des commandes.  La seconde étape consiste à satisfaire la commande à l'aide d'un service d'entrepôt .

 Les deux services backend, de validation des commandes et d'entrepôt, sont les mêmes pour les deux tests.  La partie qui change est le service de magasin en ligne qui effectue l'orchestration.  Dans un cas, le service est codé à la main comme un service WCF.  Pour l’autre cas, le service est écrit comme un service de flux de travail WCF dans WF4. Les fonctionnalités spécifiques à [!INCLUDE[wf1](../../../includes/wf1-md.md)], telles que le suivi et la persistance, sont désactivées pour ce test.
...
![Configuration de l’environnement pour la mesure des performances](./media/performance/performance-test-environment.gif)
 ...
 Les demandes du client sont transmises au service de magasin en ligne via HTTP à partir de plusieurs ordinateurs.  Un ordinateur unique héberge les trois services.  La couche de transport entre le service de magasin en ligne et les services backend est TCP ou HTTP.  La mesure des opérations/seconde est basée sur le nombre d'appels `PurchaseOrder` passés au service de magasin en ligne.  Le regroupement de canaux est une nouvelle fonctionnalité disponible dans WF4.  Dans la partie WCF de cette mise en commun de canaux d’essai n’est pas fourni hors de la boîte de sorte qu’une mise en œuvre codée à la main d’une technique de mise en commun simple a été utilisée dans le service de magasin en ligne.

### <a name="performance"></a>Performances
![Graphique de colonne indiquant les performances du service de magasin en ligne](./media/performance/online-store-performance-graph.gif)
...
#### <a name="workflow-activation-latency"></a>Latence de l'activation de workflow
 Dans une application de service de flux de travail WCF, la latence pour le démarrage d’un nouveau flux de travail ou le chargement d’un flux de travail existant est importante car elle peut être le blocage.  Ce cas de test compare un hôte XOML WF3 à un hôte XAMLX WF4 dans un scénario classique.
...
##### <a name="test-setup"></a>Configuration du test
 ...
 ![Flux de travail WF4 Correlation Scope](./media/performance/wf4-correlationscope-workflow.gif)

 L'activité <xref:System.ServiceModel.Activities.Receive> crée l'instance de workflow.  Une valeur passée dans le message reçu est répercutée dans le message de réponse.  Une séquence qui suit la réponse contient le reste du workflow.  Dans le cas ci-dessus, un seule activité de commentaire est représentée.  Le nombre d'activités de commentaire est modifié pour simuler la complexité de workflow.  Une activité de commentaire est équivalente à un <xref:System.Workflow.Activities.CodeActivity> WF3 qui n'effectue aucune tâche. Pour plus d’informations sur l’activité de commentaire, voir la section « Comparaison des performances au niveau des composants » plus tôt dans cet article.

##### <a name="test-results"></a>Résultats du test

 Latence froide et chaude pour les services de flux de travail WCF:

 ![Graphique de colonne montrant la latence froide et chaude pour les services de workflow WCF utilisant WF3 et WF4](./media/performance/latency-results-graph.gif)

 Dans le tableau précédent, le froid fait référence <xref:System.ServiceModel.WorkflowServiceHost> au cas où il n’existe pas de flux de travail donné.  En d'autres termes, la latence à froid correspond au cas dans lequel le workflow est utilisé pour la première fois et le XOML ou le XAML doit être compilé.  La latence à chaud correspond à la création d'une nouvelle instance de workflow lorsque le type de workflow a déjà été compilé.  La complexité du workflow a très peu d'impact dans le cas de WF4, mais a une progression linéaire dans le cas de WF3.

#### <a name="correlation-throughput"></a>Débit de corrélation
 WF4 introduit une nouvelle fonctionnalité de corrélation basée sur le contenu.  WF3 offrait seulement la corrélation basée sur le contexte.  La corrélation contextuelle ne pouvait être effectuée que sur des liaisons spécifiques des canaux WCF.  L’ID de workflow est inséré dans l’en-tête de message lors de l’utilisation de ces liaisons.  Le runtime WF3 ne pouvait identifier un flux de travail que par son Id.  Avec la corrélation basée sur le contenu, l’auteur du flux de travail peut créer une clé de corrélation à partir d’un élément pertinent de données comme un numéro de compte ou id client.
...
#### <a name="environment-setup"></a>Configuration de l’environnement
![Configuration de l’environnement pour le test de performance de flux de travail](./media/performance/performance-test-environment.gif)

#### <a name="test-setup"></a>Configuration du test
![Test du workflow de débit de corrélation](./media/performance/correlation-throughput-workflow.gif "Configuration de test de flux de travail de débit de corrélation")

 Le flux de travail précédent est le même que celui utilisé dans la section [Persistance.](#persistence) Pour les tests de corrélation sans persistance, il n’y a pas de fournisseur de persistance installé dans le temps d’exécution. La corrélation se produit en deux endroits : CreateOrder et CompleteOrder.
...
#### <a name="correlation-with-persistence"></a>Corrélation et persistance.
 ...
 ![Graphique de ligne montrant des résultats de corrélation et de persistance](./media/performance/correlation-persistence-graph.gif)
...
### <a name="test-setup"></a>Configuration du test
 ...
 ![Équation pour calculer le nombre d’activités WF3](./media/performance/wf3-number-activities-equation.gif)
 ...
 ![Un tableau qui montre le nombre d’activités utilisées dans chaque test](./media/performance/workflow-variation-compare-table.gif)
 ...
 ![Flowchart du flux de travail complexe de débit](./media/performance/complex-workflow-throughput-workflow.gif)
 ...
 ![Extrait de Flowchart du flux de travail WF3](./media/performance/wf3-workflow-snippet.gif)
 ...
 ![Flowchart d’une séquence imbriquée](./media/performance/nested-sequence-workflow.gif)

 Le suivi et la persistance ne sont pas utilisés dans ce test.

### <a name="test-results"></a>Résultats du test
 ![Graphique de colonne affichant les résultats de performance de débit](./media/performance/throughput-performance-results.gif)
...
### <a name="memory"></a>Mémoire
 La charge mémoire de Windows Workflow Foundation est mesurée dans deux domaines clés : complexité de workflow et nombre de définitions de workflow.  Les mesures de mémoire proviennent d'une station de travail Windows 7 64 bits.  Il existe de nombreuses façons d’obtenir la mesure de la taille de l’ensemble de travail tels que la surveillance des compteurs de performance, le sondage Environment.WorkingSet, ou l’utilisation d’un outil comme VMMap disponible à partir de [VMMap](/sysinternals/downloads/vmmap). Une combinaison de méthodes a été utilisée pour obtenir et vérifier les résultats de chaque test.
...
### <a name="workflow-complexity-test"></a>Test de complexité de workflow
 ...
 ![Flux de travail complexes pour WF3 et WF4](./media/performance/complex-workflow-wf3-wf4.gif)
 ...
 ![Graphique de colonne montrant l’utilisation complexe de mémoire de flux de travail pour les flux de travail WF3 et WF4](./media/performance/complex-memory-usage-wf3-wf4.gif)
...
L’image suivante montre un flux de travail WF3 avec ReceiveActivity et un flux de travail WF4 avec modèle de demande/réponse :

 ![Services de flux de travail dans WF3 et WF4](./media/performance/workflow-receive-activity.gif)

 Le tableau suivant montre le delta dans l’ensemble de travail entre une définition de flux de travail unique et 1001 définitions:
...
|Services de workflow hébergés par IIS|446 Mo|364 MB|
 ...
 L’hébergement des définitions de flux de travail <xref:System.ServiceModel.WorkflowServiceHost>dans l’IIS consomme beaucoup plus de mémoire en raison des artefacts de service WCF, et de la logique de traitement des messages associée à l’hôte.
...
### <a name="test-setup"></a>Configuration du test
 ...
 ![Flux de travail de persistance dans WF3 sur la gauche et WF4 sur la droite](./media/performance/persist-workflow-wf3-wf4.gif)

 Les deux workflows sont créés par un message reçu.  Une fois la réponse initiale envoyée, le workflow est persistant.  Dans le cas de WF3, une <xref:System.Workflow.ComponentModel.TransactionScopeActivity> vide est utilisée pour initier la persistance.  La même chose pourrait être réalisée dans WF3 en marquant une activité comme «persister sur la clôture."  Un second message corrélé termine le workflow.  Les workflows sont persistants mais pas déchargés.

### <a name="test-results"></a>Résultats du test
 ![Graphique de colonne affichant la persistance de débit](./media/performance/throughput-persistence-graph.gif)
...
- PhysicalDisk\\%Disk Lire l’heure

- PhysicalDisk\\% Temps de disque

- PhysicalDisk\\% Disk Écrire le temps

- PhysicalDisk\\% Avg. Longueur de file d’attente de disque

- PhysicalDisk-Avg. Disque Lire la longueur de file d’attente

- PhysicalDisk-Avg. Disque Écrire la longueur de file d’attente

- PhysicalDisk\Longueur actuelle de la file d'attente du disque

- Processeur\\Information % Temps de processeur
...
### <a name="tracking"></a>Suivi
 ...
 WF4 n'est pas fourni avec un service de suivi SQL.  L’enregistrement des informations de suivi à une base de données SQL peut être mieux géré à partir d’un serveur d’application plutôt que intégré dans le cadre .NET. Par conséquent, le suivi SQL est désormais géré par AppFabric.  Le fournisseur de suivi prédéfini de WF4 est basé sur ETW (suivi d'événements Windows).
...
- Les événements de suivi ETW sont facilement combinés avec les événements WCF ETW ou d’autres fournisseurs d’ETW tels qu’un serveur SQL ou un fournisseur de noyau.
...
- Un administrateur peut activer ou désactiver le suivi sans recycler le processus hôte.
 ...
 Les événements ETW peuvent être enregistrés à l’aide d’outils tels que logman ou xperf.  Le fichier ETL compact peut être affiché avec un outil tel que xperfview ou converti dans un format plus lisible, tel que XML, à l’aide de tracerpt.  Dans WF3, la seule option permettant d'obtenir des événements de suivi sans base de données SQL consiste à créer un service de suivi personnalisé. Pour plus d’informations sur ETW, voir [WCF Services et Event Tracing for Windows](../wcf/samples/wcf-services-and-event-tracing-for-windows.md) and Event Tracing - Applications [Windows](/windows/desktop/etw/event-tracing-portal).
...
Le tableau suivant montre les résultats de l’exécution d’un flux de travail contenant cinq activités dans une séquence dans diverses configurations.
