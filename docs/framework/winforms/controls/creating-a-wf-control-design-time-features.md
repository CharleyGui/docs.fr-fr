...
Cette procédure pas à pas montre comment créer un concepteur personnalisé pour un contrôle personnalisé. Vous allez implémenter `MarqueeControl` un type et une classe de concepteur associée `MarqueeControlRootDesigner`, appelée.

Le `MarqueeControl` type implémente un affichage similaire à une sélection de théâtre, avec des lumières animées et du texte clignotant.
...
- Référencement du projet de contrôle personnalisé
...
- Gestion des modifications de composants
...
Une fois que vous avez terminé, votre contrôle personnalisé ressemble à ce qui suit:
...
Ouvrez Visual Studio et créez un projet d’application Windows Forms nommé «MarqueeControlTest» (**fichier** > **nouveau** > **projet** > **Visual C#**  ou **Visual Basic**  >  **Bureau classique** **Windows Forms application).**  > 
...
### <a name="to-create-the-control-library-project"></a>Pour créer le projet de bibliothèque de contrôles
...
6. Vous devrez utiliser des types de l’assembly System. Design. Ajoutez cette référence au `MarqueeControlLibrary` projet.

    > [!NOTE]
    > Pour utiliser l’assembly System. Design, votre projet doit cibler la version complète du .NET Framework, et non le profil client .NET Framework. Pour modifier la version cible de .NET [Framework, consultez Procédure: Cibler une version du .NET Framework](/visualstudio/ide/how-to-target-a-version-of-the-dotnet-framework).

## <a name="referencing-the-custom-control-project"></a>Référencement du projet de contrôle personnalisé
...
## <a name="defining-a-custom-control-and-its-custom-designer"></a>Définition d’un contrôle personnalisé et de son concepteur personnalisé
...
4. Modifiez la déclaration de `MarqueeControlRootDesigner` pour qu’elle hérite de la <xref:System.Windows.Forms.Design.DocumentDesigner> classe. Appliquez pour spécifier l’interaction du concepteur avec la **boîte à outils.** <xref:System.ComponentModel.ToolboxItemFilterAttribute>
     ...
     **Remarque** La définition de la `MarqueeControlRootDesigner` classe a été placée dans un espace de noms appelé «MarqueeControlLibrary. Design». Cette déclaration place le concepteur dans un espace de noms spécial réservé aux types liés à la conception.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#530](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/marqueecontrolrootdesigner.cs#530)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#530](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/marqueecontrolrootdesigner.vb#530)]

5. Définissez le constructeur pour la `MarqueeControlRootDesigner` classe. Insérez une <xref:System.Diagnostics.Trace.WriteLine%2A> instruction dans le corps du constructeur. Cela est utile à des fins de débogage.
...
## <a name="creating-an-instance-of-your-custom-control"></a>Création d’une instance de votre contrôle personnalisé
 Pour observer le comportement personnalisé au moment du design de votre contrôle, vous allez placer une instance de votre contrôle sur le formulaire `MarqueeControlTest` dans Project.
...
1. Modifiez la déclaration de `DemoMarqueeControl` pour qu’elle hérite de la `MarqueeControl` classe.

2. Générez le projet.

3. Ouvrez `Form1` dans le Concepteur Windows Forms.

4. Recherchez l’onglet **Composants MarqueeControlTest** dans la **boîte à outils** et ouvrez-le. Faites glisser `DemoMarqueeControl` un de la **boîte à outils** vers votre formulaire.
...
5. Cliquez avec le bouton `MarqueeControlLibrary` droit sur le projet et sélectionnez «Définir comme projet de démarrage» pour activer cette configuration de débogage.
...
1. Ouvrez le `MarqueeControlRootDesigner` fichier source dans l' **éditeur de code** et placez un point d' <xref:System.Diagnostics.Trace.WriteLine%2A> arrêt sur l’instruction.

2. Appuyez sur F5 pour démarrer la session de débogage. Notez qu’une nouvelle instance de Visual Studio est créée.

3. Dans la nouvelle instance de Visual Studio, ouvrez la solution «MarqueeControlTest». Vous pouvez facilement trouver la solution en sélectionnant **projets récents** dans le menu **fichier** . Le fichier solution «MarqueeControlTest. sln» s’affiche comme le fichier le plus récemment utilisé.

4. `DemoMarqueeControl` Ouvrez dans le concepteur. Notez que l’instance de débogage de Visual Studio acquiert le focus et l’exécution s’arrête à votre point d’arrêt. Appuyez sur F5 pour continuer la session de débogage.

À ce stade, tout est en place pour vous permettant de développer et de déboguer votre contrôle personnalisé et son concepteur personnalisé associé. Le reste de cette procédure pas à pas se concentrera sur les détails de l’implémentation des fonctionnalités du contrôle et du concepteur.
...
6. Dans le fenêtre Propriétés, définissez les <xref:System.ComponentModel.BackgroundWorker> propriétés et `WorkerReportsProgress` <xref:System.ComponentModel.BackgroundWorker.WorkerSupportsCancellation%2A> du composant sur `true`. Ces paramètres permettent au <xref:System.ComponentModel.BackgroundWorker> composant de déclencher périodiquement <xref:System.ComponentModel.BackgroundWorker.ProgressChanged> l’événement et d’annuler les mises à jour asynchrones. Pour plus d’informations, consultez [composant BackgroundWorker](backgroundworker-component.md).
...
11. Implémentez les accesseurs de propriété. Vous allez exposer deux propriétés aux clients: `LightColor` et `DarkColor`. Les <xref:System.ComponentModel.CategoryAttribute.Category%2A> attributs <xref:System.ComponentModel.BrowsableAttribute.Browsable%2A> et sont appliqués à ces propriétés, de sorte que les propriétés s’affichent dans une section personnalisée du fenêtre Propriétés appelée «texte défilant».
...
4. Dans le Fenêtre Propriétés, cliquez sur le bouton événements. Attachez des gestionnaires pour <xref:System.ComponentModel.BackgroundWorker.DoWork> les <xref:System.ComponentModel.BackgroundWorker.ProgressChanged> événements et.
...
## <a name="creating-a-custom-designer-to-shadow-and-filter-properties"></a>Création d’un concepteur personnalisé pour masquer et filtrer les propriétés

La `MarqueeControlRootDesigner` classe fournit l’implémentation pour le concepteur racine. En plus de ce concepteur, qui s’exécute sur `MarqueeControl`le, vous avez besoin d’un concepteur personnalisé qui est spécifiquement associé `MarqueeBorder` au contrôle. Ce concepteur fournit un comportement personnalisé approprié dans le contexte du concepteur racine personnalisé.
...
Lors de la modification de l’interface publique d’un composant à l’aide de ces méthodes, vous devez suivre les règles suivantes:
...
2. Ouvrez le `MarqueeBorderDesigner` fichier source dans l' **éditeur de code**. En haut du fichier, importez les espaces de noms suivants:
...
## <a name="handling-component-changes"></a>Gestion des modifications de composants
...
Vous allez ajouter deux verbes de concepteur à vos concepteurs: **Exécuter le test** et **arrêter le test**. Ces verbes vous permettent d’afficher le comportement au moment de l' `MarqueeControl` exécution de au moment du Design. Ces verbes seront ajoutés au `MarqueeControlRootDesigner`.
...
1. Ajoutez un nouvel <xref:System.Windows.Forms.UserControl> élément `MarqueeControlLibrary` au projet. Donnez au nouveau fichier source le nom de base «LightShapeSelectionControl».
...
1. Déclarez `MarqueeLightShape` une variable d' `lightShapeValue`instance appelée.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#330](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#330)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#330](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#330)]

2. Dans le `LightShapeSelectionControl` constructeur, attachez <xref:System.Windows.Forms.Control.Click> les gestionnaires `squarePanel` `circlePanel` d’événements aux événements et aux contrôles. <xref:System.Windows.Forms.Control.Click> Définissez également une surcharge de constructeur qui assigne la `MarqueeLightShape` valeur de l’environnement de conception `lightShapeValue` au champ.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#340](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#340)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#340](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#340)]

3. Dans la <xref:System.ComponentModel.Component.Dispose%2A> méthode, détachez les <xref:System.Windows.Forms.Control.Click> gestionnaires d’événements.

     [!code-csharp[System.Windows.Forms.Design.DocumentDesigner#350](~/samples/snippets/csharp/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/CS/lightshapeselectioncontrol.cs#350)]
     [!code-vb[System.Windows.Forms.Design.DocumentDesigner#350](~/samples/snippets/visualbasic/VS_Snippets_Winforms/System.Windows.Forms.Design.DocumentDesigner/VB/lightshapeselectioncontrol.vb#350)]

4. Dans l’**Explorateur de solutions**, cliquez sur le bouton **Afficher tous les fichiers**. Ouvrez le fichier LightShapeSelectionControl.Designer.cs ou LightShapeSelectionControl. Designer. vb, puis supprimez la définition par défaut <xref:System.ComponentModel.Component.Dispose%2A> de la méthode.
...
6. Remplacez la méthode <xref:System.Windows.Forms.Control.OnPaint%2A> . Cette implémentation dessinera un carré rempli et un cercle. Il met également en surbrillance la valeur sélectionnée en dessinant une bordure autour d’une forme ou de l’autre.
...
1. Définissez le `MarqueeControlTest` projet en tant que projet de démarrage, puis exécutez-le. Vous verrez le formulaire qui affiche `DemoMarqueeControl`votre. Cliquez sur le bouton **Démarrer** pour démarrer l’animation. Vous devez voir le texte clignotant et les lumières qui se déplacent dans la bordure.
