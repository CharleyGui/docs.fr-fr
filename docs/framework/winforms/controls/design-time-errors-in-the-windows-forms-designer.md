...
### <a name="identifier-name-is-not-a-valid-identifier"></a>'\<identificateur name > 'n’est pas un identificateur valide
...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>La\<boîte à outils nom de l’onglet > n’est pas une catégorie de boîte à outils
...
Message d’erreur : «Un analyseur de langage demandé n’est pas installé. Le nom de l’analyseur de{0}langage est «».
...
Message d’erreur : «Un autre éditeur a\<'nom du document > 'ouvert dans un mode incompatible. Fermez l’éditeur et recommencez l’opération.»
...
Ce message est similaire à « un autre éditeur a\<'nom du document > 'ouvert dans un mode incompatible », mais Visual Studio ne peut pas déterminer le nom du fichier. Pour corriger cette erreur, fermez l’éditeur où le fichier est ouvert, puis réessayez.

### <a name="array-rank-rank-in-array-is-too-high"></a>Le rang du\<tableau’Rank dans le tableau > 'est trop élevé
...
Message d’erreur : Impossible d’ouvrir\<« assembly’assembly name > ». Vérifiez que le fichier existe toujours.»
...
### <a name="cannot-bind-an-event-handler-to-the-event-name-event-because-it-is-read-only"></a>Impossible de lier un gestionnaire d’événements à\<l’événement’Event name > ', car il est en lecture seule
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>La catégorie\<« nom de l’onglet Boîte à outils > » n’a pas\<d’outil pour la classe « nom de la classe > »
...
### <a name="code-generation-for-property-property-name-failed"></a>Échec de la génération de\<code pour la propriété’nom de propriété > '
...
Pour corriger l’erreur, contactez le fournisseur du composant. Ou, s’il s’agit d’un composant que vous avez `IContainer.Add` créé, appelez la méthode dans le constructeur du composant.
...
### <a name="could-not-find-type-type-name"></a>Impossible de trouver le type\<'nom de type > '

Message d’erreur : «Le type'\<nom de type > 'est introuvable. Assurez-vous que l’assembly qui contient ce type est référencé. Si ce type fait partie de votre projet de développement, assurez-vous que le projet a été généré avec succès.»
...
### <a name="could-not-load-type-type-name"></a>Impossible de charger le type\<'nom de type > '

Message d’erreur : «Impossible de charger le type\<'nom de type > '. Assurez-vous que l’assembly contenant ce type est ajouté aux références du projet.»
...
Cette erreur se produit parce qu’une variable membre a été déclarée deux fois (par exemple `Button1` , deux contrôles nommés sont déclarés dans le code). Les noms doivent être uniques dans les formulaires hérités. En outre, les noms ne peuvent pas différer uniquement par la casse.
...
### <a name="failed-to-parse-method-method-name"></a>Impossible d’analyser la méthode\<'nom de la méthode > '

Message d’erreur : «Impossible d’analyser la méthode\<'nom de la méthode > '. L’analyseur a signalé l’erreur suivante :\<'error String > '. Recherchez les erreurs potentielles dans le Liste des tâches.»
...
Lorsque vous définissez une classe dans plusieurs fichiers à l’aide du mot clé [Partial](/dotnet/csharp/language-reference/keywords/partial-type) , vous ne pouvez avoir qu’une seule définition partielle dans chaque fichier.
...
Message d’erreur : «L’assembly\<'assembly' > 'n’a pas pu être trouvé. Assurez-vous que l’assembly est référencé. Si l’assembly fait partie du projet de développement actuel, assurez-vous que le projet a été généré.»

Cette erreur est semblable à « le type'\<nom du type > 'est introuvable », mais cette erreur se produit généralement en raison d’un attribut de métadonnées. Pour corriger cette erreur, vérifiez que tous les assemblys utilisés par les attributs sont référencés.

### <a name="the-assembly-name-assembly-name-is-invalid"></a>Le nom d’assembly'\<assembly name > 'n’est pas valide
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Impossible d’ajouter le composant, car il contient une référence circulaire à\<« nom de la référence > »
...
Cette erreur s’est produite parce que la classe de base de l’objet passé au concepteur est [abstraite](/dotnet/csharp/language-reference/keywords/abstract), ce qui n’est pas autorisé.
...
Message d’erreur : «Le langage de ce fichier ne prend pas en charge les services d’analyse et de génération de code nécessaires. Vérifiez que le fichier que vous ouvrez est membre d’un projet, puis réessayez d’ouvrir le fichier.»
...
Message d’erreur : «La classe de l’analyseur\<de langage’nom de la classe > 'n’est pas implémentée correctement. Contactez le fournisseur pour obtenir un module d’analyseur mis à jour.»
...
Visual Studio a essayé de créer un composant, mais l’objet créé n’implémente <xref:System.ComponentModel.IComponent> pas l’interface. Contactez le fournisseur du composant pour obtenir un correctif.

### <a name="the-object-object-name-returned-null-for-the-property-property-name-but-this-is-not-allowed"></a>L’objet «\<nom d’objet > » a retourné la valeur null\<pour la propriété « nom de la propriété > », mais cela n’est pas autorisé
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Le service'\<nom du service > 'est requis, mais il est introuvable

Message d’erreur : «Le service'\<nom du service > 'est requis, mais il est introuvable. Il se peut qu’il y ait un problème avec votre installation de Visual Studio.»
...
Message d’erreur : «Impossible de récupérer l’élément\<de boîte à outils pour’Component name > 'à partir de la boîte à outils. Assurez-vous que l’assembly qui contient l’élément de boîte à outils est correctement installé. L’élément de boîte à outils a généré \<l’erreur suivante : chaîne d’erreur >.»
...
Message d’erreur : « Impossible de récupérer l’élément\<de boîte à outils pour «nom d’élément de boîte à outils > » à partir de la boîte à outils. Essayez de supprimer l’élément de la boîte à outils et de le rajouter.»
...
Message d’erreur : «Le type'\<nom de type > 'est introuvable. Assurez-vous que l’assembly contenant le type est référencé. Si l’assembly fait partie du projet de développement actuel, assurez-vous que le projet a été généré.»
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Il existe déjà un gestionnaire de commandes pour la commande de\<menu’menu’nom de la commande > '
...
Message d’erreur : «Un composant nommé'\<Component name > 'existe déjà. Les composants doivent avoir des noms uniques et les noms ne doivent pas être sensibles à la casse. Un nom ne peut pas non plus être en conflit avec le nom d’un composant dans une classe héritée.»
...
### <a name="there-is-already-a-toolbox-item-creator-registered-for-the-format-format-name"></a>Un créateur d’élément de boîte à outils est déjà inscrit au\<format « nom de format > »
...
### <a name="type-type-name-does-not-have-a-constructor-with-parameters-of-types-parameter-type-names"></a>Le type\<'nom\>de type’n’a pas de constructeur avec des paramètres\<de types’noms de type de paramètre > '

Visual Studio n’a pas pu trouver un [constructeur](/dotnet/csharp/programming-guide/classes-and-structs/constructors) qui avait des paramètres correspondants. Cela peut être le résultat de la fourniture d’un constructeur avec des types autres que ceux qui sont requis. Par exemple, un constructeur **point** peut prendre deux entiers. Si vous avez fourni des valeurs float, cette erreur est générée.
...
### <a name="unable-to-add-reference-reference-name-to-the-current-application"></a>Impossible d’ajouter la référence\<« nom de la référence > » à l’application actuelle

Message d’erreur : « Impossible d’ajouter\<le nom de référence > » à l’application actuelle. Vérifiez qu’une version différente de'\<nom de la référence > 'n’est pas déjà référencée. "
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Impossible de trouver la page nommée\<« options nom de l’onglet de la boîte de dialogue > »
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Impossible de trouver la propriété\<« nom de la propriété > »\<dans la page « Options nom de la boîte de dialogue > »
...
### <a name="visual-studio-cannot-save-or-load-instances-of-the-type-type-name"></a>Visual Studio ne peut pas enregistrer ou charger les instances du\<type’nom de type > '
