...
> - Pour les applications C++, les erreurs n’ont pas de liens d’emplacement du code.
...
### <a name="identifier-name-is-not-a-valid-identifier"></a>' \<identifier name> 'n’est pas un identificateur valide

Cette erreur indique que le nom d’un champ, d’une méthode, d’un événement ou d’un objet est incorrect.

### <a name="name-already-exists-in-project-name"></a>' \<name> 'existe déjà dans' \<project name> '

Message d’erreur : « ' » \<name> existe déjà dans « \<project name> ». Veuillez entrer un nom unique.»
...
### <a name="toolbox-tab-name-is-not-a-toolbox-category"></a>' \<Toolbox tab name> 'n’est pas une catégorie de boîte à outils
...
### <a name="an-exception-occurred-while-trying-to-create-an-instance-of-object-name"></a>Une exception s’est produite lors de la tentative de création d’une instance de' \<object name> '

Message d’erreur : « une exception s’est produite lors de la tentative de création d’une instance de « \<object name> ». L’exception était « \<exception string\> ».
...
### <a name="another-editor-has-document-name-open-in-an-incompatible-mode"></a>«» Est ouvert dans un autre éditeur \<document name> dans un mode incompatible

Message d’erreur : « le mode « \<document name> » est ouvert dans un autre éditeur dans un mode incompatible. Fermez l’éditeur et recommencez l’opération.»
...
### <a name="another-editor-has-made-changes-to-document-name"></a>Un autre éditeur a apporté des modifications à « \<document name> »
...
Ce message est semblable à « \<document name> l’ouverture d’un autre éditeur dans un mode incompatible », mais Visual Studio ne peut pas déterminer le nom du fichier. Pour corriger cette erreur, fermez l’éditeur où le fichier est ouvert, puis réessayez.

### <a name="array-rank-rank-in-array-is-too-high"></a>Le rang de tableau « \<rank in array> » est trop élevé
...
### <a name="assembly-assembly-name-could-not-be-opened"></a>Impossible d’ouvrir l’assembly' \<assembly name> '

Message d’erreur : «l’assembly' \<assembly name> 'n’a pas pu être ouvert. Vérifiez que le fichier existe toujours.»
...
### <a name="bad-element-type-this-serializer-expects-an-element-of-type-type-name"></a>Type d’élément incorrect. Ce sérialiseur attend un élément de type' \<type name> '
...
### <a name="cannot-bind-an-event-handler-to-the-event-name-event-because-it-is-read-only"></a>Impossible de lier un gestionnaire d’événements à l' \<event name> événement' ', car il est en lecture seule
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Impossible de nommer l’objet' \<name> ', car il est déjà nommé' \<name> '
...
### <a name="cannot-remove-or-destroy-inherited-component-component-name"></a>Impossible de supprimer ou de détruire le composant hérité' \<component name> '
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>La catégorie' \<Toolbox tab name> 'n’a pas d’outil pour la classe' \<class name> '
...
### <a name="code-generation-for-property-property-name-failed"></a>Échec de la génération du code pour la propriété' \<property name> '
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>Le composant' \<component name> 'n’a pas appelé Container. Add () dans son constructeur
...
### <a name="could-not-access-the-variable-variable-name-because-it-has-not-been-initialized-yet"></a>Impossible d’accéder à la variable « \<variable name> », car elle n’a pas encore été initialisée
...
### <a name="could-not-find-type-type-name"></a>Impossible de trouver le type' \<type name> '

Message d’erreur : «impossible de trouver le type' \<type name> '. Assurez-vous que l’assembly qui contient ce type est référencé. Si ce type fait partie de votre projet de développement, assurez-vous que le projet a été généré avec succès.»
...
### <a name="could-not-load-type-type-name"></a>Impossible de charger le type' \<type name> '

Message d’erreur : «impossible de charger le type' \<type name> '. Assurez-vous que l’assembly contenant ce type est ajouté aux références du projet.»
...
### <a name="duplicate-declaration-of-member-member-name"></a>Déclaration de membre' 'dupliquée \<member name>
...
### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Erreur lors de la lecture des ressources à partir du fichier de ressources pour la culture' \<culture name> '
...
### <a name="error-reading-resources-from-the-resource-file-for-the-default-culture-culture-name"></a>Erreur lors de la lecture des ressources à partir du fichier de ressources pour la culture par défaut' \<culture name> '
...
### <a name="failed-to-parse-method-method-name"></a>Échec de l’analyse de la méthode' \<method name> '

Message d’erreur : «impossible d’analyser la méthode' \<method name> '. L’analyseur a signalé l’erreur suivante : « \<error string> ». Recherchez les erreurs potentielles dans le Liste des tâches.»
...
### <a name="invalid-component-name-component-name"></a>Nom de composant non valide : ' \<component name> '
...
### <a name="the-assembly-assembly-name-could-not-be-found"></a>L’assembly' \<assembly name> 'est introuvable

Message d’erreur : «l’assembly' \<assembly name> 'est introuvable. Assurez-vous que l’assembly est référencé. Si l’assembly fait partie du projet de développement actuel, assurez-vous que le projet a été généré.»

Cette erreur est semblable à « le type « \<type name> » est introuvable», mais cette erreur se produit généralement en raison d’un attribut de métadonnées. Pour corriger cette erreur, vérifiez que tous les assemblys utilisés par les attributs sont référencés.

### <a name="the-assembly-name-assembly-name-is-invalid"></a>Le nom de l’assembly' \<assembly name> 'n’est pas valide
...
### <a name="the-base-class-class-name-cannot-be-designed"></a>La classe de base' \<class name> 'ne peut pas être conçue
...
### <a name="the-base-class-class-name-could-not-be-loaded"></a>Impossible de charger la classe de base' \<class name> '
...
### <a name="the-class-class-name-cannot-be-designed-in-this-version-of-visual-studio"></a>La classe' \<class name> 'ne peut pas être conçue dans cette version de Visual Studio
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Impossible d’ajouter le composant, car il contient une référence circulaire à' \<reference name> '
...
### <a name="the-designer-must-create-an-instance-of-type-type-name-but-it-cant-because-the-type-is-declared-as-abstract"></a>Le concepteur doit créer une instance de type' \<type name> ', mais il ne peut pas parce que le type est déclaré comme abstract
...
### <a name="the-language-parser-class-class-name-is-not-implemented-properly"></a>La classe de l’analyseur de langage' \<class name> 'n’est pas implémentée correctement

Message d’erreur : «la classe de l’analyseur de langage' \<class name> 'n’est pas implémentée correctement. Contactez le fournisseur pour obtenir un module d’analyseur mis à jour.»
...
### <a name="the-object-object-name-does-not-implement-the-icomponent-interface"></a>L’objet' \<object name> 'n’implémente pas l’interface IComponent

Visual Studio a essayé de créer un composant, mais l’objet créé n’implémente pas l' <xref:System.ComponentModel.IComponent> interface. Contactez le fournisseur du composant pour obtenir un correctif.

### <a name="the-object-object-name-returned-null-for-the-property-property-name-but-this-is-not-allowed"></a>L’objet' \<object name> 'a retourné null pour la propriété' \<property name> ', mais cela n’est pas autorisé
...
### <a name="the-service-service-name-is-required-but-could-not-be-located"></a>Le service « \<service name> » est obligatoire, mais il est introuvable

Message d’erreur : « le service « » \<service name> est obligatoire, mais il est introuvable. Il se peut qu’il y ait un problème avec votre installation de Visual Studio.»
...
### <a name="the-service-instance-must-derive-from-or-implement-interface-name"></a>L’instance de service doit dériver de ou implémenter' \<interface name> '
...
### <a name="the-toolbox-item-for-component-name-could-not-be-retrieved-from-the-toolbox"></a>Impossible de récupérer l’élément de boîte à outils pour' \<component name> 'à partir de la boîte à outils

Message d’erreur : «impossible de récupérer l’élément de boîte à outils pour' \<component name> 'à partir de la boîte à outils. Assurez-vous que l’assembly qui contient l’élément de boîte à outils est correctement installé. L’élément de boîte à outils a généré l’erreur suivante : \<error string> .

Le composant en question a levé une exception lorsque Visual Studio y a accédé. Contactez le fournisseur du composant.

### <a name="the-toolbox-item-for-toolbox-item-name-could-not-be-retrieved-from-the-toolbox"></a>Impossible de récupérer l’élément de boîte à outils pour' \<Toolbox item name> 'à partir de la boîte à outils

Message d’erreur : «impossible de récupérer l’élément de boîte à outils pour' \<Toolbox item name> 'à partir de la boîte à outils. Essayez de supprimer l’élément de la boîte à outils et de le rajouter.»
...
### <a name="the-type-type-name-could-not-be-found"></a>Le type « \<type name> » est introuvable

Message d’erreur : « le type « » \<type name> est introuvable. Assurez-vous que l’assembly contenant le type est référencé. Si l’assembly fait partie du projet de développement actuel, assurez-vous que le projet a été généré.»
...
### <a name="the-variable-variable-name-is-either-undeclared-or-was-never-assigned"></a>La variable' \<variable name> 'n’est pas déclarée ou n’a jamais été assignée
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Il existe déjà un gestionnaire de commandes pour la commande de menu' \<menu command name> '
...
Message d’erreur : « il existe déjà un composant nommé « \<component name> ». Les composants doivent avoir des noms uniques et les noms ne doivent pas être sensibles à la casse. Un nom ne peut pas non plus être en conflit avec le nom d’un composant dans une classe héritée.»
...
### <a name="there-is-already-a-toolbox-item-creator-registered-for-the-format-format-name"></a>Un créateur d’élément de boîte à outils est déjà inscrit pour le format' \<format name> '
...
### <a name="type-type-name-does-not-have-a-constructor-with-parameters-of-types-parameter-type-names"></a>Le type' \<type name\> 'n’a pas de constructeur avec des paramètres de types' \<parameter type names> '
...
### <a name="unable-to-add-reference-reference-name-to-the-current-application"></a>Impossible d’ajouter la référence' \<reference name> 'à l’application actuelle

Message d’erreur : «impossible d’ajouter la référence' \<reference name> 'à l’application actuelle. Vérifiez qu’une version différente de' \<reference name> 'n’est pas déjà référencée. "
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Impossible de trouver la page nommée' \<Options dialog box tab name> '
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Impossible de trouver la propriété' \<property name> 'sur la page' \<Options dialog box tab name> '
...
### <a name="visual-studio-cannot-save-or-load-instances-of-the-type-type-name"></a>Visual Studio ne peut pas enregistrer ou charger les instances du type' \<type name> '

Il s’agit d’un problème avec un composant tiers. Contactez le fournisseur du composant.

### <a name="visual-studio-is-unable-to-open-document-name-in-design-view"></a>Visual Studio ne peut pas ouvrir « \<document name> » dans mode création

Message d’erreur : « Visual Studio ne peut pas ouvrir « \<document name> » dans mode création. Aucun analyseur n’est installé pour le type de fichier.»
...
### <a name="visual-studio-was-unable-to-find-a-designer-for-classes-of-type-type-name"></a>Visual Studio n’a pas pu trouver de concepteur pour les classes de type' \<type name> '
