...
Ce sujet décrit les éléments essentiels d’une application Windows Forms et fournit un exemple simple qui utilise les contrôles et gère les événements dans une application Windows Forms.  
  
## <a name="simple-windows-forms-applications"></a>Applications simples de formulaires Windows  
 Au minimum, une application Windows Forms se compose des éléments suivants :  
  
- Une ou plusieurs classes <xref:System.Windows.Forms.Form?displayProperty=nameWithType>qui dérivent de .  
  
- Une `Main` méthode qui `static` invoque la méthode (dans`shared` Visual Basic) <xref:System.Windows.Forms.Application.Run%2A> et lui transmet une <xref:System.Windows.Forms.Form> instance. La <xref:System.Windows.Forms.Application.Run%2A> méthode traite les messages du système d’exploitation à l’application.  
...
## <a name="using-controls-in-a-windows-forms-application"></a>Utilisation de contrôles dans une application de formulaires Windows  
 L’exemple de code suivant montre une application simple qui illustre la façon dont les applications Windows Forms utilisent les contrôles et gèrent les événements. L’exemple se compose de trois boutons sur une forme; chaque bouton modifie la couleur de l’arrière-plan lorsqu’il est cliqué.  
