---
title: XpsDocumentWriter.raise__WritingProgressChanged Méthode (System.Windows.Xps)
ms.date: 12/12/2007
api_location:
- system.printing.dll
api_name:
- System.Windows.Xps.XpsDocumentWriter.raise__WritingProgressChanged
api_type:
- Assembly
topic_type:
- apiref
ms.openlocfilehash: cceabb2b4b1fad68fdda44fea8172cb21319b4bc
ms.sourcegitcommit: 7588136e355e10cbc2582f389c90c127363c02a5
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/12/2020
ms.locfileid: "79155633"
---
# <a name="xpsdocumentwriterraise__writingprogresschanged-method"></a>XpsDocumentWriter.raise\_\_WritingProgressChanged Method XpsDocumentWriter.raise WritingProgressChanged Method XpsDocumentWriter.raise WritingProgressChanged Method Xps

Déclenche l'événement <xref:System.Windows.Xps.XpsDocumentWriter.WritingProgressChanged>.

## <a name="syntax"></a>Syntaxe

```csharp
public void raise__WritingProgressChanged (object value0,
  System.Windows.Documents.Serialization.WritingProgressChangedEventArgs value1);
```

## <a name="parameters"></a>Paramètres

- `value0` <xref:System.Object>  
  Source de l’événement.

- `value1`  <xref:System.Windows.Documents.Serialization.WritingProgressChangedEventArgs>  
  Données d'événement.
  
## <a name="requirements"></a>Spécifications

**Espace nom:**<xref:System.Windows.Xps>

**Assemblée:** System.Printing (in system.printing.dll)

**.NET Versions du Cadre:** 3.0
