...
|`-Timeout` \<*secondes*>|The amount of time to allow the Windows 8.x Store app to run before the app exits.|
...
- Vous ne pouvez pas utiliser les chemins d’accès entre guillemets avec des marques de barre oblique finales, car les macros Visual Studio utilisent également des marques de barre oblique finales par défaut. (For example, `–OutDir "C:\Output Folder\"` is invalid.) To work around this restriction, you can escape the trailing slash. (Par exemple, utilisez `-OutDir "$(OutDir)\"` à la place.)  
