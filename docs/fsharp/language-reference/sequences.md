...
Une expression `if` utilisée dans une séquence est un filtre. Par exemple, pour générer une séquence de nombres premiers uniquement, en supposant que vous avez une fonction `isprime` de type `int -> bool`, construisez la séquence comme suit.
...
En utilisant [Seq. Cast](https://msdn.microsoft.com/library/1d087db3-a8b2-41dd-8ddc-227544529334), vous pouvez créer une séquence à partir d’une collection faiblement typée, telle que celles définies dans `System.Collections`. Ces collections faiblement typées ont le type d’élément `System.Object` et sont énumérées à l’aide du type de `System.Collections.Generic.IEnumerable&#96;1` non générique. Le code suivant illustre l’utilisation de `Seq.cast` pour convertir un `System.Collections.ArrayList` en séquence.
...
[Seq. Truncate](https://msdn.microsoft.com/library/1892dfeb-308e-45e2-857a-3c3405d02244) crée une séquence à partir d’une autre séquence, mais limite la séquence à un nombre spécifié d’éléments. [Seq. Take](https://msdn.microsoft.com/library/6e75f701-640b-4c4a-9d63-4313fc090596) crée une séquence qui contient uniquement un nombre spécifié d’éléments à partir du début d’une séquence. S’il y a moins d’éléments dans la séquence que celui que vous spécifiez pour prendre, `Seq.take` lève une `System.InvalidOperationException`. La différence entre `Seq.take` et `Seq.truncate` est que `Seq.truncate` ne génère pas d’erreur si le nombre d’éléments est inférieur au nombre que vous spécifiez.
...
L’exemple de code suivant illustre le comportement de et les différences entre `Seq.takeWhile`, `Seq.skip`et `Seq.skipWhile`.
...
[Seq. Windowed](https://msdn.microsoft.com/library/8b565b8f-d645-4dba-be22-099075fe4744) est semblable à `Seq.pairwise`, mais au lieu de produire une séquence de tuples, il produit une séquence de tableaux qui contiennent des copies d’éléments adjacents (une *fenêtre*) de la séquence. Vous spécifiez le nombre d’éléments adjacents que vous souhaitez dans chaque tableau.
