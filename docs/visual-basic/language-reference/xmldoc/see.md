...
## <a name="remarks"></a>Notes  
 Utilisez la `<see>` balise pour spécifier un lien à partir de texte. Utilisez [\<seealso>](seealso.md) pour indiquer le texte que vous souhaitez voir apparaître dans une section « Voir aussi ».  
...
## <a name="example"></a>Exemple  
 Cet exemple utilise la `<see>` balise dans la `UpdateRecord` section Notes pour faire référence à la `DoesRecordExist` méthode.  
