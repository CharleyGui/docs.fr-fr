...
## <a name="parts"></a>Éléments  
 `On`  
 Facultatif. Active la `Option Explicit` vérification. Si `On` ou `Off` n’est pas spécifié, la valeur par défaut est `On` .  
  
 `Off`  
 Facultatif. Désactive la `Option Explicit` vérification.  
