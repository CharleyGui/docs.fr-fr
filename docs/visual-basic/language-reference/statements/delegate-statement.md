...
|Terme|Définition|  
|---|---|  
|`attrlist`|Facultatif. Liste des attributs qui s’appliquent à ce délégué. Les attributs multiples sont séparés par des virgules. Vous devez placer la [liste des attributs](attribute-list.md) entre crochets pointus (« `<` » et «» `>` ).|  
...
|`parameterlist`|Facultatif. Liste des paramètres qui sont passés à la procédure quand elle est appelée. Vous devez placer la [liste de paramètres](parameter-list.md) entre parenthèses.|  
