...
|`statements`|Optionnel. Une ou plusieurs instructions qui suivent s' `Case` exécutent si `testexpression` correspond à une clause de `expressionlist` .|  
|`elsestatements`|Optionnel. Une ou plusieurs instructions qui suivent s' `Case Else` exécutent si `testexpression` ne correspond à aucune clause dans le `expressionlist` de l’une des `Case` instructions.|  
