...
|Terme|Définition|  
|---|---|  
|`On`|facultatif. Active `Option Strict` la vérification.|  
...
- Elle accélère l’exécution du code. Cela est dû au fait que si vous ne spécifiez pas de type de données pour un élément de programmation, le compilateur Visual Basic lui `Object` assigne le type. Le code compilé peut être obligé de basculer entre `Object` et d’autres types de données, ce qui réduit les performances.  
...
- [Char (type de données)](../../../visual-basic/language-reference/data-types/char-data-type.md)  
 ...
 Les erreurs de conversion restrictive implicite sont supprimées au moment de la compilation pour les conversions des `For Each…Next` éléments d’une collection vers la variable de contrôle de boucle. Cela se produit même `Option Strict` si est activé. Pour plus d’informations, consultez la section «conversions restrictives» dans [for each... Instruction suivante](../../../visual-basic/language-reference/statements/for-each-next-statement.md).  
...
## <a name="implicit-object-type-errors"></a>Erreurs de type d’objet implicite  
 ...
 Pour les paramètres de méthode `As` , la clause est `Option Strict` facultative si est désactivé. Toutefois, si un paramètre utilise une `As` clause, ils doivent tous l’utiliser. Si `Option Strict` est activé, la `As` clause est requise pour chaque définition de paramètre.  
  
 Si vous déclarez une variable sans utiliser `As` de clause et que vous `Nothing`lui affectez la valeur, la `Object`variable a un type de. Aucune erreur de compilation ne se produit dans ce cas `Option Strict` lorsque est activé `Option Infer` et est activé. Un exemple est `Dim something = Nothing`.  
...
## <a name="when-an-option-strict-statement-is-not-present"></a>Quand une instruction option strict n’est pas présente  
 ...
 Si vous utilisez le compilateur de ligne de commande, vous pouvez utiliser l’option du compilateur [/optionstrict](../../../visual-basic/reference/command-line-compiler/optionstrict.md) pour spécifier un paramètre `Option Strict`pour.  
...
### <a name="conditions"></a>Pour définir des configurations d’avertissement dans l’IDE  
 Lorsque vous utilisez la [page compiler du concepteur de projets (Visual Basic)](/visualstudio/ide/reference/compile-page-project-designer-visual-basic) au lieu `Option Strict` d’une instruction, vous avez un contrôle supplémentaire sur les conditions qui génèrent des erreurs. La section **configurations d’avertissement** de la **page compiler** contient des paramètres qui correspondent aux trois conditions qui provoquent une erreur au moment de `Option Strict` la compilation lorsque la valeur est on. Voici ces paramètres :  
...
## <a name="example"></a>Exemples  
 Les exemples suivants illustrent des erreurs provoquées par des variables déclarées avec un `Object`type implicite de. Cette catégorie d’erreurs correspond au **type implicite;** condition supposée par l’objet sur la **page de compilation**.  
