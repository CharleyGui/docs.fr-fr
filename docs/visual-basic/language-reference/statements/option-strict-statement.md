...
|`Off`|Optionnel. Désactive la vérification de la `Option Strict`.|  
...
## <a name="implicit-object-type-errors"></a>Erreurs de type d’objet implicite  
 ...
 Si vous déclarez une variable sans utiliser de clause `As` et que vous la définissez sur `Nothing`, la variable a un type de `Object`. Aucune erreur de compilation ne se produit dans ce cas lorsque `Option Strict` est activé et que `Option Infer` est activé. @No__t_0 en est un exemple.  
