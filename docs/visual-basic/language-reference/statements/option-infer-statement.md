...
> [!NOTE]
> Quand une variable est déclarée comme `Object`, le type au moment de l'exécution peut changer pendant que le programme s'exécute. Visual Basic effectue des opérations appelées *boxing* et *unboxing* pour effectuer `Object` une conversion entre un et un type valeur, ce qui ralentit l’exécution. Pour plus d’informations sur le boxing et l’unboxing, consultez la [spécification du langage Visual Basic](~/_vblang/spec/conversions.md#value-type-conversions).
...
|Type de données spécifié ?|Initialiseur spécifié ?|Exemples|Résultat|  
|---|---|---|---|  
