...
## <a name="remarks"></a>Notes  
 L’option `-langversion` spécifie la syntaxe acceptée par le compilateur. Par exemple, si vous spécifiez que la version du langage est 9,0, le compilateur génère des erreurs pour la syntaxe qui est valide uniquement dans la version 10,0 et les versions ultérieures.  
 ...
 Vous ne pouvez définir `-langversion` directement qu’à l’aide de la ligne de commande. Pour plus d’informations, consultez [Ciblage d’une version spécifique du .NET Framework](/visualstudio/ide/targeting-a-specific-dotnet-framework-version).  
