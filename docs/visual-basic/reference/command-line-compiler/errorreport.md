...
Le compilateur envoie des données qui incluent la pile au moment de l’erreur, qui comprend généralement du code source. Si `-errorreport` est utilisé avec l’option [-bugreport (](bugreport.md) , alors l’intégralité du fichier source est envoyée.

Cette option est utilisée de manière optimale avec l’option [-bugreport (](bugreport.md) , car elle permet aux ingénieurs Microsoft de reproduire plus facilement l’erreur.
