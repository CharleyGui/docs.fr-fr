...
- Type d’interface, tel que <xref:System.IComparable> ou <xref:System.IDisposable>  
 ...
 Vous pouvez déclarer plusieurs variables dans une instruction sans avoir à répéter le type de données. Dans les instructions suivantes, les variables `i`, `j` et `k` sont déclarées comme étant de type `Integer`, `l` et `m` comme `Long`, et `x` et `y` comme `Single` :  
...
- [Guide pratique pour Créer une variable @ no__t-0
- [Guide pratique pour Déplacer des données dans et hors d’une variable @ no__t-0
