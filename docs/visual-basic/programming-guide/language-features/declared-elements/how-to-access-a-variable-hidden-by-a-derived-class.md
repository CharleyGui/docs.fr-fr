...
Si la variable de classe dérivée occulte une variable de la classe de base, elle masque la version de la classe de base. Toutefois, vous pouvez accéder à la variable de classe de base en la qualifiant avec le mot clé `MyBase`.
...
- Dans une expression ou une instruction d’assignation, faites précéder le nom de la variable du mot clé `MyBase` et d’un point (`.`).
