...
L’exemple suivant définit une extension de `Print` pour le type de données <xref:System.String>. La méthode utilise `Console.WriteLine` pour afficher une chaîne. Le paramètre de la méthode `Print`, `aString`, établit que la méthode étend la classe <xref:System.String>.

[!code-vb[VbVbalrExtensionMethods#1](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrExtensionMethods/VB/StringExtensions.vb#1)]

Notez que la définition de la méthode d’extension est marquée avec l’attribut d’extension `<Extension()>`. Le marquage du module dans lequel la méthode est définie est facultatif, mais chaque méthode d’extension doit être marquée. les <xref:System.Runtime.CompilerServices> doivent être importés pour accéder à l’attribut d’extension.
...
Notez que lorsque les méthodes sont appelées, aucun argument n’est envoyé dans pour le premier paramètre. Les `aString` de paramètres dans les définitions de méthode précédentes sont liées à `example`, l’instance de `String` qui les appelle. Le compilateur utilise `example` comme argument envoyé au premier paramètre.
...
Étant donné que le premier paramètre spécifie le type de données étendu par la méthode d’extension, il est obligatoire et ne peut pas être facultatif. Pour cette raison, `Optional` paramètres et `ParamArray` paramètres ne peuvent pas être le premier paramètre de la liste de paramètres.

Les méthodes d’extension ne sont pas prises en compte dans la liaison tardive. Dans l’exemple suivant, l’instruction `anObject.PrintMe()` lève une exception <xref:System.MissingMemberException>, la même exception que vous pouvez voir si la deuxième définition de la méthode d’extension `PrintMe` a été supprimée.
