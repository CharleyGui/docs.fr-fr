...
- Une procédure `Set` affecte une valeur à une propriété, y compris une référence d’objet. Elle est appelée lorsque vous assignez une valeur à la propriété.  
 ...
 Vous pouvez omettre la procédure `Get` et `Set` lors de l’utilisation d’une propriété implémentée automatiquement. Pour plus d’informations, consultez [Propriétés implémentées automatiquement](./auto-implemented-properties.md).  
  
 Vous pouvez définir des propriétés dans des classes, des structures et des modules. Les propriétés sont `Public` par défaut, ce qui signifie que vous pouvez les appeler depuis n’importe où dans votre application qui peut accéder au conteneur de la propriété.  
...
- [Guide pratique pour Créer une propriété @ no__t-0
- [Guide pratique pour Appeler une procédure de propriété @ no__t-0
- [Guide pratique pour Déclarer et appeler une propriété par défaut dans Visual Basic @ no__t-0
- [Guide pratique pour Placer une valeur dans une propriété @ no__t-0
- [Guide pratique pour Obtenir une valeur à partir d’une propriété @ no__t-0
