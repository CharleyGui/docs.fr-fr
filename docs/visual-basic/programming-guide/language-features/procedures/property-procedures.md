...
You can omit the `Get` and `Set` procedure when using an auto-implemented property. Pour plus d’informations, consultez [Propriétés implémentées automatiquement](./auto-implemented-properties.md).
