...
Les <xref:System.Threading.Thread.Abort%2A?displayProperty=nameWithType> API sont obsolètes. Les projets qui ciblent .NET 5,0 ou une version ultérieure rencontreront des avertissements au moment de la compilation si ces méthodes sont appelées. Si vous supprimez les avertissements, une <xref:System.PlatformNotSupportedException> est levée au moment de l’exécution.
