...
#### <a name="change-description"></a><span data-ttu-id="c77ec-103">Description de la modification</span><span class="sxs-lookup"><span data-stu-id="c77ec-103">Change description</span></span>

<span data-ttu-id="c77ec-104">Auparavant, `NaN` les entrées dans les <xref:System.Runtime.Intrinsics.X86.Sse> méthodes <xref:System.Runtime.Intrinsics.X86.Sse2> et retournaient un résultat incorrect.</span><span class="sxs-lookup"><span data-stu-id="c77ec-104">Previously, `NaN` inputs to the listed <xref:System.Runtime.Intrinsics.X86.Sse> and <xref:System.Runtime.Intrinsics.X86.Sse2> methods returned an incorrect result.</span></span> <span data-ttu-id="c77ec-105">Le résultat est également différent du résultat généré par la méthode correspondante dans la <xref:System.Runtime.Intrinsics.X86.Avx> classe.</span><span class="sxs-lookup"><span data-stu-id="c77ec-105">The result also differed from the result generated by the corresponding method in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="c77ec-106">À compter de .NET 5,0, ces méthodes gèrent `NaN` correctement les entrées et retournent les mêmes résultats que les <xref:System.Runtime.Intrinsics.X86.Avx> méthodes correspondantes dans la classe.</span><span class="sxs-lookup"><span data-stu-id="c77ec-106">Starting in .NET 5.0, these methods correctly handle `NaN` inputs and return the same results as the corresponding methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="c77ec-107">Les architectures de streaming SIMD Extensions (SSE) et SSE2 (streaming SIMD Extensions 2) (ISA) ne fournissent pas de prise en charge matérielle directe pour ces méthodes de comparaison, afin qu’elles soient implémentées dans le logiciel.</span><span class="sxs-lookup"><span data-stu-id="c77ec-107">The Streaming SIMD Extensions (SSE) and Streaming SIMD Extensions 2 (SSE2) industry standard architectures (ISAs) don't provide direct hardware support for these comparison methods, so they're implemented in software.</span></span> <span data-ttu-id="c77ec-108">Auparavant, les méthodes étaient implémentées de manière incorrecte et elles ne `NaN` géraient pas correctement les entrées.</span><span class="sxs-lookup"><span data-stu-id="c77ec-108">Previously, the methods were improperly implemented, and they incorrectly handled `NaN` inputs.</span></span> <span data-ttu-id="c77ec-109">Dans le cas d’un code natif, le comportement incorrect peut introduire des bogues.</span><span class="sxs-lookup"><span data-stu-id="c77ec-109">For code ported from native, the incorrect behavior may introduce bugs.</span></span> <span data-ttu-id="c77ec-110">Pour un chemin d’accès de code 256 bits, les méthodes peuvent également produire des résultats différents aux méthodes équivalentes dans la <xref:System.Runtime.Intrinsics.X86.Avx> classe.</span><span class="sxs-lookup"><span data-stu-id="c77ec-110">For a 256-bit code path, the methods can also produce different results to the equivalent methods in the <xref:System.Runtime.Intrinsics.X86.Avx> class.</span></span>

<span data-ttu-id="c77ec-111">En guise d’exemple de la façon dont les méthodes étaient précédemment incorrectes, vous pouvez implémenter `CompareNotGreaterThan(x,y)` comme `CompareLessThanOrEqual(x,y)` pour les entiers normaux.</span><span class="sxs-lookup"><span data-stu-id="c77ec-111">As an example of how the methods were previously incorrect, you can implement `CompareNotGreaterThan(x,y)` as `CompareLessThanOrEqual(x,y)` for regular integers.</span></span> <span data-ttu-id="c77ec-112">Toutefois, pour `NaN` les entrées, cette logique calcule le résultat incorrect.</span><span class="sxs-lookup"><span data-stu-id="c77ec-112">However, for `NaN` inputs, that logic computes the wrong result.</span></span> <span data-ttu-id="c77ec-113">Au lieu de `CompareNotLessThan(y,x)` cela, l’utilisation de compare `NaN` les nombres correctement *et* prend en compte les entrées.</span><span class="sxs-lookup"><span data-stu-id="c77ec-113">Instead, using `CompareNotLessThan(y,x)` compares the numbers correctly *and* takes `NaN` inputs into consideration.</span></span>

#### <a name="version-introduced"></a><span data-ttu-id="c77ec-114">Version introduite</span><span class="sxs-lookup"><span data-stu-id="c77ec-114">Version introduced</span></span>

<span data-ttu-id="c77ec-115">5,0 Preview 4</span><span class="sxs-lookup"><span data-stu-id="c77ec-115">5.0 Preview 4</span></span>

#### <a name="recommended-action"></a><span data-ttu-id="c77ec-116">Action recommandée</span><span class="sxs-lookup"><span data-stu-id="c77ec-116">Recommended action</span></span>

- <span data-ttu-id="c77ec-117">Si le comportement précédent était un bogue, aucune modification n’est requise.</span><span class="sxs-lookup"><span data-stu-id="c77ec-117">If the previous behavior was a bug, no change is required.</span></span>

- <span data-ttu-id="c77ec-118">Si le comportement précédent était souhaité, vous pouvez maintenir ce comportement en modifiant l’appel approprié comme suit :</span><span class="sxs-lookup"><span data-stu-id="c77ec-118">If the previous behavior was desired, you can maintain that behavior by changing the relevant invocation as follows:</span></span>
...
#### <a name="category"></a><span data-ttu-id="c77ec-119">Catégorie</span><span class="sxs-lookup"><span data-stu-id="c77ec-119">Category</span></span>

<span data-ttu-id="c77ec-120">Bibliothèques .NET Core</span><span class="sxs-lookup"><span data-stu-id="c77ec-120">Core .NET libraries</span></span>

#### <a name="affected-apis"></a><span data-ttu-id="c77ec-121">API affectées</span><span class="sxs-lookup"><span data-stu-id="c77ec-121">Affected APIs</span></span>
