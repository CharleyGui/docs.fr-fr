...
Cela fait partie d’un effort plus important pour améliorer la gestion d’UTF-8 dans .NET, y compris par les nouveaux types <xref:System.Text.Unicode.Utf8?displayProperty=nameWithType> et <xref:System.Text.Rune?displayProperty=nameWithType>. Le type <xref:System.Text.UTF8Encoding> a obtenu des mécanismes améliorés de gestion des erreurs afin de produire une sortie cohérente avec les types nouvellement introduits.
